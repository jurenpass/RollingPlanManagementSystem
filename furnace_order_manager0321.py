...
class FurnaceOrderManager:
    ...
    def run_excel_macro_with_pandas(self, file_path):
        ...
        # 8. 设置列宽（根据新的字段顺序）
        # 新顺序：序号、钢卷号、牌号（钢级）、坯宽、减宽、调宽、轧宽、公差带、粗轧报信、除鳞、坯厚、坯长、轧厚、中厚、RT2、强度、装炉顺、订宽、去向、切边
        col_widths = [
            ...
            int(6 * 256 * 1.04),         # 11. 坯厚
            int(6.2 * 256 * 1.13),       # 12. 坯长
            int(6 * 256 * 1.04),         # 13. 轧厚
            int(5 * 256 * 1.04),         # 14. 中厚（中间坯厚度设定值）
            int(6.57 * 256 * 1.04),      # 15. RT2（RT2目标值）
            int(3.6 * 256 * 1.2),        # 16. 强度（硬度组）
            int(7 * 256 * 1.1),        # 17. 装炉顺（装炉顺序号）
            int(5 * 256 * 1.17),         # 18. 订宽（订货宽度）
            int(3.6 * 256 * 1.2),        # 19. 去向
            int(3.6 * 256 * 1.2),        # 20. 切边（切边方式）
            # 保留的其他字段
    ...
    def print_excel_file(self, file_path):
        ...
        try:
            ...
            # 即使找不到装炉顺列，也强制设置打印范围为前17列（根据标准列顺序）
            if 装炉顺列索引 == -1:
                print("未找到装炉顺序号/装炉顺列，使用默认打印范围：前17列")
                装炉顺列索引 = 16  # 默认第17列（索引16）为装炉顺列
            else:
            ...
            try:
                ...
                # 设置每页打印表头
                worksheet.PageSetup.PrintTitleRows = "$1:$1"  # 第一行作为表头
