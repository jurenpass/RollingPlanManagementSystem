...
class FurnaceOrderManager:
    ...
    def update_next_execution_time(self, next_exec_time_str):
        ...
        try:
            ...
            if hasattr(self, 'next_execution_label_main'):
                try:
                    self.next_execution_label_main.config(text=f"下一次执行: {next_exec_time_str}")
            ...
            if hasattr(self, 'next_execution_label_furnace'):
                try:
                    self.next_execution_label_furnace.setText(f"下一次执行: {next_exec_time_str}")
                except Exception as e:
    ...
    def start_auto_execution(self):
        ...
        with self.auto_exec_lock:
            # 停止现有的自动执行服务
            if self.is_auto_exec_running:
                print("自动执行服务已经在运行，停止现有服务...")
                self.is_auto_exec_running = False
                # 清理线程引用，不等待线程完全停止，提高响应速度
                self.auto_exec_timer = None
                    ...
                    try:
                        ...
                        if exec_mode == "interval":
                            ...
                            if self.is_auto_exec_running:
                                ...
                                # 检查是否已经在最近执行过（避免重复执行）
                                if self.last_interval_exec_time is None or current_time - self.last_interval_exec_time > interval * 0.9:
                                    print(f"执行时间到达，执行自动导出...")
                                    # 记录执行时间
                                    self.last_interval_exec_time = current_time
                                    # 关闭装炉明细画面（如果存在）
                            ...
                            if exec_times:
                                ...
                                if valid_times:
                                    ...
                                    for hour, minute in valid_times:
                                        # 计算时间差（分钟）
                                        time_diff = (hour - current_hour) * 60 + (minute - current_minute)
                                        if time_diff <= 0:
                                            # 今天的时间已经过了，使用明天的时间
                                            time_diff += 24 * 60
                                        if time_diff < min_time_diff:
                                    ...
                                    if next_exec_time:
                                        ...
                                        if self.is_auto_exec_running:
                                            ...
                                            # 检查是否已经执行过
                                            if current_time_key != self.last_executed_time:
                                                print(f"执行时间到达，当前时间: {current_hour:02d}:{current_minute:02d}")
                                                print("执行自动导出...")
                                                # 记录执行时间
                                                self.last_executed_time = current_time_key
                                                # 关闭装炉明细画面（如果存在）
                                                if hasattr(self, 'furnace_detail_window') and self.furnace_detail_window is not None:
                                                    try:
                                                        self.furnace_detail_window.close()
                                                ...
                                                print(f"当前时间 {current_hour:02d}:{current_minute:02d} 已经执行过，跳过")
                            else:
                                # 没有指定时间，等待1分钟后重新检查
                                print("没有指定执行时间，等待1分钟后重新检查")
                                time.sleep(60)
                        else:
                            # 无效的执行模式，等待1分钟后重新检查
                            print(f"无效的执行模式: {exec_mode}，等待1分钟后重新检查")
                            time.sleep(60)
                    except Exception as e:
                        print(f"自动执行服务错误: {e}")
                        import traceback
...
class ExportConfigGUI:
    ...
    def create_widgets(self):
        ...
        def on_spacing_focus_out(event):
            try:
                ...
                # 非整数输入，显示错误
                self.custom_messagebox("错误", "请输入有效的整数")
                # 不再自动设置焦点，让用户自己处理
        ...
        def on_spacing_enter(event):
            try:
                ...
                if value > 0:
                    ...
                    # 超出范围，显示错误
                    self.custom_messagebox("错误", "请输入大于0的数值")
                    # 保持焦点在文本框并全选内容
                    spacing_entry.focus()
        ...
        ttk.Label(anti_logout_frame, text="防止退出登录时间间隔:", font=('微软雅黑', 10, 'bold')).pack(side=tk.LEFT, padx=5)
        ...
        def on_focus_out(event):
            ...
            # 防止重复触发
            if hasattr(self, 'is_showing_message') and self.is_showing_message:
                return
            ...
            try:
                ...
                if value < 30:
                    ...
                    self.custom_messagebox("错误", "请输入30-999之间的数值")
                    self.is_showing_message = False
                    # 保持焦点在文本框并全选内容
        ...
        ttk.Label(anti_logout_frame, text="(30-999)", foreground="gray", font=('微软雅黑', 9)).pack(side=tk.LEFT, padx=5)
        ...
        def on_exec_interval_focus_out(event):
            ...
            try:
                ...
                if value < 5:
                    # 小于最小值，显示错误
                    self.is_showing_message = True
                    self.custom_messagebox("错误", "请输入5-999之间的数值")
                    self.is_showing_message = False
                    # 保持焦点在文本框并全选内容
        ...
        ttk.Label(interval_mode_frame, text="(5-999)", foreground="gray", font=('微软雅黑', 9)).pack(side=tk.LEFT, padx=5)
        ...
        ttk.Label(time_input_frame, text="例如: 08:00, 12:00, 16:00", foreground="gray", font=('微软雅黑', 9)).pack(anchor=tk.W, pady=2)
            ...
            if time_str:
                ...
                for t in times:
                    if len(t) != 5 or t[2] != ':':
                        # 格式错误，显示错误
                        self.is_showing_message = True
                        self.custom_messagebox("错误", "时间格式错误，请输入正确的时间格式 HH:MM")
                        self.is_showing_message = False
                        # 保持焦点在文本框并全选内容
                        time_entry.focus()
    ...
    def create_coord_row(self, parent, label_text, coord_key, string_var):
        ...
        def on_coord_focus_out(event):
            ...
            if coord:
                ...
                # 无效输入，显示错误
                self.custom_messagebox("错误", "请输入有效的坐标格式 (X, Y)")
                # 不再自动设置焦点，让用户自己处理
    ...
    def custom_messagebox(self, title, message, msg_type='info'):
        ...
        if msg_type == 'yesno':
            ...
            yes_btn = ttk.Button(btn_frame, text="是", command=on_yes, width=10)
            yes_btn.pack(side=tk.LEFT, padx=10)
            no_btn = ttk.Button(btn_frame, text="否", command=on_no, width=10)
            no_btn.pack(side=tk.LEFT, padx=10)
            ...
            msg_window.lift()
            msg_window.focus_force()
            msg_window.grab_set()  # 添加grab_set确保窗口捕获所有事件
            yes_btn.focus_set()
            msg_window.wait_window()
            msg_window.grab_release()  # 释放grab
            ...
            ok_btn = ttk.Button(btn_frame, text="确定", command=on_ok, width=10)
            ok_btn.pack()
