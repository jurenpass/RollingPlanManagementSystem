...
class FurnaceOrderManager:
    ...
    def furnace_details(self):
        ...
        # 创建自定义Treeview类，支持单元格级别的样式
        class CellStyledTreeview(ttk.Treeview):
            ...
            def tag_configure(self, tagname, **kw):
                super().tag_configure(tagname, **kw)
            
            def set_cell_style(self, item, column, **kw):
                """设置单元格的样式"""
                if item not in self.cell_styles:
                    self.cell_styles[item] = {}
                self.cell_styles[item][column] = kw
                # 触发重绘
            ...
            def item(self, item, option=None, **kw):
                if option is not None:
                    ...
                    result = super().item(item, **kw)
                return result
        
        # 创建表格
        tree = CellStyledTreeview(
            table_frame,
            columns=columns,
            show='headings',
        ...
        def refresh_data():
            ...
            try:
                ...
                if not furnace_data:
                    print("没有装炉数据可处理")
                    status_label.config(text="没有装炉数据可处理")
                    return
                ...
                for item in furnace_data:
                    ...
                    if plan_file:
                        ...
                        if coil_data:
                            ...
                            # 插入数据
                            item_id = tree.insert('', tk.END, values=row_data)
                            
                            # 检查每个单元格是否有标识（三角符号Δ或星号*）
                            has_marked = False
                            ...
                            # 如果有标注，为整行应用黄色背景
                            if has_marked:
                                tag_name = f'marked_{item_id}'
                                # 配置标签样式（黄色背景）
                                tree.tag_configure(tag_name, background='#FFFF00', foreground=默认配色["表格文字"])
                                # 应用标签到整行
                ...
                import traceback
                traceback.print_exc()
                status_label.config(text=f"加载失败: {str(e)}")
