...
class FurnaceOrderManager:
    ...
    def start_auto_execution(self):
        ...
        with self.auto_exec_lock:
            ...
            self.is_executing = False  # 添加执行状态标记
                        ...
                        if should_execute:
                            ...
                            if last_executed_datetime is None or (now - last_executed_datetime).total_seconds() > 60:
                                ...
                                try:
                                    ...
                                    time.sleep(0.1)
                                    # 检查是否在装炉明细画面中
                                    in_furnace_detail = hasattr(self, 'furnace_detail_window') and self.furnace_detail_window is not None
                                    # 使用主线程执行GUI操作，根据是否在装炉明细画面中设置from_main_window参数
                                    self.root.after(0, lambda: self.auto_export(from_main_window=not in_furnace_detail))
    ...
    def auto_export(self, from_main_window=True):
        ...
        try:
            ...
            self.is_executing = False  # 重置执行状态
            self.custom_messagebox("错误", "pyautogui未安装，请先安装: pip install pyautogui")
            return
        ...
        try:
            ...
            if not no_file_plans:
                ...
                if progress_window:
                    progress_window.destroy()
                # 当没有需要导出的计划号时，不显示消息框，直接返回
                # 这样可以避免消息框阻塞主线程，导致auto_execution_function重复执行
                print("没有需要导出的计划号，直接返回")
                # 激活主程序窗口
                self.root.after(0, lambda:
                    (self.root.lift(),
                     self.root.focus_force(),
...
class ExportConfigGUI:
    ...
    def create_widgets(self):
        ...
        def update_next_anti_logout_time_display():
            if self.main_app:
                # 检查防退出登录服务是否正在运行
                if hasattr(self.main_app, 'is_anti_logout_running') and self.main_app.is_anti_logout_running:
                    if hasattr(self.main_app, 'anti_logout_target_execution_time'):
                        import time
                        ...
                        # 如果没有目标执行时间，尝试启动防退出登录服务以获取
                        if hasattr(self.main_app, 'anti_logout_var') and self.main_app.anti_logout_var.get():
                            try:
                                self.main_app.stop_anti_logout()
                                self.main_app.start_anti_logout()
                                # 等待一下让服务初始化
        ...
        def update_next_exec_time_display():
            if self.main_app:
                # 确保自动执行服务正在运行
                if hasattr(self.main_app, 'is_auto_exec_running') and self.main_app.is_auto_exec_running:
