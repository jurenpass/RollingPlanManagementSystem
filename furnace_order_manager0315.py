...
class FurnaceOrderManager:
    ...
    def export_zhuanglu_shunxu(self, add_debug_log=None):
        ...
        if os.path.exists(zhuanglu_file):
            try:
                os.remove(zhuanglu_file)
                add_debug_log(f"✓ 已删除原装炉顺序.xls文件")
            except Exception as e:
                add_debug_log(f"✗ 删除原装炉顺序.xls文件失败: {str(e)}")
    ...
    def export_zhizhi_plan_list(self, test_window=None, test_mode=False, add_debug_log=None):
        ...
        try:
            ...
            # 导出前删除原文件，避免文件占用
            if os.path.exists(total_plan_file):
                try:
                    os.remove(total_plan_file)
                    add_debug_log(f"✓ 已删除原总计划号列表.xls文件")
                except Exception as e:
                    add_debug_log(f"✗ 删除原总计划号列表.xls文件失败: {str(e)}")
