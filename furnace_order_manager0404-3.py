    ...
    def auto_export(self, from_main_window=True):
        ...
        try:
            ...
            if not no_file_plans:
                ...
                self.root.attributes('-topmost', True)
                self.root.attributes('-topmost', False)
                # 无论是否在前台，都显示提示窗口，3秒后自动关闭
                self.custom_messagebox("完成", "没有需要导出的计划号", auto_close=3)
                return
        ...
        try:
            ...
            if not from_main_window:
                ...
                print("打开装炉明细窗口并恢复状态...")
                # 使用主线程执行GUI操作
                self.root.after(1000, self.furnace_details)
        except Exception as e:
            print(f"刷新界面失败: {e}")
    ...
    def furnace_details(self):
        ...
        def show_context_menu(event):
            ...
            if row >= 0:
                ...
                # 创建右键菜单
                context_menu = QMenu()
                scroll_action = context_menu.addAction("开始滚动" if not is_scrolling else "停止滚动")
                
                # 连接菜单项到槽函数
                scroll_action.triggered.connect(toggle_scroll)
        ...
        def save_window_state():
            ...
            try:
                ...
                print(f"保存装炉明细窗口状态: 滚动位置={self.furnace_detail_scroll_state}, 光标行={self.furnace_detail_cursor_row}")
            except Exception as e:
                print(f"保存窗口状态失败: {e}")
