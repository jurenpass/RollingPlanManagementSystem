...
class MainWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        self.table_widget.setHorizontalHeaderLabels(["计划b7, "块数", "状?, "起止钢卷?])
        ...
        # 统计信息标签
        # 总块数显?
        self.stats_label = QLabel("总块? 0 | 无文件 0")
    ...
    def create_required_folders(self):
        """自动创建所需的文件夹和文件""
        # 创建计划号文件夹
        plan_dir = os.path.join(self.plan_dir, "计划b7)
    ...
    def cleanup_backup_directory(self, backup_dir):
        """清理backup目录中超过12小时的文件,将其移动到原始文件"子目录
        ...
        try:
            ...
            with open(no_aps_file, 'w', encoding='utf-8') as f:
                for plan_no in self.no_aps_plans:
                    f.write(f"{plan_no}\n")
            print(f"已保?{len(self.no_aps_plans)} 个无APS计划b7)
        except Exception as e:
        ...
        try:
            ...
            with open(low_roll_width_file, 'w', encoding='utf-8') as f:
                for plan_no in self.low_roll_width_plans:
                    f.write(f"{plan_no}\n")
            print(f"已保?{len(self.low_roll_width_plans)} 个低轧宽计划b7)
        except Exception as e:
                ...
                try:
                    ...
                    if event.success:
                        ...
                        if event.exported_plans:
                            ...
                            print(f"已更新最近导出的计划b7 {event.exported_plans}")
                            # 刷新计划号列?使绿色标注生?
                            ...
                            print(f"已选中 {len(event.exported_plans)} 个导出的计划b7)
    ...
    def scan_and_rename_plan_files(self):
        ...
        try:
            ...
            print(f"扫描完成: 处理 {len(files)} 个文件跳过 {skipped_count} 个文件)
        except Exception as e:
    ...
    def process_plans(self, auto_print=False, show_result=True):
        ...
        try:
            ...
            if not selected_items:
                ...
                msg_box.setText("请先在计划号列表中选择要处理的计划b7)
    ...
    def print_furnace_details(self):
        ...
        try:
            ...
            # 设置列名
            提示?= ["装炉明细打印"]
            headers = ["序号", "计划b7, "钢卷?, "牌号(钢级)", "坯宽", "减宽", "调宽", "轧宽", "公差?,
    ...
    def run_excel_macro_with_pandas(self, file_path):
        ...
        装炉顺序映射 = {}
        轧宽余量映射 = {}
        装炉顺序文件 = os.path.join(self.plan_dir, "计划b7, "装炉顺序.xls")
        
        # 加载?工作表的钢种列表
        钢种列表 = []
        ?工作表文件= os.path.join(self.plan_dir, "计划b7, "装炉顺序.xls")  # 假设?工作表在同一个文件中
        if os.path.exists(?工作表文件:
            success, result = self.safe_read_excel(?工作表文件 max_retries=3, retry_delay=0.5)
            if success:
                workbook3 = result
                try:
        ...
        target_columns = [
            ...
            "计划b7, "热轧产品分类", "炼钢钢种", "宽度负公?, "宽度正公?, "板坯炉后拒收次数", "轧宽",
            "装炉顺序?
        ]
        ...
        for row_idx in range(1, sheet.nrows):
            if "计划b7 in current_columns:
                计划号?= sheet.cell_value(row_idx, current_columns.index("计划b7))
                if 计划号?
                    计划b7= str(计划号?
                    break
        ...
        if 计划b7and (计划b7startswith('D') or 计划b7startswith('d')):
            filtered_row_data = []
        ...
        try:
            ...
            if not os.path.exists(excel_file):
                ...
                if os.path.exists(backup_excel_file):
                    ...
                    self.plan_data = []
                    self.update_table()
                    self.stats_label.setText("总块? 0 | 无文件 0")
                    return False
                ...
                try:
                    ...
                    if plan_no and coil_no:
                        ...
                        if row % 50 == 0:  # ?0行打印一?减少输出
                            print(f"加载数据: 装炉顺序={order}, 计划b7{plan_no}, 钢卷?{coil_no} (?{row})")
                    else:
                        if row % 100 == 0:  # ?00行打印一?减少输出
                            print(f"跳过空数据行: 计划b7'{plan_no}', 钢卷?'{coil_no}' (?{row})")
                except Exception as e:
                    print(f"读取?{row} 失败: {e}")
                    # 继续处理下一?不中断整个加载过?
                    continue
            ...
            # 更新表格
            self.update_table()
            self.stats_label.setText(f"总块? {total_count} | 无文件 {no_file_count}")
            
            print(f"成功加载 {len(self.plan_data)} 个计划号,总计 {total_count} ?无文件{no_file_count} ?)
            return True
    ...
    def show_selected(self):
        ...
        try:
            ...
            if not plan_item:
                QMessageBox.warning(self, "错误", "无法获取计划b7)
                return
    ...
    def get_plan_file(self, plan_no):
        ...
        if os.path.exists(backup_dir):
            ...
            if os.path.exists(file_path):
                print(f"找到计划号文件backup): {file_path}")
                return file_path
    ...
    def print_selected(self):
        ...
        try:
            ...
            selected_items = self.table_widget.selectionModel().selectedRows()
            if not selected_items:
                QMessageBox.warning(self, "警告", "请先在计划号列表中选择要打印的计划b7)
                return
            ...
            # 构建选中计划号的文件路径列表
            selected_files = []
            update_progress("正在检查文件..", f"?{len(selected_plans)} 个计划号")
            ...
            for plan_no in selected_plans:
                ...
                if os.path.exists(file_path):
                    ...
                    if os.path.exists(backup_file_path):
                        ...
                        update_progress("正在检查文件..", f"文件不存? {plan_no}.xls")
    ...
    def print_plan(self, plan_no):
        ...
        try:
            # 找到计划号文件
            plan_dir = os.path.join(self.plan_dir, "计划b7, plan_no)
            if not os.path.exists(plan_dir):
    ...
    def export_zhizhi_plan_list(self, test_window=None, test_mode=False, add_debug_log=None):
        ...
        try:
            ...
            add_debug_log(f"  选择计划b7 {plan_select}")
    ...
    def export_single_plan_detail(self, plan_no, plan_coord, plan_detail_export_btn=None, test_window=None, add_debug_log=None):
        ...
        try:
            ...
            # 步骤4: 键盘输入计划号(使用剪贴板粘贴)
            add_debug_log(f"    [4] 输入计划b7 {plan_no}")
            try:
                import pyperclip
            ...
            # 模拟操作执行完成,直接标记为成?
            # 不检查文件是否存?只按照模拟操作导出了哪些计划b7
            print(f"[√] 计划b7{plan_no} 的明细文件已导出")
            result = True
        ...
        try:
            ...
            for row_index, (plan_no, seq_value) in enumerate(plan_data):
                if plan_no:
                    ...
                    # 只显示前13?避免日志过长
                    if row_index < 13:
                        add_debug_log(f"  计划b7{plan_no}: 行索?{row_index}, 坐标=({coord_x}, {coord_y})")
                    elif row_index == 13:
...
class FurnaceDetailsWindow(QMainWindow):
    ...
    def __init__(self, parent=None):
        ...
        # 加载保存的自定义信息和换辊信?
        plan_dir = os.path.join(os.getcwd(), "计划b7)
    ...
    def load_furnace_data(self):
        ...
        try:
            ...
            if hasattr(self, 'parent') and self.parent and hasattr(self.parent, 'plan_dir'):
                plan_dir = os.path.join(self.parent.plan_dir, "计划b7)
            ...
            for row in range(1, sheet.nrows):
                try:
                    ...
                    if plan_no and coil_no_clean:
                        ...
                        print(f"加载数据: 装炉顺序={order}, 计划b7{plan_no}, 钢卷?{coil_no}, 纯净钢卷?{coil_no_clean}")
                    else:
                        print(f"跳过空数据行: 计划b7'{plan_no}', 钢卷?'{coil_no}', 纯净钢卷?'{coil_no_clean}'")
                except Exception as e:
                    print(f"读取?{row} 失败: {e}")
                    continue
