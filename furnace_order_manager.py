...
if sys.platform == 'win32':
    import io
    # 检查sys.stdout和sys.stderr是否为None
    if hasattr(sys, 'stdout') and sys.stdout and hasattr(sys.stdout, 'buffer'):
        sys.stdout = io.TextIOWrapper(sys.stdout.buffer, encoding='utf-8', errors='replace')
    if hasattr(sys, 'stderr') and sys.stderr and hasattr(sys.stderr, 'buffer'):
...
class FurnaceOrderManager:
    def __init__(self, root):
        ...
        # 确保debug_mode为False，不显示监控窗口
        self.coordinates["debug_mode"] = False
        self.processed_plans = set()  # 已处理的计划号集合
        self.printed_plans = set()  # 已打印的计划号集合
        self.no_aps_plans = set()  # 包含无APS钢种的计划号集合
        self.low_roll_width_plans = set()  # 包含低轧宽板坯的计划号集合
        ...
        self.is_auto_export_running = False  # 新增：防止auto_export递归调用
        ...
        # 装炉明细窗口状态
        self.furnace_detail_scroll_state = 0  # 滚动条位置
        self.furnace_detail_cursor_row = 0  # 光标行位置
        ...
        # 根据保存的状态启动服务（复选框已在create_ui中设置，这里只启动服务）
        ...
        # 绑定窗口关闭事件
        self.root.protocol("WM_DELETE_WINDOW", on_main_window_close)
    ...
    def create_ui(self):
        ...
        # 防退出登录选项
        self.anti_logout_var = tk.BooleanVar(value=self.coordinates.get("anti_logout_enabled", False))
        ...
        self.anti_logout_checkbox.pack(side=tk.RIGHT, padx=10)
        
        # 自动执行选项
        self.auto_exec_var = tk.BooleanVar(value=self.coordinates.get("auto_exec_enabled", False))
        
        # 创建自动执行选项框架
        auto_exec_frame = tk.Frame(stats_frame, bg='#f0f0f0')
        auto_exec_frame.pack(side=tk.RIGHT)
        ...
        self.auto_exec_checkbox = tk.Checkbutton(
            ...
            font=("宋体", 20),
            bg='#f0f0f0',
            command=self.on_auto_exec_toggle
        )
        self.auto_exec_checkbox.pack(side=tk.LEFT)
        
        # 添加下一次执行时间显示标签
        self.next_execution_label_main = tk.Label(
            auto_exec_frame,
            text="下一次执行: 无",
            font=("宋体", 14),
            fg="blue",
        ...
        self.next_execution_label_main.pack(side=tk.LEFT, padx=10)
        ...
        self.create_style_button(right_btn_frame, "打印", self.print_selected, width=6, tooltip="按计划号进行打印")
    
    def create_style_button(self, parent, text, command, width=6, is_active=False, tooltip=None):
        """创建ttk风格按钮"""
        btn = ttk.Button(
            parent,
            text=text,
        ...
        if tooltip:
            ...
            def show_tooltip(event):
                ...
                # 创建提示窗口
                tooltip_window = tk.Toplevel(btn)
                tooltip_window.wm_overrideredirect(True)
                tooltip_window.wm_geometry(f"+{event.x_root+10}+{event.y_root+10}")
                label = tk.Label(tooltip_window, text=tooltip, bg="lightyellow", relief="solid", borderwidth=1)
                label.pack()
    ...
    def custom_messagebox(self, title, message, msg_type='info', auto_close=None):
        ...
        if msg_type == 'yesno':
            ...
            # 绑定ESC键到"否"按钮
            msg_window.bind('<Escape>', lambda event: on_no())
            ...
            # 绑定ESC键到"确定"按钮
            msg_window.bind('<Escape>', lambda event: on_ok())
    ...
    def refresh_plan_list_from_file(self):
        ...
        try:
            ...
            for plan_no, count, min_coil, max_coil in plan_counts:
                ...
                # 从保存的集合中恢复低轧宽板坯状态
                if plan_no in self.low_roll_width_plans:
                    has_low_roll_width = True
    ...
    def on_anti_logout_toggle(self):
        """防退出登录选项状态变化处理"""
        enabled = self.anti_logout_var.get()
        self.coordinates["anti_logout_enabled"] = enabled
        self.save_coordinates()
        if enabled:
            self.start_anti_logout()
        else:
    ...
    def on_auto_exec_toggle(self):
        """自动执行选项状态变化处理"""
        enabled = self.auto_exec_var.get()
        self.coordinates["auto_exec_enabled"] = enabled
        self.save_coordinates()
        if enabled:
            print("自动执行已启用")
            self.start_auto_execution()
        ...
        # 更新设置窗口的下一次执行时间显示
        if hasattr(self, 'export_config_window') and hasattr(self.export_config_window, 'update_next_exec_time_display'):
            try:
                self.export_config_window.update_next_exec_time_display()
            except Exception as e:
                print(f"更新设置窗口执行时间显示失败: {e}")
    
    def stop_auto_execution(self):
        """停止自动执行服务"""
        if hasattr(self, 'is_auto_exec_running') and self.is_auto_exec_running:
            self.is_auto_exec_running = False
            print("自动执行服务已停止")
        # 更新显示为无
        self.update_next_execution_time("无")
    ...
    def start_anti_logout(self):
        ...
        # 获取防退出登录时间间隔（毫秒）
        interval_ms = self.coordinates.get("anti_logout_interval", 60000)  # 默认1分钟
        interval_seconds = interval_ms / 1000
        
        # 计算并记录目标执行时间
        import time
        self.anti_logout_target_execution_time = time.time() + interval_seconds
        
        # 更新显示
        next_time_str = time.strftime("%H:%M:%S", time.localtime(self.anti_logout_target_execution_time))
        self.update_next_anti_logout_time(next_time_str)
        
        # 启动定时器，等待设定的时间间隔后执行，之后每隔同样的时间间隔执行一次
        import threading
        import datetime
        def timer_function():
            while self.is_anti_logout_running:
                try:
                    ...
                    # 显示当前的下一次执行时间
                    print(f"防退出登录下一次执行时间: {next_time_str}")
                    
                    # 更新显示
                    self.root.after(0, lambda: self.update_next_anti_logout_time(next_time_str))
                    
                    # 解析下一次执行时间
                    try:
                        target_hour, target_minute, target_second = map(int, next_time_str.split(':'))
                    except:
                        print(f"解析防退出登录执行时间失败: {next_time_str}")
                        time.sleep(60)
                        continue
                    ...
                    print(f"防退出登录等待 {wait_time:.0f} 秒后执行...")
                    
                    # 分小段等待，以便能够及时响应停止信号
                    while wait_time > 0 and self.is_anti_logout_running:
                        wait_chunk = min(wait_time, 10)  # 每次等待10秒
                        time.sleep(wait_chunk)
                        wait_time -= wait_chunk
                    
                    # 检查是否应该执行
                    if not self.is_anti_logout_running:
                        print("防退出登录已停止")
                        break
                    ...
                    if time_diff <= 30:
                        print(f"防退出登录执行时间到达，当前时间: {now.strftime('%H:%M:%S')}")
                        self.anti_logout_action()
                        # 计算下一次目标执行时间
                        self.anti_logout_target_execution_time = time.time() + interval_seconds
                except Exception as e:
                    print(f"防退出登录定时器错误: {e}")
                    time.sleep(10)
        ...
        print(f"防退出登录已启动，每隔{interval_seconds}秒激活窗口并按下F2键")
    
    def stop_anti_logout(self):
        """停止防退出登录"""
        if hasattr(self, 'is_anti_logout_running') and self.is_anti_logout_running:
            self.is_anti_logout_running = False
            print("防退出登录已停止")
        # 更新显示为无
        self.update_next_anti_logout_time("无")
    
    def update_next_anti_logout_time(self, next_time_str):
        """更新防退出登录下一次执行时间显示（只更新设置窗口）"""
        try:
            # 更新设置窗口中的下一次执行时间标签
            if hasattr(self, 'export_config_window') and hasattr(self.export_config_window, 'next_anti_logout_label_settings'):
                try:
                    self.export_config_window.next_anti_logout_label_settings.config(text=next_time_str)
                except Exception as e:
                    print(f"更新设置窗口防退出登录执行时间标签失败: {e}")
        except Exception as e:
            print(f"更新防退出登录执行时间显示失败: {e}")
    
    def update_next_execution_time(self, next_exec_time_str):
        """更新下一次执行时间显示"""
        try:
            # 检查自动执行选项是否被选中
            display_time = next_exec_time_str
            if hasattr(self, 'auto_exec_var') and not self.auto_exec_var.get():
                display_time = "无"
            
            # 更新主程序中的下一次执行时间标签
            if hasattr(self, 'next_execution_label_main'):
                try:
                    self.next_execution_label_main.config(text=f"下一次执行: {display_time}")
                    self.next_execution_label_main.update()
                except Exception as e:
                    print(f"更新主程序执行时间标签失败: {e}")
            
            # 更新装炉明细画面中的下一次执行时间标签
            if hasattr(self, 'next_execution_label_furnace'):
                try:
                    self.next_execution_label_furnace.setText(f"下一次执行: {display_time}")
                except Exception as e:
                    print(f"更新装炉明细执行时间标签失败: {e}")
            
            # 更新设置窗口中的下一次执行时间标签
            if hasattr(self, 'export_config_window') and hasattr(self.export_config_window, 'next_exec_time_label_settings'):
                try:
                    self.export_config_window.next_exec_time_label_settings.config(text=display_time)
                except Exception as e:
                    print(f"更新设置窗口执行时间标签失败: {e}")
        except Exception as e:
            print(f"更新执行时间显示失败: {e}")
    
    def calculate_next_execution_time(self):
        """计算下一次执行时间"""
        import time
        # 获取自动执行模式
        exec_mode = self.coordinates.get("auto_exec_mode", "interval")
        
        # 确保设置生效时的秒数变量存在
        if not hasattr(self, 'execution_settings_second'):
            self.execution_settings_second = time.localtime().tm_sec
        
        if exec_mode == "interval":
            # 按时间间隔执行
            # 检查是否已经记录了目标执行时间
            if hasattr(self, 'interval_target_execution_time') and self.interval_target_execution_time is not None:
                # 使用已记录的目标执行时间
                next_time_str = time.strftime("%H:%M:%S", time.localtime(self.interval_target_execution_time))
                return next_time_str
            else:
                # 如果没有记录，计算新的目标执行时间
                interval = self.coordinates.get("auto_exec_interval", 30) * 60  # 转换为秒
                self.interval_target_execution_time = time.time() + interval
                next_time_str = time.strftime("%H:%M:%S", time.localtime(self.interval_target_execution_time))
                return next_time_str
        elif exec_mode == "time":
            # 按指定时间执行
            ...
            if hasattr(self, 'time_mode_target_execution_time') and self.time_mode_target_execution_time is not None:
                # 使用已记录的目标执行时间
                next_time_str = time.strftime("%H:%M:%S", time.localtime(self.time_mode_target_execution_time))
                return next_time_str
            
            # 如果没有记录，计算新的目标执行时间
            exec_times = self.coordinates.get("auto_exec_times", "").strip()
            if exec_times:
                # 解析时间
                ...
                if valid_times:
                    ...
                    # 检查是否有上次执行的时间记录（用于执行后重新计算）
                    if hasattr(self, 'last_executed_plan_time') and self.last_executed_plan_time:
                        try:
                            # 使用上次执行的时间作为参考点
                            exec_hour, exec_minute = map(int, self.last_executed_plan_time.split(':'))
                            current_hour = exec_hour
                            current_minute = exec_minute
                            current_second = 0  # 从0秒开始计算
                            print(f"使用上次执行时间作为参考: {self.last_executed_plan_time}")
                        except Exception as e:
                            print(f"解析上次执行时间失败: {e}")
                    ...
                    # 如果今天没有未到的时间，使用明天最早的时间
                    if not found_today:
                        # 重置最小时间差
                        min_time_diff = float('inf')
                        for hour, minute in valid_times:
                            # 计算时间差（秒），使用明天的时间
                            time_diff = (hour - current_hour) * 3600 + (minute - current_minute) * 60 - current_second + 24 * 3600
                            if time_diff < min_time_diff:
                                min_time_diff = time_diff
                                next_exec_time = (hour, minute)
                                target_hour = hour
                    ...
                    if next_exec_time:
                        # 构造下一次执行时间（使用目标小时、目标分钟、设置生效时的秒数）
                        # 获取当前日期
                        today = datetime.date.today()
                        # 构造目标时间，使用设置生效时记录的秒数
                        target_time = datetime.datetime(today.year, today.month, today.day, target_hour, target_minute, self.execution_settings_second)
                        # 如果目标时间已过（包括当前正在执行的时间），使用明天的日期
                        now_datetime = datetime.datetime.now()
                        if target_time <= now_datetime:
                            target_time += datetime.timedelta(days=1)
                        # 转换为时间戳并存储
                        self.time_mode_target_execution_time = target_time.timestamp()
                        # 格式化时间，使用设置生效时的秒数
                        next_time_str = target_time.strftime("%H:%M:%S")
                        print(f"计算指定时间模式目标执行时间: {next_time_str}")
                        return next_time_str
            return "无"
        else:
            return "无"
    ...
    def start_auto_execution(self):
        """启动自动执行服务"""
        # 确保线程锁存在
        if not hasattr(self, 'auto_exec_lock'):
            import threading
            self.auto_exec_lock = threading.Lock()
        
        # 使用线程锁确保方法的原子性
        with self.auto_exec_lock:
            # 如果服务已经在运行，先停止
            if hasattr(self, 'is_auto_exec_running') and self.is_auto_exec_running:
                print("自动执行服务已经在运行，先停止现有服务...")
                self.is_auto_exec_running = False
                # 等待旧线程结束（最多等待2秒）
                if hasattr(self, 'auto_exec_timer') and self.auto_exec_timer is not None:
                    self.auto_exec_timer.join(timeout=2)
                    self.auto_exec_timer = None
            
            # 重置所有执行时间跟踪变量
            self.last_executed_time = None
            self.last_interval_exec_time = None  # 记录上次间隔执行时间（类成员变量）
            ...
            self.update_next_execution_time(next_exec_time_str)
            
            # 启动新的自动执行服务
            import threading
            def auto_execution_function():
                import time
                import datetime
                print("自动执行服务线程已启动")
                
                # 确保last_executed_datetime作为类成员变量存在
                if not hasattr(self, 'last_executed_datetime'):
                    self.last_executed_datetime = None
                
                # 确保interval_target_execution_time存在
                if not hasattr(self, 'interval_target_execution_time') or self.interval_target_execution_time is None:
                    exec_mode = self.coordinates.get("auto_exec_mode", "interval")
                    if exec_mode == "interval":
                        interval = self.coordinates.get("auto_exec_interval", 30) * 60  # 转换为秒
                        self.interval_target_execution_time = time.time() + interval
                ...
                while self.is_auto_exec_running:
                    try:
                        ...
                        if next_exec_time_str == "无":
                            # 没有有效的执行时间，等待1分钟后重新检查
                        ...
                        # 更新显示
                        self.root.after(0, lambda: self.update_next_execution_time(next_exec_time_str))
                        ...
                        if exec_mode == "interval":
                            # 时间间隔模式：直接使用interval_target_execution_time
                            if hasattr(self, 'interval_target_execution_time') and self.interval_target_execution_time is not None:
                                ...
                                wait_time = interval
                                target_time = datetime.datetime.fromtimestamp(self.interval_target_execution_time)
                                print(f"重新计算目标执行时间: {target_time.strftime('%H:%M:%S')}")
                        else:
                            # 指定时间模式：使用time_mode_target_execution_time
                            if hasattr(self, 'time_mode_target_execution_time') and self.time_mode_target_execution_time is not None:
                                ...
                                if wait_time < 0:
                                    ...
                                    if next_time_str and next_time_str != "无":
                                        try:
                                            ...
                                            self.time_mode_target_execution_time = target_time.timestamp()
                                            wait_time = (target_time - now).total_seconds()
                                            print(f"重新计算指定时间模式目标执行时间: {target_time.strftime('%H:%M:%S')}")
                                        except Exception as e:
                        ...
                        # 记录是否被配置变化打断
                        interrupted_by_config_change = False
                        
                        # 分小段等待，以便能够及时响应停止信号和配置变化
                        while wait_time > 0 and self.is_auto_exec_running:
                            wait_chunk = min(wait_time, 5)  # 每次等待5秒，提高响应速度
                            time.sleep(wait_chunk)
                            wait_time -= wait_chunk
                            
                            # 每5秒检查一下下一次执行时间是否发生了变化（配置可能被修改）
                            new_next_exec_time_str = self.calculate_next_execution_time()
                            if new_next_exec_time_str != next_exec_time_str:
                                print(f"检测到下一次执行时间变化: 从 {next_exec_time_str} 变为 {new_next_exec_time_str}")
                                interrupted_by_config_change = True
                                break
                        ...
                        # 如果被配置变化打断，重新开始循环
                        if interrupted_by_config_change:
                            continue
                        ...
                        if exec_mode == "time":
                            ...
                            if hasattr(self, 'time_mode_target_execution_time') and self.time_mode_target_execution_time is not None:
                                ...
                                if current_time >= self.time_mode_target_execution_time:
                                    should_execute = True
                                    target_time = datetime.datetime.fromtimestamp(self.time_mode_target_execution_time)
                                    print(f"指定时间模式：时间到达，当前时间: {now.strftime('%H:%M:%S')}，目标时间: {target_time.strftime('%H:%M:%S')}")
                                else:
                                    target_time = datetime.datetime.fromtimestamp(self.time_mode_target_execution_time)
                                    print(f"指定时间模式：时间未到达，当前时间: {now.strftime('%H:%M:%S')}，目标时间: {target_time.strftime('%H:%M:%S')}")
                            else:
                                # 如果没有目标执行时间，重新计算
                                next_time_str = self.calculate_next_execution_time()
                                if next_time_str and next_time_str != "无":
                                    try:
                                        ...
                                        # 转换为时间戳并存储
                                        self.time_mode_target_execution_time = target_time.timestamp()
                                        print(f"指定时间模式：重新计算目标执行时间: {target_time.strftime('%H:%M:%S')}")
                                    except Exception as e:
                                        print(f"解析时间失败: {e}")
                                should_execute = False
                        else:
                            ...
                            if hasattr(self, 'interval_target_execution_time') and self.interval_target_execution_time is not None:
                                ...
                                if current_time >= self.interval_target_execution_time:
                                    should_execute = True
                                    print(f"时间间隔模式：时间到达，当前时间: {now.strftime('%H:%M:%S')}，目标时间: {datetime.datetime.fromtimestamp(self.interval_target_execution_time).strftime('%H:%M:%S')}")
                                else:
                                    print(f"时间间隔模式：时间未到达，当前时间: {now.strftime('%H:%M:%S')}，目标时间: {datetime.datetime.fromtimestamp(self.interval_target_execution_time).strftime('%H:%M:%S')}")
                            else:
                                # 如果没有目标执行时间，重新计算
                                interval = self.coordinates.get("auto_exec_interval", 30) * 60  # 转换为秒
                                self.interval_target_execution_time = time.time() + interval
                                print(f"时间间隔模式：重新计算目标执行时间: {datetime.datetime.fromtimestamp(self.interval_target_execution_time).strftime('%H:%M:%S')}")
                                should_execute = False
                        ...
                        if should_execute:
                            ...
                            if self.last_executed_datetime is None or (now - self.last_executed_datetime).total_seconds() > 60:
                                ...
                                # 关闭装炉明细画面（如果存在）
                                if hasattr(self, 'furnace_detail_window') and self.furnace_detail_window is not None:
                                    try:
                                        # 使用QTimer在主线程中关闭窗口
                                        from PyQt5.QtCore import QTimer
                                        QTimer.singleShot(0, self.furnace_detail_window.close)
                                        print("装炉明细画面已关闭")
                                        # 等待窗口关闭
                                        time.sleep(0.5)
                                    except Exception as e:
                                        print(f"关闭装炉明细画面失败: {e}")
                                # 先激活主窗口
                                ...
                                time.sleep(0.2)
                                # 使用主线程执行GUI操作
                                self.root.after(0, lambda: self.auto_export(from_main_window=False))
                                # 重新计算下一次目标执行时间
                                exec_mode = self.coordinates.get("auto_exec_mode", "interval")
                                if exec_mode == "interval":
                                    import time
                                    ...
                                    # 重新计算指定时间模式的下一次目标执行时间
                                    # 先清除当前的目标执行时间，强制重新计算
                                    self.time_mode_target_execution_time = None
                                    # 记录当前执行的时间点，用于计算下一个时间
                                    self.last_executed_plan_time = now.strftime("%H:%M")
                                    next_time_str = self.calculate_next_execution_time()
                                    print(f"执行后重新计算，当前执行时间: {self.last_executed_plan_time}，calculate_next_execution_time返回: {next_time_str}")
                                    if next_time_str and next_time_str != "无":
                                        try:
                                            # 直接使用calculate_next_execution_time返回的时间
                                            # 不需要再解析和重新构造，因为calculate_next_execution_time已经处理了日期逻辑
                                            print(f"重新计算指定时间模式下一次目标执行时间: {next_time_str}")
                                        except Exception as e:
                                            print(f"重新计算指定时间模式目标执行时间失败: {e}")
                                    # 清除记录
                                    if hasattr(self, 'last_executed_plan_time'):
                                        delattr(self, 'last_executed_plan_time')
                                # 立即更新下一次执行时间
                                next_exec_time_str = self.calculate_next_execution_time()
                                self.root.after(0, lambda: self.update_next_execution_time(next_exec_time_str))
                                # 等待自动导出完成（避免重复执行）
                                time.sleep(1)
                                # 重置last_executed_datetime，确保下次执行不会被跳过
                                # 注意：这里不应该重置，因为我们需要避免在同一时间重复执行
                                # 但为了确保不同时间的执行能够正常进行，我们使用60秒的时间窗口
                            else:
                                print(f"最近已执行过自动导出，跳过")
                                # 如果是指定时间模式，重新计算下一次执行时间，避免卡在同一个时间点
                                exec_mode = self.coordinates.get("auto_exec_mode", "interval")
                                if exec_mode == "time":
                                    # 重新计算指定时间模式的下一次目标执行时间
                                    # 先清除当前的目标执行时间，强制重新计算
                                    self.time_mode_target_execution_time = None
                                    next_time_str = self.calculate_next_execution_time()
                                    print(f"跳过执行重新计算，calculate_next_execution_time返回: {next_time_str}")
                                    if next_time_str and next_time_str != "无":
                                        try:
                                            # 直接使用calculate_next_execution_time返回的时间
                                            # 不需要再解析和重新构造，因为calculate_next_execution_time已经处理了日期逻辑
                                            print(f"跳过执行，重新计算指定时间模式下一次目标执行时间: {next_time_str}")
                                        except Exception as e:
                                            print(f"重新计算指定时间模式目标执行时间失败: {e}")
                                # 立即更新下一次执行时间
                                next_exec_time_str = self.calculate_next_execution_time()
            ...
            self.auto_exec_timer = threading.Thread(target=auto_execution_function, daemon=True)
    ...
    def auto_export(self, from_main_window=True):
        ...
        try:
            ...
            if debug_mode and hasattr(self, 'root') and self.root:
                # 检查是否已存在调试窗口
                ...
                if not hasattr(self, 'debug_window') or self.debug_window is None:
                    try:
                        ...
                        def add_debug_log(msg):
                            if hasattr(self, 'debug_text') and self.debug_text is not None and hasattr(self, 'debug_window') and self.debug_window is not None:
                                try:
                                    # 检查窗口是否还存在
            ...
            if show_warning:
                if is_in_foreground and hasattr(self, 'root') and self.root:
                    # 应用程序在前台，显示操作提示
                    try:
            ...
            def update_progress(main_msg, detail_msg=""):
                """更新进度显示"""
                if progress_window and progress_label and detail_label:
                    try:
                        progress_label.config(text=main_msg)
                        if detail_msg:
                            detail_label.config(text=detail_msg)
                        # 不使用update()，避免事件循环触发导致递归
            ...
            if not success1:
                ...
                raise Exception(f"装炉顺序导出失败: {log1}")
            ...
            if not no_file_plans:
                ...
                # 显示提示弹窗
                if is_in_foreground:
                    self.custom_messagebox("提示", "没有需要导出的计划号\n\n所有计划号都已有文件，无需导出", auto_close=3)
                
                # 返回到启动自动执行时的窗口
                if hasattr(self, 'previous_foreground_hwnd') and self.previous_foreground_hwnd:
                    try:
                        win32gui.SetForegroundWindow(self.previous_foreground_hwnd)
                        print(f"已返回到启动自动执行时的窗口")
                    except Exception as e:
                        print(f"返回到启动自动执行时的窗口失败: {e}")
                        # 如果失败，则返回到主窗口
                        self.root.lift()
                        self.root.focus_force()
                        self.root.attributes('-topmost', True)
            ...
            if exported_plans:
                ...
                if auto_print:
                    ...
                    if is_in_foreground:
                        ...
                        # 移除grab_set()，避免消息框捕获所有事件
                        # temp_window.grab_set()  # 模态化，阻止操作其他窗口
                        temp_window.lift()
                        temp_window.focus_force()
                        
                        try:
                            # 显示弹窗并获取用户选择，设置5秒自动关闭
                            user_choice = self.custom_messagebox("完成", detail_info, msg_type='yesno', auto_close=5)
                            # 释放模态抓取
            ...
            # 自动执行模式下，不打开装炉明细画面，避免递归调用
            # 如果不是从主程序画面启动的，才打开装炉明细画面
            # if not from_main_window:
            #     # 自动执行完打开装炉明细画面，恢复之前的滚动状态及光标条位置
            #     print("打开装炉明细窗口并恢复状态...")
            #     # 直接在单独的线程中打开装炉明细窗口，避免事件循环冲突
    ...
    def open_export_config(self):
        ...
        self.export_config_window = ExportConfigGUI(self.config_window, self.plan_dir, self)
    ...
    def furnace_details(self):
        ...
        # 保存窗口引用到主实例，以便自动执行服务可以关闭它
        self.furnace_detail_window = detail_window
        ...
        def update_block_count_display():
            """更新块数显示"""
            try:
                ...
                if selected_rows:
                    # 获取当前选中行的行号（从0开始，加1后显示）
                    current_row = selected_rows[0].row() + 1
                    block_count_label.setText(f"{current_row}/{total_rows}")
                else:
                    # 没有选中行时，显示0/总行数
                    block_count_label.setText(f"0/{total_rows}")
                ...
                try:
                    ...
                    block_count_label.setText("0/0")
        
        # 连接表格选择事件
        table.selectionModel().currentRowChanged.connect(lambda: update_block_count_display())
        ...
        # 左侧伸缩空间（减小比例，使块数显示往左移动2个字符的距离）
        status_layout.addStretch(2)
        
        # 块数显示（移到右边）
        block_count_label = QLabel("正在加载...")
        block_count_label.setFont(QFont("微软雅黑", 24, QFont.Bold))  # 增大字号
        block_count_label.setStyleSheet("color: blue;")  # 设置字体颜色为蓝色
        block_count_label.setAlignment(Qt.AlignCenter)
        ...
        # 下一次执行时间显示（移到右侧）
        next_execution_label_furnace = QLabel("下一次执行: 无")
        next_execution_label_furnace.setFont(QFont("微软雅黑", 12, QFont.Bold))
        next_execution_label_furnace.setStyleSheet("color: blue;")
        next_execution_label_furnace.setAlignment(Qt.AlignRight)
        right_layout.addWidget(next_execution_label_furnace)
        ...
        right_layout.addWidget(scroll_status_label)
        
        # 滚动时间输入框（直接显示，无标签）
        scroll_time_edit = QLineEdit("110")
        scroll_time_edit.setFont(QFont("微软雅黑", 14))
        scroll_time_edit.setFixedWidth(60)  # 3位数宽度
        scroll_time_edit.setAlignment(Qt.AlignCenter)  # 文本居中
        # 设置占位符提示
        scroll_time_edit.setPlaceholderText("滚动间隔(秒)")
        right_layout.addWidget(scroll_time_edit)
        
        # 保存标签引用到主实例
        self.next_execution_label_furnace = next_execution_label_furnace
        
        # 初始更新下一次执行时间
        next_exec_time_str = self.calculate_next_execution_time()
        next_execution_label_furnace.setText(f"下一次执行: {next_exec_time_str}")
        
        # 定期更新下一次执行时间显示（与主程序保持同步）
        def update_exec_time_display():
            """定期更新下一次执行时间显示"""
            if detail_window.isVisible():
                # 获取主程序的下一次执行时间显示
                if hasattr(self, 'next_execution_label_main'):
                    try:
                        main_text = self.next_execution_label_main.cget("text")
                        next_execution_label_furnace.setText(main_text)
                    except:
                        next_exec_time_str = self.calculate_next_execution_time()
                        next_execution_label_furnace.setText(f"下一次执行: {next_exec_time_str}")
                else:
                    ...
                    next_execution_label_furnace.setText(f"下一次执行: {next_exec_time_str}")
                # 1秒后再次更新
                QTimer.singleShot(1000, update_exec_time_display)
        
        # 启动定期更新
        QTimer.singleShot(100, update_exec_time_display)
        
        # 将右侧容器添加到状态布局
        status_layout.addWidget(right_container)
        ...
        print_btn.setToolTip("按装炉明细进行打印")
        center_buttons.addWidget(print_btn)
        
        # 全部打印复选框
        all_print_checkbox = QCheckBox("全部打印")
        all_print_checkbox.setFont(QFont("微软雅黑", 14))
        center_buttons.addWidget(all_print_checkbox)
        ...
        # 播放/停止按钮（合并开始和停止按钮）
        scroll_toggle_btn = QPushButton()
        scroll_toggle_btn.setFont(QFont("微软雅黑", 20))  # 调整字体大小以适应按钮
        scroll_toggle_btn.setFixedWidth(40)  # 正方形尺寸，与其他按钮高度一致
        scroll_toggle_btn.setFixedHeight(40)  # 正方形尺寸，与其他按钮高度一致
        scroll_toggle_btn.setStyleSheet("text-align: center;")  # 确保文本（图标）居中
        right_controls.addWidget(scroll_toggle_btn)
        # 初始设置为播放状态
        scroll_toggle_btn.setText("▶")  # 播放图标
        scroll_toggle_btn.setEnabled(True)
        ...
        def apply_theme(theme_name):
            ...
            scroll_toggle_btn.setStyleSheet(button_style)
        ...
        # 初始化默认值
        if not hasattr(self, 'furnace_detail_cursor_row'):
            self.furnace_detail_cursor_row = 0
        if not hasattr(self, 'furnace_detail_scroll_state'):
            self.furnace_detail_scroll_state = 0
        if not hasattr(self, 'furnace_detail_is_scrolling'):
            self.furnace_detail_is_scrolling = False
        if not hasattr(self, 'furnace_detail_scroll_time'):
            self.furnace_detail_scroll_time = 110  # 默认值
        ...
        def toggle_scroll():
            ...
            if not is_scrolling:
                ...
                scroll_timer.start()
                is_scrolling = True
                scroll_toggle_btn.setText("■")  # 停止图标
                scroll_toggle_btn.setFont(QFont("微软雅黑", 20))  # 确保字体大小与按钮匹配
                scroll_toggle_btn.setEnabled(True)
        ...
        def show_context_menu(event):
            ...
            if row >= 0:
                ...
                def on_find_coil_number():
                    ...
                    # 创建输入对话框，增大尺寸和字体
                    input_dialog = QInputDialog(detail_window)
                    ...
                    if ok and coil_number.strip():
                        # 遍历表格查找钢卷号（钢卷号在第二列，索引为1）
                        found = False
                        for i in range(table.rowCount()):
                            item = table.item(i, 1)  # 钢卷号列
                            if item:
                        ...
                        if not found:
                            # 未找到钢卷号
                            QMessageBox.information(detail_window, "提示", f"未找到钢卷号: {coil_number}")
                ...
                # 添加自定义信息选项（仅在非标记行显示）
                if not is_roll_changed and not is_custom_info:
                    ...
                    def on_add_custom_info():
                        ...
                        if ok and text.strip():
                            ...
                            # 设置行高为表格最小行高80
                            table.setRowHeight(new_row, 80)
                            
                            # 合并第一列到强度列（索引15）
                            # 创建一个合并的单元格
                            merge_item = QTableWidgetItem(text.strip())
                            # 设置背景色为黄色
                            merge_item.setBackground(QBrush(QColor('#FFFF00')))  # 黄色背景
                            # 设置字体大小
                            font = QFont()
                            font.setPointSize(20)
                            merge_item.setFont(font)
                            # 设置文本居中
                            merge_item.setTextAlignment(Qt.AlignCenter)
                            
                            # 设置合并单元格
                            table.setItem(new_row, 0, merge_item)
                            # 合并单元格从第0列到第15列（强度列）
                            table.setSpan(new_row, 0, 1, 16)  # 1行，16列（从0到15）
                            
                            # 保存自定义信息
                            save_custom_info(row, text.strip())
                ...
                if is_roll_changed or is_custom_info:
                    ...
                    def on_change_roll():
                        ...
                        # 合并第一列到强度列（索引15）
                        # 首先创建一个合并的单元格，3个"换辊"平分居中
                        # 使用空格和制表符实现均匀分布
                        merge_item = QTableWidgetItem("换辊\t\t\t\t\t换辊\t\t\t\t\t换辊")
                        # 设置绿色背景
                        merge_item.setBackground(QBrush(QColor('#00FF00')))  # 绿色背景
                        # 设置字体大小（调大字号）
                        font = QFont()
                        font.setPointSize(20)
                        font.setBold(True)
        ...
        # 绑定按钮事件
        scroll_toggle_btn.clicked.connect(toggle_scroll)
            ...
            try:
                ...
                for row_idx in range(table.rowCount()):
                    ...
                    if not is_special_row:
                        # 正常数据行，获取钢卷号
                        coil_item = table.item(row_idx, 1)  # 钢卷号列（索引1）
                        if coil_item:
        ...
        def refresh_data(detail_window):
            """刷新数据"""
            try:
                import time
                print("开始刷新数据...")
                ...
                print(f"装炉数据: {[item['coil_no'] for item in furnace_data]}")
                ...
                # 检查是否有新增的钢卷号
                def check_new_coils(detail_window):
                    """检查是否有新增的钢卷号"""
                    try:
                        import json
                        import os
                                    ...
                                    if data and isinstance(data, list):
                                        ...
                                        if not data:
                                            ...
                                            if isinstance(last_data, list):
                                                # 打印上一次的数据
                                                print(f"上一次的数据: {last_data}")
                                                previous_table_data = last_data  # 上一次的数据
                                            else:
                                                # 如果最后一个元素不是列表，返回空列表
                                                print("最后一个元素不是列表，返回空列表")
                                                previous_table_data = []
                            except Exception as e:
                                print(f"读取之前表格数据失败: {str(e)}")
                                previous_table_data = []
                        else:
                            print("表格数据文件不存在")
                        
                        # 打印调试信息
                        print(f"当前表格数据数量: {len(current_table_data)}")
                        print(f"当前表格的钢卷号: {[item['coil_number'] for item in current_table_data]}")
                        print(f"之前表格数据数量: {len(previous_table_data)}")
                        if previous_table_data:
                            print(f"之前表格的钢卷号: {[item['coil_number'] for item in previous_table_data]}")
                        else:
                            print("之前表格数据为空")
                        
                        # 比较当前数据和之前数据，找出新增的钢卷号
                        new_coils = []
                        if previous_table_data:
                            # 提取之前的钢卷号集合
                            previous_coil_set = set(item['coil_number'] for item in previous_table_data)
                            # 识别新增的钢卷号
                            ...
                            # 之前没有数据，所有钢卷号都是新增的
                            new_coils = current_table_data
                        
                        # 打印调试信息
                        print(f"新增钢卷号数量: {len(new_coils)}")
                        print(f"新增钢卷号: {[item['coil_number'] for item in new_coils]}")
                        
                        # 不显示弹窗提示
                        # if new_coils:
                        #     from PyQt5.QtWidgets import QMessageBox
                        #     new_coil_numbers = [item['coil_number'] for item in new_coils]
                        #     message = f"发现新增钢卷号：\n" + "\n".join(new_coil_numbers)
                        #     QMessageBox.information(detail_window, "提示", message)
                    except Exception as e:
                        print(f"检查新增钢卷号失败: {str(e)}")
                        import traceback
                        traceback.print_exc()
                
                # 检查新增钢卷号
                check_new_coils(detail_window)
                
                # 保存当前表格数据
                save_current_table_data()
            except Exception as e:
        ...
        def print_furnace_details():
            ...
            try:
                ...
                borders_data_seq.left = xlwt.Borders.THIN  # 左边框为实线
                ...
                # 14pt 样式（坯宽,减宽,调宽,轧宽,粗轧报信,坯厚,RT2）
                ...
                # 12pt 样式（计划号，坯长，轧厚，中厚，强度，除鳞，公差带）
                ...
                style_data_dashed.alignment = alignment_data
                ...
                borders_data_14pt_dashed.left = xlwt.Borders.NO_LINE
                ...
                # 设置列宽（修改为要求的列宽）
                col_widths = [
                    int(5.5 * 256 * 1.2),       # 1. 序号（5.5宽度）
                    int(7.0 * 256 * 1.04),       # 2. 计划号（修改为7）
                    int(19 * 256 * 1.04),        # 3. 钢卷号（修改为19）
                    int(18 * 256 * 1.04),       # 4. 牌号（钢级）
                    int(7 * 256 * 1.04),         # 5. 坯宽
                    int(7 * 256 * 1.05),         # 6. 减宽（侧压量）
                    int(6 * 256 * 1.2),          # 7. 调宽（板坯宽度调宽标记）
                    int(7 * 256 * 1.13),         # 8. 轧宽（轧宽+（余量））
                    int(9 * 256 * 1.03),         # 9. 公差带（碳）
                    int(33 * 256 * 1.04),        # 10. 粗轧报信
                    int(7 * 256 * 1.08),         # 11. 除鳞（层号）
                    int(7 * 256 * 1.04),         # 12. 坯厚
                    int(6.2 * 256 * 1.13),       # 13. 坯长
                    int(6 * 256 * 1.04),         # 14. 轧厚
                    int(5 * 256 * 1.04),         # 15. 中厚（中间坯厚度设定值）
                    int(6.57 * 256 * 1.04),      # 16. RT2（RT2目标值）
                    int(2 * 256 * 1.2)           # 17. 强度（硬度组）（修改为2）
                ]
                ...
                for col, header in enumerate(headers):
                    if col == len(headers) - 1:  # 强度字段（最后一列）
                ...
                plan_no = ""
                # 优先使用类属性中的计划号
                if hasattr(self, 'plan_no') and self.plan_no:
                    plan_no = self.plan_no
                else:
                    # 如果类属性中没有计划号，从表格中获取
                    for row_idx in range(table.rowCount()):
                        item = table.item(row_idx, 1)  # 计划号列（索引1）
                        if item and item.text().strip():
                            plan_no = item.text().strip()
                            break
                
                # 打印调试信息
                print(f"计划号: {plan_no}")
                print(f"当前钢卷号数量: {len(coil_numbers_with_seq)}")
                print(f"当前钢卷号: {[item['coil_number'] for item in coil_numbers_with_seq]}")
                
                # 读取之前保存的表格数据
                def read_previous_table_data():
                    """读取之前保存的表格数据"""
                    import json
                    import os
                    # 记录文件路径
                ...
                # 读取之前的表格数据
                previous_table_data = read_previous_table_data()
                
                # 打印调试信息
                print(f"之前表格数据数量: {len(previous_table_data)}")
                print(f"之前表格的钢卷号: {[item['coil_number'] for item in previous_table_data]}")
                
                # 比较当前列表与之前表格的列表，识别新增的钢卷号
                ...
                # 检查是否选择了全部打印
                is_all_print = all_print_checkbox.isChecked()
                
                # 打印调试信息
                print(f"是否全部打印: {is_all_print}")
                
                # 识别新增的钢卷号
                new_coils = identify_new_coils(coil_numbers_with_seq, previous_table_data)
                ...
                if not new_coils and not is_all_print:
                    from PyQt5.QtWidgets import QMessageBox
                    QMessageBox.information(detail_window, "提示", "没有新增的钢卷号")
                    return
                    ...
                    if is_all_print:
                        ...
                        for current_index, position_coils in position_groups.items():
                            if current_index >= len(previous_coils):
                                ...
                                # 在中间插入，序号是插入位置对应的之前序号加">"
                                # 找到插入位置对应的之前序号
                                previous_sequence = None
                                if current_index < len(previous_coils):
                                    previous_sequence = previous_coils[current_index]['sequence']
                                else:
                                    # 如果插入位置超出之前列表长度，使用最后一个序号
                                    if previous_coils:
                                        previous_sequence = previous_coils[-1]['sequence']
                                    else:
                                        previous_sequence = 0
                                
                                # 对于同一位置的多个钢卷号，统一使用同一个序号格式
                ...
                if is_all_print:
                    # 全部打印，使用所有钢卷号
                    coils_to_print = coil_numbers_with_seq
                else:
                    # 只打印新增钢卷号
                    coils_to_print = new_coils
                
                # 确定插入位置并生成序号
                coils_with_sequence = determine_insert_position_and_sequence(coils_to_print, coil_numbers_with_seq, previous_table_data, is_all_print)
                
                # 打印调试信息
                print(f"带有序号的钢卷号: {[(item['coil_number'], item['display_sequence']) for item in coils_with_sequence]}")
                
                # 写入数据（从第4行开始，第1行是提示行，第2行是计划信息行，第3行是字段名行）
                data_row_start = 3
                
                # 启用自动换行
                style_data.alignment.wrap = True
                
                # 收集当前表格中的钢卷号和对应的数据行
                current_coil_data = []
                # 直接从表格中获取最新数据，而不是使用之前的 rows 变量
                for row_idx in range(table.rowCount()):
                    # 检查是否是换辊行或自定义信息行（通过背景色判断）
                    first_item = table.item(row_idx, 0)
                    is_special_row = False
                    if first_item:
                        bg_color = first_item.background().color().name()
                ...
                # 打印调试信息
                print(f"当前表格中的钢卷号: {[item['coil_number'] for item in current_coil_data]}")
                
                # 打印钢卷号信息
                new_rows_written = 0
                for coil in coils_with_sequence:
                    # 找到对应的行数据
                    found = False
                    for coil_data in current_coil_data:
                        if coil_data['coil_number'] == coil['coil_number']:
                            row_data = coil_data['row_data']
                            # 写入序号和数据
                            # 序号列使用14pt样式，使用生成的显示序号
                            sheet.write(data_row_start + new_rows_written, 0, coil['display_sequence'], style_data_seq)
                            # 写入数据（从第1列开始，第0列是序号）
                            for col_idx, value in enumerate(row_data):
                                if col_idx < len(headers) - 1:  # 只写入到强度列（不包括最后一列的打印时间）
                                    current_col = col_idx + 1
                                    if current_col == 2:  # 钢卷号列（第3列，索引为2）
                                        # 钢卷号列使用16pt样式
                                        sheet.write(data_row_start + new_rows_written, current_col, value, style_data_coil)
                                    elif current_col == 4:  # 坯宽 (14pt) - 左侧长间隔虚线边框
                                        # 14pt左侧长间隔虚线边框样式
                                        sheet.write(data_row_start + new_rows_written, current_col, value, style_data_12pt_left_dashdot)
                                    elif current_col == 5:  # 减宽 (14pt) - 长间隔虚线边框
                                        # 14pt长间隔虚线边框样式
                                        sheet.write(data_row_start + new_rows_written, current_col, value, style_data_14pt_long_dashed)
                                    elif current_col == 7:  # 轧宽 (14pt) - 实线边框（去掉左边线）
                                        # 14pt实线边框样式（去掉左边线）
                                        sheet.write(data_row_start + new_rows_written, current_col, value, style_data_14pt_dashed)
                                    elif current_col == 8:  # 公差带 (12pt) - 长间隔虚线边框
                                        # 12pt长间隔虚线边框样式
                                        sheet.write(data_row_start + new_rows_written, current_col, value, style_data_12pt_dashed)
                                    elif current_col == 10:  # 除鳞 (12pt) - 长间隔虚线边框
                                        # 12pt长间隔虚线边框样式
                                        sheet.write(data_row_start + new_rows_written, current_col, value, style_data_12pt_dashed)
                                    elif current_col == 13:  # 轧厚 (12pt) - 长间隔虚线边框
                                        # 12pt长间隔虚线边框样式
                                        sheet.write(data_row_start + new_rows_written, current_col, value, style_data_12pt_dashdot)
                                    elif current_col in [6, 9, 11, 15]:  # 调宽,粗轧报信,坯厚,RT2 (14pt)
                                        # 14pt样式
                                        sheet.write(data_row_start + new_rows_written, current_col, value, style_data_14pt)
                                    elif current_col in [1, 12, 14]:  # 计划号，坯长，中厚 (12pt)
                                        # 12pt样式
                                        sheet.write(data_row_start + new_rows_written, current_col, value, style_data_12pt)
                                    elif current_col == 16:  # 强度 (12pt) - 带右边框
                                        # 12pt样式（带右边框）
                                        sheet.write(data_row_start + new_rows_written, current_col, value, style_data_12pt_strength)
                                    else:
                                        # 其他列使用11pt样式
                                        sheet.write(data_row_start + new_rows_written, current_col, value, style_data)
                            # 设置数据行行高（最小25pt）
                            # 找到粗轧报信值
                            粗轧报信值 = ""
                            if len(row_data) > 8:  # 粗轧报信在第9个元素（索引8）
                                粗轧报信值 = row_data[8]
                            ...
                            if 粗轧报信值:
                                ...
                                基础行高 = 500  # 25pt
                                每增加行高 = 360  # 18pt
                                行高 = 基础行高 + (需要行数 - 1) * 每增加行高
                                sheet.row(data_row_start + new_rows_written).height = 行高
                            else:
                                # 没有粗轧报信内容，使用固定行高
                                sheet.row(data_row_start + new_rows_written).height = 500  # 25pt
                            sheet.row(data_row_start + new_rows_written).height_mismatch = True
                            new_rows_written += 1
                            found = True
                            break
                    if not found:
                        print(f"未找到钢卷号 {coil['coil_number']} 的对应数据行")
                
                # 打印调试信息
                print(f"已打印的钢卷号数量: {new_rows_written}")
                
                # 保存文件
                workbook.save(temp_file_path)
                
                # 添加页面设置（采用主程序中的页面设置方法）
                try:
                    ...
                    def get_column_letter(col_idx):
                        ...
                        return letter
                    last_row = data_row_start + new_rows_written - 1
                    last_col = len(headers) - 1
                    print_area = f"A1:{get_column_letter(last_col)}{last_row + 1}"
                    ws.PageSetup.PrintArea = print_area
                    
                    # 设置打印标题行（表头）
                    ws.PageSetup.PrintTitleRows = "$3:$3"  # 字段名行
                ...
                # 保存打印记录（钢卷号和对应的序号）
                def save_print_record(plan_no, coil_numbers_with_seq):
                    """保存打印记录到文件"""
                    import json
                    import os
                    # 确保计划号目录存在
                    ...
                    os.makedirs(plan_dir, exist_ok=True)
                    # 记录文件路径
                    record_file = os.path.join(plan_dir, f"{plan_no}_print_record.json")
                    # 保存数据
                    with open(record_file, 'w', encoding='utf-8') as f:
                        json.dump(coil_numbers_with_seq, f, ensure_ascii=False, indent=2)
                
                # 保存打印记录
                if plan_no and coil_numbers_with_seq:
                    save_print_record(plan_no, coil_numbers_with_seq)
        ...
        def save_roll_change_info(insert_row):
            """保存换辊行信息"""
            try:
                import json
                import os
                ...
                # 构建保存文件路径
                plan_dir = os.path.join(self.plan_dir, "计划号")
                roll_change_file = os.path.join(plan_dir, "roll_change_info.json")
                
                # 读取现有数据
                roll_change_info = []
                if os.path.exists(roll_change_file):
                    try:
                        with open(roll_change_file, 'r', encoding='utf-8') as f:
                            roll_change_info = json.load(f)
                    except:
                        roll_change_info = []
                ...
                for info in roll_change_info:
                    if info['insert_row'] <= insert_row:
                        adjusted_row += 1
                
                # 添加新的换辊行信息
                new_info = {
                    'type': 'roll_change',
                    'insert_row': adjusted_row,
                    'timestamp': int(time.time())
                }
                roll_change_info.append(new_info)
                
                # 保存到文件
                with open(roll_change_file, 'w', encoding='utf-8') as f:
                    json.dump(roll_change_info, f, ensure_ascii=False, indent=2)
                
                print(f"已保存换辊行信息: {new_info}")
            except Exception as e:
                print(f"保存换辊行信息失败: {e}")
        
        def load_roll_change_info():
            """加载并恢复换辊行和自定义信息行"""
            try:
                import json
                import os
                ...
                if not os.path.exists(roll_change_file):
                    return
                ...
                # 按插入位置排序（从大到小，避免插入时影响后续位置）
                roll_change_info.sort(key=lambda x: x['insert_row'], reverse=True)
                
                # 恢复标记行
                for info in roll_change_info:
                    insert_row = info['insert_row']
                    if insert_row < table.rowCount():
                        # 在指定位置插入行
                        ...
                        if info.get('type') == 'custom_info':
                            # 自定义信息行
                            content = info.get('content', '')
                            merge_item = QTableWidgetItem(content)
                            # 设置黄色背景
                            merge_item.setBackground(QBrush(QColor('#FFFF00')))  # 黄色背景
                            # 设置字体大小
                            font = QFont()
                            ...
                            print(f"已恢复换辊行: 插入位置 {insert_row}")
        ...
        def update_roll_change_info():
            ...
            try:
                ...
                for row_idx in range(table.rowCount()):
                    ...
                    if item:
                        if "换辊" in item.text():
                            ...
                            new_roll_change_info.append(new_info)
                        elif item.background().color().name() == '#ffff00' or item.background().color().name() == '#FFFF00':
                            # 自定义信息行
                            insert_row = row_idx - 1
                            new_info = {
                                'type': 'custom_info',
                ...
                # 保存到文件
                with open(roll_change_file, 'w', encoding='utf-8') as f:
                    json.dump(new_roll_change_info, f, ensure_ascii=False, indent=2)
                
                print(f"已更新换辊行信息，当前标记行数: {len(new_roll_change_info)}")
            except Exception as e:
                print(f"更新换辊行信息失败: {e}")
        
        def save_custom_info(insert_row, content):
            """保存自定义信息"""
            try:
                import json
                import os
        ...
        def save_window_state():
            ...
            try:
                ...
                selected_items = table.selectedItems()
                if selected_items:
                    self.furnace_detail_cursor_row = selected_items[0].row()
                else:
                    self.furnace_detail_cursor_row = 0
                # 获取滚动条位置
                self.furnace_detail_scroll_state = table.verticalScrollBar().value()
                # 保存自动滚动状态
                self.furnace_detail_is_scrolling = is_scrolling
                # 保存滚动时间设置
                try:
                    self.furnace_detail_scroll_time = int(scroll_time_edit.text())
                except:
                    self.furnace_detail_scroll_time = 110  # 默认值
                print(f"保存装炉明细窗口状态: 滚动位置={self.furnace_detail_scroll_state}, 光标行={self.furnace_detail_cursor_row}, 自动滚动状态={self.furnace_detail_is_scrolling}, 滚动时间={self.furnace_detail_scroll_time}")
            except Exception as e:
                print(f"保存窗口状态失败: {e}")
        ...
        def on_window_close():
            ...
            try:
                if hasattr(self, 'next_execution_label_furnace'):
                    del self.next_execution_label_furnace
                    print("已清理装炉明细执行时间标签引用")
            except Exception as e:
                print(f"清理标签引用失败: {e}")
            # 先激活主程序窗口
            try:
        ...
        return_btn.clicked.connect(on_window_close)
        ...
        detail_window.closeEvent = on_close_event
        
        # 显示窗口并设置为置顶
        detail_window.show()
        detail_window.raise_()  # 提升窗口到最前面
        detail_window.activateWindow()  # 激活窗口
        
        # 初始加载数据
        refresh_data(detail_window)
        # 初始更新块数显示
        update_block_count_display()
        ...
        def restore_window_state():
            try:
                # 恢复光标位置
                if 0 <= self.furnace_detail_cursor_row < table.rowCount():
                    table.selectRow(self.furnace_detail_cursor_row)
                    # 滚动到选中行
                    table.scrollToItem(table.item(self.furnace_detail_cursor_row, 0), QTableWidget.PositionAtCenter)
                # 恢复滚动条位置
                table.verticalScrollBar().setValue(self.furnace_detail_scroll_state)
                # 恢复滚动时间设置
                if hasattr(self, 'furnace_detail_scroll_time'):
                    scroll_time_edit.setText(str(self.furnace_detail_scroll_time))
                    print(f"恢复滚动时间设置: {self.furnace_detail_scroll_time}")
                # 恢复自动滚动状态
                if hasattr(self, 'furnace_detail_is_scrolling') and self.furnace_detail_is_scrolling:
                    # 延迟执行，确保界面已加载完成
                    def start_scrolling():
                        if not is_scrolling:
                            toggle_scroll()
                    QTimer.singleShot(1000, start_scrolling)
                    print(f"恢复自动滚动状态: {self.furnace_detail_is_scrolling}")
                print(f"恢复装炉明细窗口状态: 滚动位置={self.furnace_detail_scroll_state}, 光标行={self.furnace_detail_cursor_row}")
            except Exception as e:
                print(f"恢复窗口状态失败: {e}")
        ...
        def restore_and_update():
            ...
            # 恢复状态后更新块数显示
            update_block_count_display()
        QTimer.singleShot(500, restore_and_update)
        
        # 运行事件循环
        app.exec_()
    
    def process_single_plan(self, plan_no, file_path):
        """处理单个计划号文件
        ...
        Returns:
            bool: 是否检测到低轧宽板坯（has_low_roll_width）
...
# result_window.grab_set()
    ...
    def run_excel_macro_with_pandas(self, file_path):
        ...
        # 第一列左侧实线边框样式（14pt - 用于序号列）
        style_data_14pt_left_border = xlwt.XFStyle()
        font_data_14pt_left_border = xlwt.Font()
        font_data_14pt_left_border.name = '仿宋'
        font_data_14pt_left_border.height = 280  # 14pt
        font_data_14pt_left_border.bold = True
        style_data_14pt_left_border.font = font_data_14pt_left_border
        style_data_14pt_left_border.alignment = alignment_data_14pt
        borders_data_14pt_left_border = xlwt.Borders()
        borders_data_14pt_left_border.left = xlwt.Borders.THIN
        borders_data_14pt_left_border.right = xlwt.Borders.NO_LINE
        borders_data_14pt_left_border.top = xlwt.Borders.THIN
        borders_data_14pt_left_border.bottom = xlwt.Borders.THIN
        style_data_14pt_left_border.borders = borders_data_14pt_left_border
        ...
        for row_idx, row_data in enumerate(all_row_data):
            ...
            for col_idx, col_name in enumerate(target_columns):
                ...
                if 映射后的列名 == "粗轧报信":
                    ...
                    # 序号列（第一列）左侧需要实线边框，使用14pt样式
                    if is_first_column:
                        new_sheet.write(new_row_idx, col_idx, value, style_data_14pt_left_border)
                    else:
                        new_sheet.write(new_row_idx, col_idx, value, style_data_14pt)
                elif 映射后的列名 == "装炉顺":
                    # 装炉顺列（最后一列）右侧需要实线边框
    ...
    def show_selected(self):
        ...
        for plan_no in selected_plans:
            ...
            if plan_no not in self.processed_plans:
                ...
                try:
                    self.process_single_plan(plan_no, file_path)
                except Exception as e:
                    import traceback
...
# result_window.grab_set()
    ...
    def load_low_roll_width_plans(self):
        ...
        try:
            ...
            if os.path.exists(low_roll_width_file):
                with open(low_roll_width_file, 'r', encoding='utf-8') as f:
                    for line in f:
                        plan_no = line.strip()
                        if plan_no:
                            self.low_roll_width_plans.add(plan_no)
                print(f"已加载 {len(self.low_roll_width_plans)} 个低轧宽板坯计划号")
        except Exception as e:
            print(f"加载低轧宽板坯计划号失败: {str(e)}")
        ...
        try:
            ...
            with open(low_roll_width_file, 'w', encoding='utf-8') as f:
                for plan_no in self.low_roll_width_plans:
                    f.write(f"{plan_no}\n")
            print(f"已保存 {len(self.low_roll_width_plans)} 个低轧宽板坯计划号")
        except Exception as e:
            print(f"保存低轧宽板坯计划号失败: {str(e)}")
    ...
    def load_coordinate_config(self):
        ...
        default_coords = {
            ...
            # 防退出登录设置
            "anti_logout_enabled": False,         # 防退出登录是否启用
            "anti_logout_interval_value": 100,      # 防退出登录时间间隔值
            "anti_logout_interval": 6000000,          # 防退出登录时间间隔（毫秒）
            "anti_logout_unit": "分钟",             # 防退出登录时间单位
            
            # 自动执行设置
            "auto_exec_enabled": False,           # 自动执行是否启用
...
class ExportConfigGUI:
    ...
    DEFAULT_COORDS = {
        ...
        # 自动执行设置
        "auto_exec_mode": "interval",          # 自动执行模式（interval: 时间间隔, time: 指定时间）
        "auto_exec_enabled": False,           # 自动执行是否启用
        "auto_exec_interval": 30,              # 自动执行时间间隔（分钟）
        "auto_exec_times": ""                  # 自动执行指定时间
    }
    ...
    def __init__(self, root, plan_dir, main_app=None):
        ...
        self.default_config_file = os.path.join(plan_dir, "export_coordinates_default.json")
        ...
        # 窗口关闭时的处理函数
        def on_close():
            # 验证防止退出登录时间间隔（静默验证，不显示错误提示）
            try:
                anti_logout_value = int(self.anti_logout_interval_var.get())
                if not (1 <= anti_logout_value <= 999):
                    # 不在有效范围内，使用默认值
                    self.anti_logout_interval_var.set(str(self.DEFAULT_COORDS["anti_logout_interval_value"]))
                    self.coordinates["anti_logout_interval_value"] = self.DEFAULT_COORDS["anti_logout_interval_value"]
                    self.coordinates["anti_logout_interval"] = self.DEFAULT_COORDS["anti_logout_interval"]
            except ValueError:
                # 非整数输入，使用默认值
                self.anti_logout_interval_var.set(str(self.DEFAULT_COORDS["anti_logout_interval_value"]))
                self.coordinates["anti_logout_interval_value"] = self.DEFAULT_COORDS["anti_logout_interval_value"]
            ...
            # 验证自动执行时间间隔（静默验证，不显示错误提示）
            try:
                auto_exec_value = int(self.auto_exec_interval_var.get())
                if not (1 <= auto_exec_value <= 999):
                    # 不在有效范围内，使用默认值
                    self.auto_exec_interval_var.set(str(self.DEFAULT_COORDS["auto_exec_interval"]))
                    self.coordinates["auto_exec_interval"] = self.DEFAULT_COORDS["auto_exec_interval"]
            except ValueError:
                # 非整数输入，使用默认值
                self.auto_exec_interval_var.set(str(self.DEFAULT_COORDS["auto_exec_interval"]))
                self.coordinates["auto_exec_interval"] = self.DEFAULT_COORDS["auto_exec_interval"]
        ...
        if os.path.exists(self.config_file):
            try:
                with open(self.config_file, 'r', encoding='utf-8') as f:
                    saved = json.load(f)
                    # 合并默认值和保存值
                    coords = self.load_default_coordinates()
                    coords.update(saved)
                    return coords
            except:
                pass
        return self.load_default_coordinates()
    ...
    def create_widgets(self):
        ...
        # 焦点离开事件处理：验证输入并保存
        def on_spacing_focus_out(event):
            try:
                value = int(spacing_entry.get())
                # 有效输入，更新配置
                self.coordinates["plan_spacing"] = value
                self.save_coordinates()
            except ValueError:
                # 非整数输入，显示错误
                self.main_app.custom_messagebox("错误", "请输入有效的整数")
                # 不再自动设置焦点，让用户自己处理
        spacing_entry.bind("<FocusOut>", on_spacing_focus_out)
        
        # 回车键处理：只移动焦点，不保存内容
        def on_spacing_enter(event):
            try:
                value = int(spacing_entry.get())
                if value > 0:
                    # 移动焦点到框架
                    spacing_frame.focus_set()
                else:
                    # 超出范围，显示错误
                    self.main_app.custom_messagebox("错误", "请输入大于0的数值")
                    # 保持焦点在文本框并全选内容
                    spacing_entry.focus()
                    spacing_entry.select_range(0, tk.END)
            except ValueError:
                # 非整数输入，显示错误
                self.main_app.custom_messagebox("错误", "请输入有效的整数")
                # 保持焦点在文本框并全选内容
                spacing_entry.focus()
                spacing_entry.select_range(0, tk.END)
        spacing_entry.bind("<Return>", on_spacing_enter)
        ...
        # 焦点离开事件处理：验证输入并保存
        def on_offset_focus_out(event):
            try:
                value = int(offset_entry.get())
                # 有效输入，更新配置
                self.coordinates["first_plan_offset"] = value
                self.save_coordinates()
            except ValueError:
                # 非整数输入，显示错误
                self.main_app.custom_messagebox("错误", "请输入有效的整数")
                # 不再自动设置焦点，让用户自己处理
        offset_entry.bind("<FocusOut>", on_offset_focus_out)
        
        # 回车键处理：只移动焦点，不保存内容
        def on_offset_enter(event):
            try:
                value = int(offset_entry.get())
                # 移动焦点到框架
                offset_frame.focus_set()
            except ValueError:
                # 非整数输入，显示错误
                self.main_app.custom_messagebox("错误", "请输入有效的整数")
                # 保持焦点在文本框并全选内容
        ...
        offset_entry.bind("<Return>", on_offset_enter)
        ...
        # 窗口选择
        window_label = ttk.Label(test_settings_frame, text="窗口选择:")
        ...
        # 速度设置
        speed_label = ttk.Label(test_settings_frame, text="自动导出速度:")
        ...
        ttk.Label(custom_window_frame, text="自定义窗口名称:").pack(anchor=tk.W)
        ...
        # 焦点离开事件处理：保存内容
        def on_custom_window_focus_out(event):
            # 保存内容
            self.coordinates["custom_window_title"] = self.custom_window_var.get().strip()
            self.save_coordinates()
        custom_window_entry.bind("<FocusOut>", on_custom_window_focus_out)
        
        # 回车键处理：只移动焦点，不保存内容
        def on_custom_window_enter(event):
            # 移动焦点到框架
            custom_window_frame.focus_set()
        custom_window_entry.bind("<Return>", on_custom_window_enter)
        ...
        # 自动执行和防止退出登录设置
        service_frame = ttk.Frame(test_settings_frame)
        service_frame.pack(anchor=tk.W, pady=10)
        
        # 自动执行设置
        self.auto_exec_enabled_var = tk.BooleanVar(value=self.coordinates.get("auto_exec_enabled", False))
        self.auto_exec_checkbox = ttk.Checkbutton(
            service_frame, 
            text="启用自动执行", 
            variable=self.auto_exec_enabled_var,
            command=self.on_auto_exec_enabled_changed
        )
        self.auto_exec_checkbox.pack(anchor=tk.W)
        
        ttk.Label(service_frame, text="启用后将按照设定的时间间隔或指定时间自动执行导出操作", 
                 foreground="gray").pack(anchor=tk.W, padx=20)
        
        # 防止退出登录设置
        self.anti_logout_enabled_var = tk.BooleanVar(value=self.coordinates.get("anti_logout_enabled", False))
        self.anti_logout_checkbox = ttk.Checkbutton(
            service_frame, 
            text="启用防止退出登录", 
            variable=self.anti_logout_enabled_var,
            command=self.on_anti_logout_enabled_changed
        )
        self.anti_logout_checkbox.pack(anchor=tk.W, pady=5)
        
        ttk.Label(service_frame, text="启用后将定期激活窗口，防止系统自动退出登录", 
                 foreground="gray").pack(anchor=tk.W, padx=20)
        ...
        # 防止退出登录时间间隔设置
        anti_logout_frame = ttk.Frame(test_settings_frame)
        anti_logout_frame.pack(anchor=tk.W, pady=10, fill=tk.X)
        
        ttk.Label(anti_logout_frame, text="防止退出登录时间间隔:").pack(side=tk.LEFT, padx=5)
        
        self.anti_logout_interval_var = tk.StringVar(value=str(self.coordinates.get("anti_logout_interval_value", self.DEFAULT_COORDS["anti_logout_interval_value"])))
        interval_entry = ttk.Entry(anti_logout_frame, textvariable=self.anti_logout_interval_var, width=10)
        interval_entry.pack(side=tk.LEFT, padx=5)
        
        # 光标进入文本框全选内容
        def on_focus_in(event):
            interval_entry.select_range(0, tk.END)
        interval_entry.bind("<FocusIn>", on_focus_in)
        ...
        def on_key_press(event):
            ...
            # 检查是否有选中文本
            try:
                start_idx = interval_entry.index(tk.SEL_FIRST)
                end_idx = interval_entry.index(tk.SEL_LAST)
                if start_idx != end_idx:
                    # 有选中文本，允许输入（替换选中内容）
                    return
            except tk.TclError:
                # 没有选中文本，检查长度
                current_text = interval_entry.get()
                if len(current_text) >= 3:
                    # 超过3位数，阻止输入
                    return "break"
        interval_entry.bind("<KeyPress>", on_key_press)
        
        # 焦点离开事件处理：验证输入并保存
        def on_focus_out(event):
            # 检查事件源是否是当前文本框
            if event.widget != interval_entry:
                return
            
            # 防止重复触发
            if hasattr(self, '_anti_logout_validating') and self._anti_logout_validating:
                return
            ...
            try:
                ...
                if not (1 <= value <= 999):
                    # 超出范围，显示错误
                    self._anti_logout_validating = True
                    self.main_app.custom_messagebox("错误", "请输入1-999之间的数值")
                    self._anti_logout_validating = False
                    # 保持焦点在文本框并全选内容
                    ...
                    # 有效输入，更新配置
                    self.coordinates["anti_logout_interval_value"] = value
                    self.coordinates["anti_logout_interval"] = value * 60 * 1000
                    self.save_coordinates()
                    # 重启防退出登录服务以应用新的间隔
                    if self.main_app and self.main_app.anti_logout_var.get():
                        self.main_app.stop_anti_logout()
                        self.main_app.start_anti_logout()
            except ValueError:
                # 非整数输入，显示错误
                self._anti_logout_validating = True
                self.main_app.custom_messagebox("错误", "请输入有效的整数")
                ...
                interval_entry.select_range(0, tk.END)
                return
        interval_entry.bind("<FocusOut>", on_focus_out)
        ...
        def on_enter(event):
            ...
            try:
                ...
                interval_entry.focus()
                interval_entry.select_range(0, tk.END)
        interval_entry.bind("<Return>", on_enter)
        
        ttk.Label(anti_logout_frame, text="分钟").pack(side=tk.LEFT, padx=5)
        ttk.Label(anti_logout_frame, text="(1-999)", foreground="gray").pack(side=tk.LEFT, padx=5)
        
        # 添加下一次执行时间显示
        next_anti_logout_frame = ttk.Frame(anti_logout_frame)
        next_anti_logout_frame.pack(side=tk.LEFT, padx=10)
        
        ttk.Label(next_anti_logout_frame, text="下一次执行:").pack(side=tk.LEFT)
        
        self.next_anti_logout_label_settings = ttk.Label(
            next_anti_logout_frame, 
            text="无", 
            foreground='blue'
        )
        self.next_anti_logout_label_settings.pack(side=tk.LEFT, padx=5)
        
        # 更新下一次执行时间显示的函数
        def update_next_anti_logout_time_display():
            if self.main_app and hasattr(self.main_app, 'anti_logout_target_execution_time'):
                import time
                next_time_str = time.strftime("%H:%M:%S", time.localtime(self.main_app.anti_logout_target_execution_time))
                self.next_anti_logout_label_settings.config(text=next_time_str)
            else:
                self.next_anti_logout_label_settings.config(text="无")
        
        # 初始更新下一次执行时间
        update_next_anti_logout_time_display()
        
        # 保存函数引用以便外部调用
        self.update_next_anti_logout_time_display = update_next_anti_logout_time_display
        
        # 自动执行设置
        auto_exec_frame = ttk.LabelFrame(test_settings_frame, text="自动执行设置", padding="10")
        auto_exec_frame.pack(fill=tk.X, pady=10)
        
        # 执行方式选择
        self.auto_exec_mode_var = tk.StringVar(value=self.coordinates.get("auto_exec_mode", self.DEFAULT_COORDS["auto_exec_mode"]))
        
        # 按时间间隔自动执行
        interval_mode_frame = ttk.Frame(auto_exec_frame)
        interval_mode_frame.pack(anchor=tk.W, pady=5, fill=tk.X)
        
        ttk.Radiobutton(interval_mode_frame, text="按时间间隔自动执行:", variable=self.auto_exec_mode_var, value="interval").pack(side=tk.LEFT, padx=5)
        
        self.auto_exec_interval_var = tk.StringVar(value=str(self.coordinates.get("auto_exec_interval", self.DEFAULT_COORDS["auto_exec_interval"])))
        exec_interval_entry = ttk.Entry(interval_mode_frame, textvariable=self.auto_exec_interval_var, width=10)
        exec_interval_entry.pack(side=tk.LEFT, padx=5)
        
        # 光标进入文本框全选内容
        def on_exec_interval_focus_in(event):
            exec_interval_entry.select_range(0, tk.END)
        exec_interval_entry.bind("<FocusIn>", on_exec_interval_focus_in)
        
        # 限制输入为3位数
        def on_exec_interval_key_press(event):
            # 允许删除键、退格键、方向键等控制键
            if event.keysym in ('BackSpace', 'Delete', 'Left', 'Right', 'Home', 'End'):
                return
            # 检查是否有选中文本
            try:
                start_idx = exec_interval_entry.index(tk.SEL_FIRST)
                end_idx = exec_interval_entry.index(tk.SEL_LAST)
                if start_idx != end_idx:
                    # 有选中文本，允许输入（替换选中内容）
                    return
            except tk.TclError:
                # 没有选中文本，检查长度
                current_text = exec_interval_entry.get()
                if len(current_text) >= 3:
                    # 超过3位数，阻止输入
                    return "break"
        exec_interval_entry.bind("<KeyPress>", on_exec_interval_key_press)
        
        # 焦点离开事件处理：验证输入并保存
        def on_exec_interval_focus_out(event):
            # 检查事件源是否是当前文本框
            if event.widget != exec_interval_entry:
                return
            
            # 防止重复触发
            if hasattr(self, '_auto_exec_validating') and self._auto_exec_validating:
                return
            
            try:
                value = int(exec_interval_entry.get())
                if not (1 <= value <= 999):
                    # 超出范围，显示错误
                    self._auto_exec_validating = True
                    self.main_app.custom_messagebox("错误", "请输入1-999之间的数值")
                    self._auto_exec_validating = False
                    # 保持焦点在文本框并全选内容
                    exec_interval_entry.focus()
                    exec_interval_entry.select_range(0, tk.END)
                    return
                else:
                    # 有效输入，更新配置
                    self.coordinates["auto_exec_interval"] = value
                    self.save_coordinates()
                    # 更新下一次执行时间显示
                    update_next_exec_time_display()
                    # 重启自动执行服务以应用新的间隔
                    if self.main_app:
                        self.main_app.start_auto_execution()
            except ValueError:
                # 非整数输入，显示错误
                self._auto_exec_validating = True
                self.main_app.custom_messagebox("错误", "请输入有效的整数")
                ...
                exec_interval_entry.select_range(0, tk.END)
                return
        exec_interval_entry.bind("<FocusOut>", on_exec_interval_focus_out)
        
        # 回车键处理：验证输入并保存
        def on_exec_interval_enter(event):
            # 防止重复触发
            if hasattr(self, '_auto_exec_validating') and self._auto_exec_validating:
                return
            ...
            try:
                ...
                exec_interval_entry.focus()
                exec_interval_entry.select_range(0, tk.END)
        exec_interval_entry.bind("<Return>", on_exec_interval_enter)
        
        ttk.Label(interval_mode_frame, text="分钟").pack(side=tk.LEFT, padx=5)
        ttk.Label(interval_mode_frame, text="(1-999)", foreground="gray").pack(side=tk.LEFT, padx=5)
        
        # 按指定时间自动执行
        time_mode_frame = ttk.Frame(auto_exec_frame)
        time_mode_frame.pack(anchor=tk.W, pady=5, fill=tk.X)
        
        ttk.Radiobutton(time_mode_frame, text="按指定时间自动执行:", variable=self.auto_exec_mode_var, value="time").pack(anchor=tk.W, padx=5)
        
        # 时间输入框
        time_input_frame = ttk.Frame(auto_exec_frame)
        time_input_frame.pack(anchor=tk.W, pady=5, fill=tk.X, padx=20)
        
        ttk.Label(time_input_frame, text="指定时间(格式: HH:MM,多个时间用逗号分隔):").pack(anchor=tk.W, pady=2)
        self.auto_exec_times_var = tk.StringVar(value=self.coordinates.get("auto_exec_times", self.DEFAULT_COORDS["auto_exec_times"]))
        time_entry = ttk.Entry(time_input_frame, textvariable=self.auto_exec_times_var, width=50)
        time_entry.pack(anchor=tk.W, pady=2)
        ttk.Label(time_input_frame, text="例如: 08:00, 12:00, 16:00", foreground="gray").pack(anchor=tk.W, pady=2)
        
        # 光标进入文本框全选内容
        def on_time_entry_focus_in(event):
            time_entry.select_range(0, tk.END)
        time_entry.bind("<FocusIn>", on_time_entry_focus_in)
        
        # 焦点离开事件处理：验证输入并保存
        def on_time_entry_focus_out(event):
            # 检查事件源是否是当前文本框
            if event.widget != time_entry:
                return
            
            # 防止重复触发
            if hasattr(self, '_time_entry_validating') and self._time_entry_validating:
                return
            
            # 验证输入格式
            time_str = self.auto_exec_times_var.get().strip()
            if time_str:
                times = [t.strip() for t in time_str.split(',')]
                for t in times:
                    if len(t) != 5 or t[2] != ':':
                        # 格式错误，显示错误
                        self._time_entry_validating = True
                        self.main_app.custom_messagebox("错误", "时间格式错误，请输入正确的时间格式 HH:MM")
                        self._time_entry_validating = False
                        # 保持焦点在文本框并全选内容
                        time_entry.focus()
                        time_entry.select_range(0, tk.END)
                        return
                    ...
                    try:
                        ...
                        if not (0 <= hour <= 23 and 0 <= minute <= 59):
                            # 时间范围错误，显示错误
                            self._time_entry_validating = True
                            self.main_app.custom_messagebox("错误", "时间范围错误，小时应为00-23，分钟应为00-59")
                            self._time_entry_validating = False
                            # 保持焦点在文本框并全选内容
                            time_entry.focus()
                            time_entry.select_range(0, tk.END)
                        ...
                        return
            # 有效输入，保存内容
            self.coordinates["auto_exec_times"] = time_str
            self.save_coordinates()
            # 更新下一次执行时间显示
            update_next_exec_time_display()
            # 重启自动执行服务以应用新的时间
            if self.main_app:
                self.main_app.start_auto_execution()
        time_entry.bind("<FocusOut>", on_time_entry_focus_out)
        
        # 回车键处理：验证输入并保存
        def on_time_entry_enter(event):
            # 防止重复触发
            if hasattr(self, '_time_entry_validating') and self._time_entry_validating:
                return
            ...
            if self.main_app:
                self.main_app.start_auto_execution()
            # 移动焦点到框架
            time_input_frame.focus_set()
        time_entry.bind("<Return>", on_time_entry_enter)
        
        # 实时更新自动执行设置
        def on_auto_exec_mode_change(*args):
            self.coordinates["auto_exec_mode"] = self.auto_exec_mode_var.get()
            self.save_coordinates()
            # 更新下一次执行时间显示
            update_next_exec_time_display()
            # 重启自动执行服务以应用新的模式
            if self.main_app:
                self.main_app.start_auto_execution()
        self.auto_exec_mode_var.trace_add("write", on_auto_exec_mode_change)
        
        # 添加下一次执行时间显示
        next_exec_time_frame = ttk.Frame(auto_exec_frame)
        next_exec_time_frame.pack(anchor=tk.W, pady=5, fill=tk.X)
        
        ttk.Label(next_exec_time_frame, text="下一次执行时间:").pack(side=tk.LEFT)
        
        self.next_exec_time_label_settings = ttk.Label(
            next_exec_time_frame, 
            text="无", 
            foreground='blue'
        )
        self.next_exec_time_label_settings.pack(side=tk.LEFT, padx=5)
        
        # 更新下一次执行时间显示的函数
        def update_next_exec_time_display():
            if self.main_app:
                # 检查自动执行是否被启用
                if hasattr(self.main_app, 'auto_exec_var') and self.main_app.auto_exec_var.get():
                    next_time_str = self.main_app.calculate_next_execution_time()
                    if next_time_str and next_time_str != "无":
                        self.next_exec_time_label_settings.config(text=next_time_str)
                    else:
                        self.next_exec_time_label_settings.config(text="无")
                else:
                    # 自动执行未启用，显示"无"
                    self.next_exec_time_label_settings.config(text="无")
        ...
        # 保存这些函数引用以便外部调用
        self.update_next_exec_time_display = update_next_exec_time_display
    ...
    def create_coord_row(self, parent, label_text, coord_key, string_var):
        ...
        def on_coord_focus_out(event):
            ...
            if coord:
                ...
                # 无效输入，显示错误
                self.main_app.custom_messagebox("错误", "请输入有效的坐标格式 (X, Y)")
                # 不再自动设置焦点，让用户自己处理
        entry.bind("<FocusOut>", on_coord_focus_out)
        
        # 回车键处理：只移动焦点，不保存内容
        def on_coord_enter(event):
            # 解析坐标
            coord = self.parse_coord(string_var.get())
            if coord:
                # 移动焦点到框架
        ...
        entry.bind("<Return>", on_coord_enter)
    ...
    def on_auto_exec_enabled_changed(self):
        """自动执行开关状态改变时的回调"""
        enabled = self.auto_exec_enabled_var.get()
        self.coordinates["auto_exec_enabled"] = enabled
        status = "启用" if enabled else "禁用"
        self.log(f"自动执行已{status}")
        
        # 立即更新到MainApp的coordinates中
        if self.main_app:
            self.main_app.coordinates.update({"auto_exec_enabled": enabled})
            # 同步主画面的复选框状态
            if hasattr(self.main_app, 'auto_exec_var'):
                self.main_app.auto_exec_var.set(enabled)
            # 启动或停止自动执行服务
            if enabled:
                self.main_app.start_auto_execution()
            else:
                self.main_app.stop_auto_execution()
        
        # 更新设置窗口的下一次执行时间显示
        if hasattr(self, 'update_next_exec_time_display'):
            self.update_next_exec_time_display()
        
        # 立即保存到文件
        self.save_coordinates()
    
    def on_anti_logout_enabled_changed(self):
        """防止退出登录开关状态改变时的回调"""
        enabled = self.anti_logout_enabled_var.get()
        self.coordinates["anti_logout_enabled"] = enabled
        status = "启用" if enabled else "禁用"
        self.log(f"防止退出登录已{status}")
        
        # 立即更新到MainApp的coordinates中
        if self.main_app:
            self.main_app.coordinates.update({"anti_logout_enabled": enabled})
            # 同步主画面的复选框状态
            if hasattr(self.main_app, 'anti_logout_var'):
                self.main_app.anti_logout_var.set(enabled)
            # 启动或停止防止退出登录服务
            if enabled:
                self.main_app.start_anti_logout()
            else:
    ...
    def capture_coordinate(self, key, name):
        ...
        if not PYAUTOGUI_AVAILABLE:
            ...
            self.main_app.custom_messagebox("错误", "pyautogui未安装")
            return
    ...
    def test_coordinate(self, key, name):
        ...
        if not coord:
            self.main_app.custom_messagebox("警告", f"{name} 未设置坐标，请先获取坐标")
            return
        ...
        # 确认测试
        if not self.main_app.custom_messagebox("确认测试", f"即将测试点击 {name}\n坐标: {coord}\n测试窗口: {test_window}\n\n是否继续？", msg_type='yesno'):
            return
        ...
        try:
            ...
            self.main_app.custom_messagebox("测试完成", f"已点击 {name}\n坐标: {coord}\n测试窗口: {test_window}")
        except Exception as e:
            self.log(f"✗ 测试 {name} 失败: {str(e)}")
            self.main_app.custom_messagebox("测试失败", f"测试 {name} 时出错:\n{str(e)}")
    ...
    def restore_defaults(self):
        ...
        import tkinter.messagebox as messagebox
        if self.main_app.custom_messagebox("确认", "确定要恢复默认值吗？", msg_type='yesno'):
            print("用户确认恢复默认值")
            
            # 从默认配置文件加载默认值
            default_coords = self.load_default_coordinates()
            ...
            # 更新防退出登录和自动执行设置
            self.anti_logout_interval_var.set(str(default_coords["anti_logout_interval_value"]))
            self.auto_exec_mode_var.set(default_coords["auto_exec_mode"])
            self.auto_exec_interval_var.set(str(default_coords["auto_exec_interval"]))
            self.auto_exec_times_var.set(default_coords["auto_exec_times"])
    ...
    def show_current_config(self):
        ...
        import tkinter.messagebox as messagebox
        self.main_app.custom_messagebox("当前配置", config_text)
    ...
    def activate_window(self, window_name):
        ...
        try:
            ...
            print("win32gui未安装")
            self.main_app.custom_messagebox("错误", "win32gui未安装，无法激活窗口")
            return False
        ...
        if handles:
            ...
            self.main_app.custom_messagebox("警告", f"未找到窗口: {window_title}")
            return False
