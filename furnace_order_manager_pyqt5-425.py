...
class MainWindow(QMainWindow):
                    ...
                    if event.success:
                        ...
                        def close_msg_box():
                            try:
                                ...
                                print(f"关闭消息框并返回画面失败: {e}")
                        
                        # 3秒后执行关闭操作
                        QTimer.singleShot(3000, close_msg_box)
                    else:
                        # 显示错误消息
                        msg_box = QMessageBox(self)
                        ...
                        # 确保消息框显示在最前面
                        msg_box.setWindowFlags(Qt.WindowStaysOnTopHint | Qt.Dialog | Qt.MSWindowsFixedSizeDialogHint)
                        msg_box.setModal(True)  # 模态，阻塞用户操作
                        # 确保应用程序是活动的
                        QApplication.setActiveWindow(msg_box)
                        # 显示消息框
                        msg_box.exec_()  # 阻塞等待用户确认
                except Exception as e:
                    print(f"处理完成事件失败: {e}")
                return True
    ...
    def process_plans(self, auto_print=False, show_result=True):
        ...
        try:
            ...
            if not selected_files:
                ...
                self.show()
                # 使用自动关闭的消息框
                self.show_auto_close_message("提示", "选中的计划号没有对应的文件")
                # 弹窗关闭后再次激活主程序窗口
                self.activateWindow()
                self.raise_()
                self.show()
    ...
    def print_furnace_details(self):
        ...
        try:
            ...
            alignment_data_12pt_left.wrap = 1  # 启用自动换行
            style_data_12pt_left.alignment = alignment_data_12pt_left
            style_data_12pt_left.borders = borders_data
    ...
    def show_auto_close_message(self, title, message, duration=3000):
        ...
        msg_box.setModal(True)  # 使用模态窗口，确保用户注意到
                        ...
                        if event.exported_plans:
                            ...
                            if hasattr(self, 'previous_foreground_hwnd') and self.previous_foreground_hwnd:
                                try:
                                    import win32gui
                                    import win32con
                                    print(f"尝试返回启动时的窗口 (HWND: {self.previous_foreground_hwnd})")
                                    # 检查窗口是否仍然存在
                                    if win32gui.IsWindow(self.previous_foreground_hwnd):
                                        # 尝试激活窗口
                                        win32gui.ShowWindow(self.previous_foreground_hwnd, win32con.SW_RESTORE)
        ...
        if handles:
            ...
            try:
                ...
                try:
                    ...
                    print("方法1: ShowWindow成功")
                except Exception as e:
                    print(f"方法1: ShowWindow失败: {str(e)}")
                    errors.append(f"ShowWindow: {str(e)}")
                time.sleep(0.2)
                ...
                try:
                    ...
                    print("方法2: SetWindowPos(TOPMOST)成功")
                except Exception as e:
                    print(f"方法2: SetWindowPos(TOPMOST)失败: {str(e)}")
                    errors.append(f"SetWindowPos(TOPMOST): {str(e)}")
                time.sleep(0.2)
                ...
                try:
                    win32gui.SetWindowPos(hwnd, win32con.HWND_NOTOPMOST, 0, 0, 0, 0,
                                         win32con.SWP_NOMOVE | win32con.SWP_NOSIZE)
                    print("方法3: SetWindowPos(NOTOPMOST)成功")
                except Exception as e:
                    print(f"方法3: SetWindowPos(NOTOPMOST)失败: {str(e)}")
                    errors.append(f"SetWindowPos(NOTOPMOST): {str(e)}")
                time.sleep(0.2)

                # 方法4: 设置前台窗口
                try:
                    win32gui.SetForegroundWindow(hwnd)
                    print("方法4: SetForegroundWindow成功")
                except Exception as e:
                    print(f"方法4: SetForegroundWindow失败: {str(e)}")
                    errors.append(f"SetForegroundWindow: {str(e)}")
                time.sleep(0.2)

                # 方法5: 设置活动窗口
                try:
                    win32gui.SetActiveWindow(hwnd)
                    print("方法5: SetActiveWindow成功")
                except Exception as e:
                    print(f"方法5: SetActiveWindow失败: {str(e)}")
                    errors.append(f"SetActiveWindow: {str(e)}")
                time.sleep(0.2)

                # 方法6: 设置焦点
                try:
                    win32gui.SetFocus(hwnd)
                    print("方法6: SetFocus成功")
                except Exception as e:
                    print(f"方法6: SetFocus失败: {str(e)}")
                    errors.append(f"SetFocus: {str(e)}")
                time.sleep(0.3)

                # 方法7: 将窗口带到最前面
                try:
                    win32gui.BringWindowToTop(hwnd)
                    print("方法7: BringWindowToTop成功")
                except Exception as e:
                    print(f"方法7: BringWindowToTop失败: {str(e)}")
                    errors.append(f"BringWindowToTop: {str(e)}")
                time.sleep(0.2)
                ...
                if len(errors) == 7:
                    ...
                    try:
                        ...
                        # 如果窗口位置过小（可能在屏幕边缘），使用窗口中心位置
                        if rect[0] < 10 or rect[1] < 10:
                            # 使用窗口中心位置
                            x = (rect[0] + rect[2]) // 2
                            y = (rect[1] + rect[3]) // 2
                            print(f"窗口位置过小({rect[0]}, {rect[1]})，使用中心位置: ({x}, {y})")
                        else:
                            x = rect[0] + 10
                            y = rect[1] + 10
                        print(f"窗口矩形: {rect}, 点击坐标: ({x}, {y})")
                        pyautogui.click(x, y)
                        time.sleep(0.5)
...
class FurnaceDetailsWindow(QMainWindow):
        ...
        if is_roll_changed or is_custom_info:
            ...
            def on_cancel_mark():
                ...
                if is_roll_changed:
                    ...
                    self.remove_roll_change_info(row)
                else:
                    print(f"已取消标注，删除了第 {row} 行的自定义信息")
                    self.remove_custom_info(row)
    ...
    def save_window_state(self):
        ...
        try:
            ...
            # 收集当前状态
            current_coil_number = ""
            if self.table_widget.rowCount() > 0 and self.current_row < self.table_widget.rowCount():
                coil_item = self.table_widget.item(self.current_row, 1)  # 钢卷号列（索引1）
                if coil_item:
    ...
    def print_furnace_details(self):
        ...
        try:
                ...
                try:
                    with open(table_data_file, 'r', encoding='utf-8') as f:
                        ...
                        if combined_data and isinstance(combined_data, list):
                            previous_coils = combined_data[0] if len(combined_data) > 0 else []
                except:
                    previous_coils = []
            ...
            style_header_info.font = font_header_info
            alignment_header_info = xlwt.Alignment()
            alignment_header_info.horz = xlwt.Alignment.HORZ_CENTER
            alignment_header_info.vert = xlwt.Alignment.VERT_CENTER
            style_header_info.alignment = alignment_header_info
            ...
            # 14pt 样式 - 居中（用于坯宽、轧宽、调宽、坯厚、中厚、RT2）
            style_data_14pt_center = xlwt.XFStyle()
            font_data_14pt_center = xlwt.Font()
            font_data_14pt_center.name = '仿宋'
            font_data_14pt_center.height = 280  # 14pt
            ...
            # 14pt 样式 - 居中带虚线边框（用于坯宽、调宽、公差带、除鳞、轧厚）
            style_data_14pt_dashed = xlwt.XFStyle()
            font_data_14pt_dashed = xlwt.Font()
            font_data_14pt_dashed.name = '仿宋'
            font_data_14pt_dashed.height = 280  # 14pt
            font_data_14pt_dashed.bold = True
            style_data_14pt_dashed.font = font_data_14pt_dashed
            style_data_14pt_dashed.alignment = alignment_data_14pt_center
            borders_data_dashed = xlwt.Borders()
            borders_data_dashed.left = xlwt.Borders.MEDIUM_DASHED
            borders_data_dashed.right = xlwt.Borders.MEDIUM_DASHED
            borders_data_dashed.top = xlwt.Borders.THIN
            borders_data_dashed.bottom = xlwt.Borders.THIN
            style_data_14pt_dashed.borders = borders_data_dashed
            ...
            if len(提示信息列表) > 0:
                ...
                for i in range(提示信息总行数):
                    ...
                    左列索引 = i * 3
                    if 左列索引 < len(提示信息列表):
                        sheet.write_merge(1 + i, 1 + i, 左区域起始, 左区域结束, 提示信息列表[左列索引], style_header_info)
                    
                    # 中间列（坯宽到公差带）：显示第 i*3+1 条提示信息
                    中间列索引 = i * 3 + 1
                    if 中间列索引 < len(提示信息列表):
                        sheet.write_merge(1 + i, 1 + i, 中间区域起始, 中间区域结束, 提示信息列表[中间列索引], style_header_info)
                    
                    # 右列（粗轧报信到除鳞）：显示第 i*3+2 条提示信息
                    右列索引 = i * 3 + 2
                    if 右列索引 < len(提示信息列表):
                        sheet.write_merge(1 + i, 1 + i, 右区域起始, 右区域结束, 提示信息列表[右列索引], style_header_info)
                    
                    # 在同一行显示打印时间（坯厚到强度）
                    if i == 0:  # 只在第一行显示打印时间
                        print_time = time.strftime("%Y-%m-%d %H:%M:%S")
                        sheet.write_merge(1 + i, 1 + i, 打印时间起始, 打印时间结束, f"打印时间：{print_time}", style_time)
                    
                    # 设置提示信息行的行高
                    sheet.row(1 + i).height = 500  # 25pt
                    sheet.row(1 + i).height_mismatch = True
            else:
                # 没有提示信息时，单独显示打印时间
                print_time_row = 1
                print_time = time.strftime("%Y-%m-%d %H:%M:%S")
                打印时间起始 = 11  # 坯厚列
                打印时间结束 = len(headers) - 1  # 强度列
                sheet.write_merge(print_time_row, print_time_row, 打印时间起始, 打印时间结束, f"打印时间：{print_time}", style_time)
                sheet.row(print_time_row).height = 460  # 23pt
                sheet.row(print_time_row).height_mismatch = True
            ...
            for col, header in enumerate(headers):
                if col == len(headers) - 1:  # 强度字段
                    sheet.write(field_name_row, col, header, style_header_strength)
                else:
                    sheet.write(field_name_row, col, header, style_header_border)
            sheet.row(field_name_row).height = 500  # 25pt
            sheet.row(field_name_row).height_mismatch = True
                        ...
                        if coil_number:
                            ...
                            if is_all_print or coil_number in coil_sequence_map:
                                ...
                                if coil_number in coil_sequence_map:
                                    sheet.write(print_row_idx, 0, coil_sequence_map[coil_number], style_data_seq)
                                else:
                                    sheet.write(print_row_idx, 0, "", style_data_seq)
                                
                                # 写入其他字段（只写入到强度列，即第16列）
                                for col_idx, value in enumerate(row_data, 1):  # 从row_data的第一个元素开始写入
                                    if col_idx > 16:  # 只写入到强度列（第16列）
                                        break
                                    if col_idx == 2:  # 钢卷号列（16pt，左对齐）
                                        sheet.write(print_row_idx, col_idx, value, style_data_coil)
                                    elif col_idx == 3:  # 牌号列（14pt，左对齐）
                                        sheet.write(print_row_idx, col_idx, value, style_data_14pt)
                                    elif col_idx == 4 or col_idx == 6 or col_idx == 8 or col_idx == 10 or col_idx == 13:  # 坯宽、调宽、公差带、除鳞、轧厚（14pt，居中，虚线边框）
                                        sheet.write(print_row_idx, col_idx, value, style_data_14pt_dashed)
                                    elif col_idx == 7 or col_idx == 11 or col_idx == 14 or col_idx == 15:  # 轧宽、坯厚、中厚、RT2（14pt，居中）
                                        sheet.write(print_row_idx, col_idx, value, style_data_14pt_center)
                                    elif col_idx == 9:  # 粗轧报信列（12pt，左对齐）
                                        sheet.write(print_row_idx, col_idx, value, style_data_12pt_left)
                                    elif col_idx == 16:  # 强度列（12pt，右边框实线，居中）
                                        sheet.write(print_row_idx, col_idx, value, style_data_12pt_strength)
                                    else:  # 其他所有列（12pt，居中对齐）
                                        sheet.write(print_row_idx, col_idx, value, style_data_12pt)
                                ...
                                if rough_rolling_msg:
                                    ...
                                    # 设置行高（根据行数）
                                    row_height = 500 * lines  # 基础行高 * 行数
                                    sheet.row(print_row_idx).height = row_height
                                    sheet.row(print_row_idx).height_mismatch = True
                                
                                print_row_idx += 1
                else:
                    # 特殊行（换辊行或自定义信息行），不保存到xls文件中
                    pass
...
class SettingsWindow(QDialog):
    ...
    def save_settings(self, settings=None):
        ...
        if settings is None:
            ...
            settings = {
                ...
                # 窗口选择
                "selectedWindow": "Doc1.docx - Word" if self.ui.windowRadio1.isChecked() else "BCGMS1-宝钢股份多基地制造管理系统运行环境",
    ...
    def restore_defaults(self):
        ...
        if default_settings is None:
            default_settings = {
                "mainWindow": {"x": 284, "y": 84},
                "furnaceWindow": {"x": 988, "y": 819},
                "planInput": {"x": 107, "y": 85},
                "queryBtn": {"x": 734, "y": 135},
                "exportBtn": {"x": 78, "y": 740},
                "closeBtn": {"x": 235, "y": 207},
                "detailExportBtn": {"x": 79, "y": 859},
                "selectedWindow": "BCGMS1-宝钢股份多基地制造管理系统运行环境",
                "autoProcess": True,
                "autoPrint": True,
                "autoExec": True,
                "execMode": "interval",
