                ...
                try:
                    ...
                    if event.success:
                        ...
                        if event.exported_plans:
                            ...
                            # 确保消息框显示在最前面
                            msg_box.setWindowFlags(Qt.WindowStaysOnTopHint | Qt.Dialog | Qt.MSWindowsFixedSizeDialogHint)
                            msg_box.setModal(True)  # 模态，确保用户看到弹窗
                            # 设置消息框的字体
                            font = QFont()
                        ...
                        # 使用自定义消息框显示错误信息
                        self.custom_messagebox("错误", event.message, msg_type='error')
                except Exception as e:
                    print(f"处理完成事件失败: {e}")
                finally:
        ...
        def export_in_background():
            ...
            try:
                ...
                if not no_file_plans:
                    print("[×] 没有无文件的计划号，停止执行")
                    export_finished(True, "没有新的计划！", [], [], {})
                    return
