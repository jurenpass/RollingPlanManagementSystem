...
class FurnaceOrderManager:
    def __init__(self, root):
        ...
        self.furnace_detail_auto_scroll = False  # 自动滚动状态
        self.furnace_detail_scroll_time = 110  # 自动滚动时间间隔（秒）
        ...
        # 启动自动执行服务（仅在自动执行选项被选中时）
        if hasattr(self, 'auto_exec_var') and self.auto_exec_var.get():
            self.start_auto_execution()
        else:
            # 如果自动执行选项未选中，确保不启动自动执行
            self.is_auto_exec_running = False
    ...
    def init_database(self):
        ...
        try:
            ...
            # 创建钢卷号导出记录表
            cursor.execute('''
                CREATE TABLE IF NOT EXISTS coil_export_record (
                    id INTEGER PRIMARY KEY AUTOINCREMENT,
                    coil_no TEXT NOT NULL UNIQUE,
                    first_export_order_no TEXT NOT NULL,
                    first_export_time TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    ...
    def on_anti_logout_toggle(self):
        ...
        if self.anti_logout_var.get():
            print("防退出登录已启用，正在启动...")
            # 无论当前在哪个画面，都强制启动防退出登录
            try:
                # 确保主窗口存在
                if not hasattr(self, 'root') or self.root is None:
                    print("主窗口不存在，无法启动防退出登录")
                    return
                # 强制启动
    ...
    def save_furnace_detail_state(self):
        """保存装炉明细窗口状态"""
        try:
            if hasattr(self, 'furnace_detail_window') and self.furnace_detail_window is not None:
                # 查找表格对象
                for widget in self.furnace_detail_window.children():
                    if hasattr(widget, 'inherits') and widget.inherits('QTableWidget'):
                        table = widget
                        # 获取当前选中的行
            ...
            print(f"保存装炉明细窗口状态失败: {e}")
    
    def restore_furnace_detail_state(self):
        """恢复装炉明细窗口状态"""
        try:
            if hasattr(self, 'furnace_detail_window') and self.furnace_detail_window is not None:
                # 查找表格对象
                for widget in self.furnace_detail_window.children():
                    if hasattr(widget, 'inherits') and widget.inherits('QTableWidget'):
                        ...
                        if 0 <= self.furnace_detail_cursor_row < table.rowCount():
                            ...
                            # 滚动到选中行
                            table.scrollToItem(table.item(self.furnace_detail_cursor_row, 0), table.PositionAtCenter)
                        # 恢复滚动条位置
            ...
            print(f"恢复装炉明细窗口状态失败: {e}")
    
    def anti_logout_action(self):
        """防退出登录动作：激活选择窗口，按下F2键，返回当前画面"""
        try:
            # 检查是否在装炉明细画面
            is_in_furnace_detail = False
            if hasattr(self, 'furnace_detail_window') and self.furnace_detail_window is not None:
                try:
                    if self.furnace_detail_window.isVisible():
                        is_in_furnace_detail = True
                        # 保存装炉明细窗口状态
                        self.save_furnace_detail_state()
                        print("检测到装炉明细画面，已保存状态")
                        # 直接关闭装炉明细窗口（模拟返回按钮操作）
                        self.furnace_detail_window.close()
                        import time
                        time.sleep(0.5)
                        print("已关闭装炉明细窗口，返回主程序画面")
                except Exception as e:
                    print(f"检查装炉明细窗口状态失败: {e}")
            ...
            # 如果之前在装炉明细画面，重新打开并恢复状态
            if is_in_furnace_detail:
                try:
                    # 重新打开装炉明细窗口
                    print("重新打开装炉明细窗口并恢复状态...")
                    # 使用主窗口的after方法在主线程中打开
                    if hasattr(self, 'root') and self.root is not None:
                        self.root.after(1000, self.furnace_details)
                        print("已重新打开装炉明细窗口")
                except Exception as e:
                    print(f"重新打开装炉明细窗口失败: {e}")
        except Exception as e:
            print(f"防退出登录动作失败: {e}")
    
    def auto_exec_action(self):
        """自动执行动作：激活主程序窗口，执行自动导出，返回当前画面"""
        try:
            # 检查是否在装炉明细画面
            is_in_furnace_detail = False
            if hasattr(self, 'furnace_detail_window') and self.furnace_detail_window is not None:
            ...
            if hasattr(self, 'root') and self.root is not None:
                # 使用win32gui激活主窗口
                main_window_hwnd = win32gui.FindWindow(None, "轧制计划管理系统")
                if main_window_hwnd:
                    ...
                    print("执行自动导出...")
                    # 使用主线程执行GUI操作
                    self.root.after(0, self.auto_export)
                    # 等待自动导出完成
                    time.sleep(1.0)
    ...
    def stop_auto_execution(self):
        ...
        if hasattr(self, 'auto_exec_timer') and self.auto_exec_timer is not None:
            try:
                ...
                while self.auto_exec_timer.is_alive() and time.time() - start_time < 5:
                    time.sleep(0.1)
                if self.auto_exec_timer.is_alive():
                    print("自动执行线程仍在运行")
                else:
                    print("自动执行线程已结束")
            except Exception as e:
                print(f"停止自动执行线程失败: {e}")
            finally:
                self.auto_exec_timer = None
        print("自动执行已停止")
        # 更新显示为无
    ...
    def start_anti_logout(self):
        ...
        # 先停止之前可能运行的定时器
        if self.is_anti_logout_running:
            self.stop_anti_logout()
    ...
    def stop_anti_logout(self):
        ...
        self.is_anti_logout_running = False
        # 等待线程结束
        if hasattr(self, 'anti_logout_timer') and self.anti_logout_timer is not None:
            try:
                # 等待线程结束（最多等待5秒）
                import time
                start_time = time.time()
                while self.anti_logout_timer.is_alive() and time.time() - start_time < 5:
                    time.sleep(0.1)
                if self.anti_logout_timer.is_alive():
                    print("防退出登录线程仍在运行")
                else:
                    print("防退出登录线程已结束")
            except Exception as e:
                print(f"停止防退出登录线程失败: {e}")
            finally:
                self.anti_logout_timer = None
        print("防退出登录已停止")
        # 更新显示为无
    ...
    def calculate_next_execution_time(self):
        ...
        if exec_mode == "interval":
            # 按时间间隔执行
            # 检查是否已经记录了目标执行时间
            if hasattr(self, 'auto_exec_target_execution_time') and self.auto_exec_target_execution_time is not None:
                # 使用已记录的目标执行时间
                next_time_str = time.strftime("%H:%M:%S", time.localtime(self.auto_exec_target_execution_time))
                return next_time_str
            else:
                # 如果没有记录，计算新的目标执行时间
                interval = self.coordinates.get("auto_exec_interval", 30) * 60  # 转换为秒
                self.auto_exec_target_execution_time = time.time() + interval
                next_time_str = time.strftime("%H:%M:%S", time.localtime(self.auto_exec_target_execution_time))
                return next_time_str
        elif exec_mode == "time":
            # 按指定时间执行
            ...
            if exec_times:
                ...
                if valid_times:
                    ...
                    for hour, minute in valid_times:
                        # 计算时间差（秒）
                        time_diff = (hour - current_hour) * 3600 + (minute - current_minute) * 60
                        if time_diff <= 0:
                            # 今天的时间已经过了，使用明天的时间
                            time_diff += 24 * 3600
    ...
    def start_auto_execution(self):
        ...
        # 首先检查自动执行选项是否被选中
        if hasattr(self, 'auto_exec_var') and not self.auto_exec_var.get():
            print("自动执行选项未选中，不启动自动执行服务")
            return
        
        # 检查pyautogui是否可用
        if not self.PYAUTOGUI_AVAILABLE:
            self.custom_messagebox("错误", "pyautogui未安装，请先安装: pip install pyautogui")
            self.auto_exec_var.set(False)
            return
            ...
            while self.is_auto_exec_running:
                try:
                    ...
                    try:
                        ...
                        print(f"解析执行时间失败: {next_exec_time_str}")
                        time.sleep(60)
                        continue
                    ...
                    while wait_time > 0 and self.is_auto_exec_running:
                        ...
                        # 每10秒检查一下下一次执行时间是否发生了变化（配置可能被修改）
                    ...
                    if exec_mode == "time":
                        # 指定时间模式：使用之前解析的目标时间
                        # 构造目标时间，使用之前解析的小时、分钟和秒数
                        target_time = datetime.datetime(now.year, now.month, now.day, target_hour, target_minute, target_second)
                        # 如果目标时间已过，使用明天的日期
                        if target_time < now:
                            target_time += datetime.timedelta(days=1)
                        # 检查是否在目标时间的±10秒范围内（减小时间窗口避免重复执行）
                        time_diff = abs((now - target_time).total_seconds())
                        if time_diff <= 10:
                            should_execute = True
                    else:
                        # 时间间隔模式：检查秒数
                        target_hour, target_minute, target_second = map(int, self.calculate_next_execution_time().split(':'))
                        target_time = datetime.datetime(now.year, now.month, now.day, target_hour, target_minute, target_second)
                        # 检查是否在目标时间的±10秒范围内（减小时间窗口避免重复执行）
                        time_diff = abs((now - target_time).total_seconds())
                        if time_diff <= 10:
                    ...
                    if should_execute:
                        # 检查是否已经在最近执行过（避免重复执行）
                        if self.last_executed_time is None or (now - self.last_executed_time).total_seconds() > 60:
                            print(f"执行时间到达，当前时间: {now.strftime('%H:%M:%S')}")
                            print("执行自动导出...")
                            # 记录执行时间
                            self.last_executed_time = now
                            # 执行自动导出动作
                            self.auto_exec_action()
                            # 重新计算时间间隔模式的下一次目标执行时间
                            exec_mode = self.coordinates.get("auto_exec_mode", "interval")
                            if exec_mode == "interval":
                                import time
                                interval = self.coordinates.get("auto_exec_interval", 30) * 60  # 转换为秒
    ...
    def furnace_details(self):
        ...
        def update_block_count_display():
            ...
            current_index = table.selectionModel().currentIndex()
            if current_index.isValid():
                current_row = current_index.row() + 1  # 行号从1开始
        ...
        def start_scroll():
            ...
            # 保存滚动时间
            self.furnace_detail_scroll_time = scroll_time
            ...
            scroll_timer.start()
            self.furnace_detail_auto_scroll = True
        ...
        def stop_scroll():
            """停止自动滚动"""
            scroll_timer.stop()
            self.furnace_detail_auto_scroll = False
        ...
        def auto_scroll():
            ...
            if self.furnace_detail_cursor_row < table.rowCount() - 1:
                # 向下移动一行，这样数据会向上滚动
                self.furnace_detail_cursor_row += 1
            else:
                # 到达底部，回到顶部
                self.furnace_detail_cursor_row = 0
        ...
        def on_cell_clicked(row, column):
            ...
            table.selectRow(row)
            # 只有在自动滚动时才滚动数据
            if self.furnace_detail_auto_scroll:
                # 将该行滚动到屏幕中间
                table.scrollToItem(table.item(row, 0), QTableWidget.PositionAtCenter)
            # 更新当前行变量
            self.furnace_detail_cursor_row = row
            # 确保标注的单元格保持红色背景
        ...
        def load_plan_files_cache(furnace_data):
            """一次性加载所有计划号文件，建立钢卷号到数据的映射
            ...
            print(f"开始加载计划号文件缓存，共 {len(furnace_data)} 条数据")
            
            # 按计划号分组
            plan_no_groups = defaultdict(list)
            for item in furnace_data:
                plan_no = item['plan_no']
                coil_no = item['coil_no']
                plan_no_groups[plan_no].append(coil_no)
            
            print(f"共涉及 {len(plan_no_groups)} 个计划号文件")
            
            # 建立钢卷号到数据的映射
            coil_data_cache = {}
            
            for plan_no, coil_nos in plan_no_groups.items():
                plan_file = get_plan_file(plan_no)
                if not plan_file:
                    print(f"找不到计划号文件: {plan_no}.xls")
                    continue
                
                try:
                    print(f"加载计划号文件: {plan_file}")
                    workbook = xlrd.open_workbook(plan_file)
                    sheet = workbook.sheet_by_index(0)
                    ...
                    if coil_no_col == -1:
                        print(f"在文件 {plan_file} 中未找到钢卷号列")
                        workbook.release_resources()
                        continue
                    
                    # 建立钢卷号集合用于快速查找
                    target_coil_set = set(coil_nos)
                    ...
                    for row in range(header_row + 1, sheet.nrows):
                        try:
                            ...
                            # 检查是否是我们需要的钢卷号
                            matched_coil = None
                            if clean_cell_value in target_coil_set:
                                matched_coil = clean_cell_value
                            elif cell_value in target_coil_set:
                                matched_coil = cell_value
                                ...
                                for header, col_idx in col_map.items():
                                    if col_idx < sheet.ncols:
                                        try:
                                            ...
                                            data[header] = ''
                                coil_data_cache[matched_coil] = data
                                print(f"缓存钢卷数据: {matched_coil}")
                        except Exception as e:
                            pass
                    ...
                    print(f"加载计划号文件失败 {plan_file}: {str(e)}")
                    import traceback
                    traceback.print_exc()
            
            print(f"缓存加载完成，共缓存 {len(coil_data_cache)} 条钢卷数据")
            return coil_data_cache
        ...
        def refresh_data():
            ...
            try:
                ...
                # 一次性加载所有计划号文件，建立钢卷号数据缓存
                coil_data_cache = load_plan_files_cache(furnace_data)
                ...
                for item in furnace_data:
                    ...
                    # 从缓存中获取钢卷数据
                    coil_data = coil_data_cache.get(coil_no)
                    if coil_data:
        ...
        def show_selected_plan():
            ...
            plan_no = table.item(row, 0).text()  # 计划号在第一列
        ...
        # 绑定窗口关闭事件
        detail_window.closeEvent = lambda event: save_window_state()
        ...
        QTimer.singleShot(500, restore_window_state)
        
        # 延迟恢复自动滚动状态
        def restore_auto_scroll():
            try:
                # 设置滚动时间
                ...
                if self.furnace_detail_auto_scroll:
                    start_scroll()
                    print(f"已恢复自动滚动状态，滚动时间: {self.furnace_detail_scroll_time}秒")
            except Exception as e:
                print(f"恢复自动滚动状态失败: {e}")
        
        QTimer.singleShot(1000, restore_auto_scroll)
    ...
    def run_excel_macro_with_pandas(self, file_path):
        ...
        if os.path.exists(装炉顺序文件):
            ...
            if success:
                ...
                if 装炉钢卷号列 != -1:
                    # 直接按Excel文件中的行顺序处理，不排序
                    for 相对序号, row_idx in enumerate(range(1, sheet2.nrows), 1):
                        ...
                        if 钢卷号:
                            ...
                            if 装炉顺序号列 != -1:
                                try:
                                    # 直接读取为字符串，支持格式如"1", "1-1", "1-2"
                                    原装炉顺序号 = str(sheet2.cell_value(row_idx, 装炉顺序号列)).strip()
                                except (ValueError, TypeError):
                                    原装炉顺序号 = ""
...
# result_window.grab_set()
        ...
        try:
            if os.path.exists(temp_zhuanglu_file):
                ...
                if success:
                    ...
                    # 处理装炉顺序号填充
                    add_debug_log(f"【序号处理】开始处理装炉顺序号...")
                    log_lines.append("处理装炉顺序号...")
                    order_success = self.process_zhuanglu_order_numbers(zhuanglu_file, add_debug_log)
                    if order_success:
                        add_debug_log(f"✓ 装炉顺序号处理完成")
                        log_lines.append("装炉顺序号处理完成")
                        # 再次清理文件缓存，确保读取到最新数据
                        self.clear_file_cache(zhuanglu_file)
                    else:
                        add_debug_log(f"✗ 装炉顺序号处理失败")
                        log_lines.append("装炉顺序号处理失败")
                else:
    ...
    def process_zhuanglu_order_numbers(self, zhuanglu_file, add_debug_log=None):
        """处理装炉顺序.xls的序号填充
        
        参数:
            zhuanglu_file: 装炉顺序.xls文件路径
        ...
        try:
            ...
            # 1. 读取装炉顺序.xls文件
            add_debug_log(f"  读取文件: {zhuanglu_file}")
            workbook = xlrd.open_workbook(zhuanglu_file, formatting_info=True)
            sheet = workbook.sheet_by_index(0)
            ...
            for col in range(sheet.ncols):
                cell_value = sheet.cell_value(header_row, col)
                if "钢卷号" in str(cell_value):
                    coil_no_col = col
                if "装炉顺序号" in str(cell_value) or "装炉顺" in str(cell_value):
                    order_no_col = col
            
            if coil_no_col == -1:
                add_debug_log(f"  ✗ 未找到钢卷号列")
                return False
            if order_no_col == -1:
                add_debug_log(f"  ✗ 未找到装炉顺序号列")
                return False
            
            add_debug_log(f"  ✓ 钢卷号列: {coil_no_col}, 装炉顺序号列: {order_no_col}")
            ...
            for row in range(1, sheet.nrows):
                if row >= sheet.nrows:
                    break
                coil_no = str(sheet.cell_value(row, coil_no_col)).strip()
                if coil_no:
                    coil_data.append({
                        'row': row,
                        'coil_no': coil_no,
            ...
            add_debug_log(f"  共找到 {len(coil_data)} 个钢卷号")
            ...
            # 收集需要查询的钢卷号
            coil_nos = [item['coil_no'] for item in coil_data]
            placeholders = ','.join(['?'] * len(coil_nos))
            cursor.execute(f"""
                SELECT coil_no, first_export_order_no 
                FROM coil_export_record 
                WHERE coil_no IN ({placeholders})
            """, coil_nos)
            
            # 更新coil_data中的信息
            existing_records = {row[0]: row[1] for row in cursor.fetchall()}
            for item in coil_data:
                if item['coil_no'] in existing_records:
                    item['is_first_export'] = False
                    item['first_order_no'] = existing_records[item['coil_no']]
            
            add_debug_log(f"  ✓ 已查询数据库，找到 {len(existing_records)} 条已有记录")
            
            # 5. 根据情况处理序号
            all_first = all(item['is_first_export'] for item in coil_data)
            all_not_first = all(not item['is_first_export'] for item in coil_data)
            
            new_order_numbers = []
            
            if all_first:
                # 情况1: 全部第一次导出，按行顺序填充
                add_debug_log(f"  情况1: 全部为第一次导出，按行顺序填充序号")
                for i, item in enumerate(coil_data):
                    new_order_no = str(i + 1)
                    new_order_numbers.append((item['row'], new_order_no))
                    # 保存到数据库
                    cursor.execute("""
                        INSERT OR REPLACE INTO coil_export_record 
                        (coil_no, first_export_order_no, last_export_time)
                        VALUES (?, ?, CURRENT_TIMESTAMP)
                    """, (item['coil_no'], new_order_no))
            
            elif all_not_first:
                # 情况2: 全部非第一次导出，保持第一次的序号不变
                add_debug_log(f"  情况2: 全部为非第一次导出，保持原序号")
                for item in coil_data:
                    new_order_numbers.append((item['row'], item['first_order_no']))
                    # 更新最后导出时间
                    cursor.execute("""
                        UPDATE coil_export_record 
                ...
                # 情况3和4: 混合情况
                add_debug_log(f"  情况3/4: 混合情况，处理插入序号")
                
                # 首先确定前面连续的非第一次导出的钢卷号
                last_non_first_idx = -1
                for i, item in enumerate(coil_data):
                    if not item['is_first_export']:
                        last_non_first_idx = i
                    else:
                        break
                
                if last_non_first_idx == len(coil_data) - 1:
                    # 全部都是非第一次（理论上不会到这里，因为all_not_first已经判断过）
                    pass
                elif last_non_first_idx == -1:
                    # 第一个就是第一次导出（理论上不会到这里，因为all_first已经判断过）
                    pass
                else:
                    # 前面有非第一次的，后面有第一次的
                    # 先处理前面非第一次的
                    for i in range(last_non_first_idx + 1):
                        item = coil_data[i]
                        new_order_numbers.append((item['row'], item['first_order_no']))
                        cursor.execute("""
                            UPDATE coil_export_record 
                    ...
                    # 计算最后一个非第一次的序号
                    last_non_first_order = coil_data[last_non_first_idx]['first_order_no']
                    # 尝试转换为数字
                    try:
                        base_order = int(last_non_first_order)
                    except:
                        base_order = last_non_first_idx + 1
                    
                    # 处理后面第一次的，累加填充
                    for i in range(last_non_first_idx + 1, len(coil_data)):
                        item = coil_data[i]
                        new_order_no = str(base_order + (i - last_non_first_idx))
                        new_order_numbers.append((item['row'], new_order_no))
                        cursor.execute("""
                            INSERT OR REPLACE INTO coil_export_record 
                            (coil_no, first_export_order_no, last_export_time)
                ...
                # 现在检查是否有非第一次中间插入第一次的情况
                # 重新遍历，寻找被插入的情况
                insert_groups = []  # 存储(insert_after_idx, insert_items)
                i = 0
                while i < len(coil_data):
                    if not coil_data[i]['is_first_export']:
                        # 找到一个非第一次的，检查后面是否跟着第一次的
                        j = i + 1
                        insert_items = []
                        while j < len(coil_data) and coil_data[j]['is_first_export']:
                            insert_items.append(coil_data[j])
                            j += 1
                        if insert_items and j < len(coil_data) and not coil_data[j]['is_first_export']:
                            # 确认后面还有非第一次的，这是真正的插入
                            insert_groups.append((i, insert_items))
                        i = j
                    else:
                        i += 1
                
                # 处理插入的情况
                for insert_after_idx, insert_items in insert_groups:
                    insert_after_item = coil_data[insert_after_idx]
                    base_order = insert_after_item['first_order_no']
                    
                    for k, insert_item in enumerate(insert_items):
                        new_order_no = f"{base_order}-{k + 1}"
                        # 找到这个插入项在new_order_numbers中的位置并更新
                        for idx, (row, order) in enumerate(new_order_numbers):
                            if row == insert_item['row']:
                                new_order_numbers[idx] = (row, new_order_no)
                                break
                        # 更新数据库
                        cursor.execute("""
                            INSERT OR REPLACE INTO coil_export_record 
                            (coil_no, first_export_order_no, last_export_time)
                            VALUES (?, ?, CURRENT_TIMESTAMP)
                        """, (insert_item['coil_no'], new_order_no))
            
            # 6. 提交数据库更改
            conn.commit()
            conn.close()
            add_debug_log(f"  ✓ 数据库更新完成")
            
            # 7. 写回Excel文件
            add_debug_log(f"  写回Excel文件...")
            wb_copy = copy(workbook)
            sheet_copy = wb_copy.get_sheet(0)
            
            for row, order_no in new_order_numbers:
                sheet_copy.write(row, order_no_col, order_no)
            
            # 保存文件
            wb_copy.save(zhuanglu_file)
            add_debug_log(f"  ✓ Excel文件更新完成")
            
            return True
            
        except Exception as e:
            add_debug_log(f"  ✗ 序号处理失败: {str(e)}")
            import traceback
            traceback.print_exc()
            return False
...
class ExportConfigGUI:
    ...
    def __init__(self, root, plan_dir, main_app=None):
        ...
        def on_close():
            ...
            # 保存自动执行和防止退出登录选项的状态
            self.coordinates["auto_exec_enabled"] = self.auto_exec_var.get()
            self.coordinates["anti_logout_enabled"] = self.anti_logout_var.get()
