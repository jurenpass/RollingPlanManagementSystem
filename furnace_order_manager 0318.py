...
class FurnaceOrderManager:
    ...
    def load_data(self):
        ...
        plan_dir = os.path.join(self.plan_dir, "计划号")
        latest_zhuanglu = get_latest_file(plan_dir, "装炉顺序", ".xls")
        if latest_zhuanglu:
    ...
    def refresh_plan_list_from_file(self):
        ...
        try:
            ...
            for plan_no, count in plan_counts:
                # 检查是否有对应的文件
                status = ''
    ...
    def furnace_details(self):
        ...
        # 设置表头字体
        header_font = QFont("微软雅黑", 17, QFont.Bold)
        ...
        status_label.setFont(QFont("微软雅黑", 12, QFont.Bold))
        ...
        button_container.addWidget(show_btn)
        ...
        button_container.addWidget(print_btn)
        ...
        button_container.addWidget(return_btn)
        ...
        button_container.addWidget(theme_btn)
        ...
        def apply_theme(theme_name):
            ...
            table.setStyleSheet(f"""
                ...
                QTableWidget::item:selected {{ background-color: {colors['选中行']}; }}
                QTableWidget::item:hover {{ background-color: {colors['悬停行']}; }}
            """)
            ...
            # 更新状态标签
            status_label.setStyleSheet(f"color: {colors['表格文字']};")
        ...
        # 添加按钮间距
        for i in range(button_container.count()):
            if i > 0:
                button_container.insertSpacing(i, 10)
        ...
        def refresh_data():
            ...
            try:
                ...
                if not furnace_data:
                    print("没有装炉数据可处理")
                    status_label.setText("没有装炉数据可处理")
                    return
                    ...
                    if plan_file:
                        ...
                        if coil_data:
                            ...
                            for i, value in enumerate(row_data):
                                ...
                                elif i == 10 and isinstance(value, str) and ('回' in value or '无APS' in value):
                                    # 设置红色背景
                                    item.setBackground(QBrush(QColor('#FF0000')))
                                
                                # 设置对齐方式
                                if i == 9:  # 粗轧报信在第10列（索引9）
                                    item.setTextAlignment(Qt.AlignLeft | Qt.AlignTop)
                                elif i == 8:  # 公差带在第9列（索引8）
                ...
                # 更新状态显示
                status_label.setText(f"一共 {processed_count} 块")
            except Exception as e:
                print(f"刷新数据失败: {str(e)}")
                import traceback
                traceback.print_exc()
                status_label.setText(f"加载失败: {str(e)}")
    ...
    def run_excel_macro_with_pandas(self, file_path):
        ...
        # 8. 设置列宽（根据新的字段顺序）
        # 新顺序：序号、钢卷号、牌号（钢级）、坯宽、减宽、调宽、轧宽、公差带、粗轧报信、除鳞、坯厚、坯长、轧厚、中厚、去向、切边、RT2、强度、装炉顺、订宽
        col_widths = [
            ...
            int(4.5 * 256 * 1.04),       # 13. 轧厚
            int(4.3 * 256 * 1.04),       # 14. 中厚（中间坯厚度设定值）
            int(3.6 * 256 * 1.2),        # 15. 去向
            int(3.6 * 256 * 1.2),        # 16. 切边（切边方式）
            int(6.0 * 256 * 1.04),       # 17. RT2（RT2目标值）
            int(3.6 * 256 * 1.2),        # 18. 强度（硬度组）
            int(7 * 256 * 1.1),        # 19. 装炉顺（装炉顺序号）
            int(5 * 256 * 1.17),         # 20. 订宽（订货宽度）
            # 保留的其他字段
            ...
            for col_idx, col_name in enumerate(target_columns):
                ...
                if 映射后的列名 == "粗轧报信":
                    ...
                    if 映射后的列名 == "坯厚":
                        ...
                        # 坯宽字段使用左侧点划线边框样式
                        new_sheet.write(new_row_idx, col_idx, value, style_data_12pt_left_dashdot)
                    else:
                        # 其他字段使用默认12pt样式
                        new_sheet.write(new_row_idx, col_idx, value, style_data_12pt)
                elif 映射后的列名 == "钢卷号":
