...
class MainWindow(QMainWindow):
    ...
    def process_plans(self, auto_print=False):
        ...
        try:
            ...
            if not selected_items:
                QMessageBox.warning(self, "警告", "请先在计划号列表中选择要处理的计划号")
                return
            ...
            if already_processed:
                QMessageBox.information(self, "提示", f"以下计划号已处理过，无需重复处理：\n\n{', '.join(already_processed)}")
                return
    ...
    def show_selected(self):
        ...
        if not selected_items:
            QMessageBox.information(self, "提示", "请先选择计划")
            return
        print(f"显示选中的 {len(selected_items) // 4} 个计划")
    ...
    def print_selected(self):
        ...
        if not selected_items:
            QMessageBox.information(self, "提示", "请先选择计划")
            return
        print(f"打印选中的 {len(selected_items) // 4} 个计划")
    ...
    def auto_export(self, from_main_window=True):
                    ...
                    if event.success:
                        ...
                        if event.exported_plans:
                            ...
                            def close_msg_box():
                                msg_box.close()
                                # 关闭后激活主程序窗口
                                main_window_title = self.coordinates.get("main_window", "[无标题] - 记事本")
                                self.activate_window(main_window_title)
                            threading.Timer(3.0, close_msg_box).start()
                    else:
                        # 创建一个自动关闭的消息框
                        msg_box = QMessageBox(self)
        ...
        def export_in_background():
            ...
            try:
                ...
                # 即使装炉顺序导出失败，也继续执行后续步骤
                if not success1:
                    print("[×] 装炉顺序导出失败，继续执行后续步骤")
                ...
                # 即使总计划号列表导出失败，也继续执行后续步骤
                if not success3:
                    print(f"[×] 总计划号列表导出失败，继续执行后续步骤")
    ...
    def export_zhuanglu_shunxu(self, add_debug_log=None):
        ...
        if not window_exists:
            ...
            # 发送错误信号到主线程
            error_msg = f"开始导出装炉顺序...\n[1/9] 激活目标窗口: {window_title}...\n× 激活窗口失败"
            ...
            if not window_exists:
                ...
                try:
                    with open(plan_file, 'w') as f:
                        f.write(f"计划号: {plan_no}\n")
                    print(f"[√] 模拟导出成功，创建了文件: {plan_file}")
                    result = True
                except Exception as e:
                    print(f"[×] 模拟导出失败: {e}")
                    result = False
...
class FurnaceDetailsWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        self.block_count_label.setFont(QFont("微软雅黑", 24, QFont.Bold))
        self.block_count_label.setStyleSheet("color: blue;")
        self.block_count_label.setAlignment(Qt.AlignCenter)
        status_layout.addWidget(self.block_count_label)
        ...
        right_container.setStyleSheet("background-color: #f0f0f0;")
        ...
        # 下一次执行时间显示
        self.next_execution_label = QLabel("下一次执行: 无")
        self.next_execution_label.setFont(QFont("微软雅黑", 12, QFont.Bold))
        self.next_execution_label.setStyleSheet("color: blue;")
        self.next_execution_label.setAlignment(Qt.AlignRight)
        ...
        self.scroll_status_label.setAlignment(Qt.AlignRight)
        ...
        self.scroll_time_edit.setFixedWidth(60)
        ...
        self.print_btn.setFont(QFont("微软雅黑", 14))
        self.print_btn.setFixedSize(100, 40)
        self.print_btn.setStyleSheet(button_style)
        center_buttons.addWidget(self.print_btn)
        
        # 全部打印复选框
        self.all_print_checkbox = QCheckBox("全部打印")
        self.all_print_checkbox.setFont(QFont("微软雅黑", 14))
        center_buttons.addWidget(self.all_print_checkbox)
        
        # 返回按钮
        self.return_btn = QPushButton("返回")
        self.return_btn.setFont(QFont("微软雅黑", 14))
        self.return_btn.setFixedSize(100, 40)
        self.return_btn.setStyleSheet(button_style)
        center_buttons.addWidget(self.return_btn)
        ...
        self.scroll_toggle_btn.setFont(QFont("微软雅黑", 20))
        self.scroll_toggle_btn.setFixedSize(40, 40)
        self.scroll_toggle_btn.setStyleSheet(button_style)
        right_controls.addWidget(self.scroll_toggle_btn)
...
class SettingsWindow(QDialog):
    ...
    def showEvent(self, event):
        ...
        # 窗口显示后再次设置初始焦点，确保焦点位置正确
        self.set_initial_focus()
        
    def connect_signals(self):
        """连接信号，实现实时保存"""
        # 坐标设置文本框
        coordinate_text_boxes = [
            self.ui.mainWindowXEdit,
            self.ui.mainWindowYEdit,
            self.ui.furnaceWindowXEdit,
            self.ui.furnaceWindowYEdit,
            self.ui.planInputXEdit,
            self.ui.planInputYEdit,
            self.ui.queryBtnXEdit,
            self.ui.queryBtnYEdit,
            self.ui.exportBtnXEdit,
            self.ui.exportBtnYEdit,
            self.ui.closeBtnXEdit,
            self.ui.closeBtnYEdit,
            self.ui.detailExportBtnXEdit,
            self.ui.detailExportBtnYEdit
        ]
        
        # 间隔时间文本框
        interval_text_boxes = [
            self.ui.intervalMinutesEdit,
            self.ui.timeEdit,
            self.ui.intervalEdit  # 防退出登录时间间隔文本框
        ]
        
        # 复选框
        check_boxes = [
            self.ui.autoProcessCheckBox,
            self.ui.autoPrintCheckBox,
            self.ui.autoExecCheckBox,
            self.ui.antiLogoutCheckBox
        ]
        
        # 单选按钮
        radio_buttons = [
            self.ui.windowRadio1,
            self.ui.windowRadio2,
            self.ui.intervalModeRadio,
            self.ui.timeModeRadio
        ]
        
        # 为所有文本框添加textChanged信号
        for text_box in coordinate_text_boxes + interval_text_boxes:
            text_box.textChanged.connect(lambda: self.save_settings())
        
        # 为所有复选框添加stateChanged信号
        for check_box in check_boxes:
            check_box.stateChanged.connect(lambda: self.save_settings())
        
        # 为所有单选按钮添加toggled信号
        for radio_button in radio_buttons:
            radio_button.toggled.connect(lambda: self.save_settings())
    ...
    def load_settings(self):
        ...
        if os.path.exists(self.config_file):
            try:
                # 先检查文件大小
                if os.path.getsize(self.config_file) < 5:
    ...
    def apply_settings(self, settings):
        ...
        if "autoProcess" in settings:
            self.ui.autoProcessCheckBox.setChecked(settings["autoProcess"])
        ...
        if settings is None:
            try:
                settings = {
                    ...
                    # 自动导出设置
                    "autoProcess": self.ui.autoProcessCheckBox.isChecked(),
    ...
    def restore_defaults(self):
        ...
        try:
            ...
            self.apply_settings(hardcoded_defaults)
            self.save_settings(hardcoded_defaults)
    

    
    def setup_text_box_focus(self):
        """设置文本框焦点事件，进入时全选文本"""
        # 坐标设置文本框
        coordinate_text_boxes = [
            self.ui.mainWindowXEdit,
        ...
        for text_box in coordinate_text_boxes + interval_text_boxes:
            # 保存原始的事件处理函数
            original_mouse_press = text_box.mousePressEvent
            
            # 重写鼠标按下事件
            def create_mouse_handler(tb):
                def mouse_press_event(event):
                    # 检查是否是第一次点击（文本框没有焦点时）
                    if not tb.hasFocus():
                        # 先设置焦点
                        tb.setFocus(Qt.MouseFocusReason)
                        # 全选文本
                        tb.selectAll()
                    else:
                        # 如果已经有焦点且文本被全选，取消全选并将光标定位到点击位置
                        if tb.hasSelectedText():
                            # 调用原始事件处理，会自动取消全选并定位光标
            ...
            # 绑定事件处理函数
            text_box.mousePressEvent = create_mouse_handler(text_box)
    ...
    def set_initial_focus(self):
        ...
        # 设置焦点到第一个坐标文本框
        if coordinate_text_boxes:
            # 强制设置焦点，确保生效
            coordinate_text_boxes[0].setFocus(Qt.MouseFocusReason)
            coordinate_text_boxes[0].selectAll()
        elif interval_text_boxes:
            # 如果没有坐标文本框，设置焦点到第一个间隔时间文本框
            interval_text_boxes[0].setFocus(Qt.MouseFocusReason)
            interval_text_boxes[0].selectAll()
    
    def focusNextChild(self):
        """重写焦点移动方法，确保焦点不会自动跳到防退出登录文本框"""
        # 优先将焦点移动到坐标文本框
        coordinate_text_boxes = [
            self.ui.mainWindowXEdit,
            self.ui.mainWindowYEdit,
        ...
        # 确定下一个应该获得焦点的控件
        all_text_boxes = coordinate_text_boxes + interval_text_boxes
        if current_focus in all_text_boxes:
            index = all_text_boxes.index(current_focus)
            next_index = (index + 1) % len(all_text_boxes)
            next_widget = all_text_boxes[next_index]
            next_widget.setFocus(Qt.MouseFocusReason)
            next_widget.selectAll()
            return True
        else:
            # 如果当前焦点不在文本框中，设置焦点到第一个文本框
            if coordinate_text_boxes:
                coordinate_text_boxes[0].setFocus(Qt.MouseFocusReason)
                coordinate_text_boxes[0].selectAll()
            elif interval_text_boxes:
    ...
    def resizeEvent(self, event):
        ...
        if hasattr(self, 'table_widget'):
            # 获取表格的父容器宽度
            parent_width = self.table_widget.parent().width() - 20  # 减去边距
            if parent_width > 0:
                # 计算各列的宽度比例
                total_ratio = 170 + 100 + 800 + 395  # 总宽度
                plan_no_ratio = 170 / total_ratio
                count_ratio = 100 / total_ratio
                status_ratio = 800 / total_ratio
                coil_no_ratio = 395 / total_ratio
                # 调整列宽
                self.table_widget.setColumnWidth(0, int(parent_width * plan_no_ratio))
                self.table_widget.setColumnWidth(1, int(parent_width * count_ratio))
                self.table_widget.setColumnWidth(2, int(parent_width * status_ratio))
                self.table_widget.setColumnWidth(3, int(parent_width * coil_no_ratio))

class ProcessPlansWindow(QDialog):
    """处理计划窗口"""
    def __init__(self, parent=None):
        super().__init__(parent)
        self.setWindowTitle("处理计划")
        ...
        self.ok_btn.setFont(QFont("宋体", 14))
        self.ok_btn.setFixedWidth(100)
        btn_layout.addWidget(self.ok_btn)
        
        # 取消按钮
        self.cancel_btn = QPushButton("取消")
        self.cancel_btn.setFont(QFont("宋体", 14))
...
class ShowSelectedWindow(QDialog):
    """显示选中窗口"""
    def __init__(self, parent=None):
        super().__init__(parent)
        self.setWindowTitle("显示选中")
        self.setGeometry(300, 300, 800, 600)
...
class PrintSelectedWindow(QDialog):
    """打印选中窗口"""
    def __init__(self, parent=None):
        super().__init__(parent)
        self.setWindowTitle("打印选中")
