...
# 导入PyQt5模块
from PyQt5.QtWidgets import QApplication, QMainWindow, QTableWidget, QTableWidgetItem, QVBoxLayout, QHBoxLayout, QWidget, QPushButton, QLabel, QScrollArea, QHeaderView, QMessageBox, QLineEdit
...
class FurnaceOrderManager:
    ...
    def create_ui(self):
        ...
        # 防退出登录复选框（靠右）
        print(f"防退出登录配置: anti_logout_enabled = {self.coordinates.get('anti_logout_enabled', False)}")
        self.anti_logout_var = tk.BooleanVar(value=self.coordinates.get("anti_logout_enabled", False))
        print(f"防退出登录复选框值: {self.anti_logout_var.get()}")
        anti_logout_checkbox = ttk.Checkbutton(
            stats_frame,
            text="防止退出登录",
            variable=self.anti_logout_var,
            command=self.toggle_anti_logout
        )
        anti_logout_checkbox.pack(side=tk.RIGHT, padx=5)
        
        # 防退出登录定时器
        self.anti_logout_timer = None
        # 防退出登录线程运行状态
        self._anti_logout_running = False
        
        # 启动防退出登录功能（如果已启用）
        print(f"启动防退出登录功能检查: {self.anti_logout_var.get()}")
        if self.anti_logout_var.get():
            print("调用start_anti_logout_timer()")
            self.start_anti_logout_timer()
    ...
    def toggle_anti_logout(self):
        """切换防退出登录功能"""
        # 保存防退出登录状态到配置
        self.coordinates["anti_logout_enabled"] = self.anti_logout_var.get()
        # 保存配置到文件
        with open(os.path.join(self.plan_dir, "export_coordinates.json"), 'w', encoding='utf-8') as f:
        ...
        if self.anti_logout_var.get():
            ...
            # 禁用防退出登录
            self.stop_anti_logout_timer()
    
    def start_anti_logout_timer(self):
        """启动防退出登录定时器"""
        # 停止之前的定时器
        self.stop_anti_logout_timer()
        
        # 直接使用配置中的间隔时间（秒），默认15秒
        interval_seconds = self.coordinates.get("anti_logout_interval", 15)
        
        # 标记线程是否应该继续运行
        self._anti_logout_running = True
        
        # 使用线程来实现防退出登录功能，避免GUI事件循环阻塞
        def anti_logout_thread():
            import time
            # 等待一段时间再执行，避免程序启动时立即执行
            time.sleep(5)
            while self._anti_logout_running:
                time.sleep(interval_seconds)
                if self._anti_logout_running:
                    self.anti_logout_action()
        
        # 启动线程
        self.anti_logout_thread = threading.Thread(target=anti_logout_thread, daemon=True)
        self.anti_logout_thread.start()
        
        print(f"防退出登录功能已启动，间隔时间：{interval_seconds}秒")
    
    def stop_anti_logout_timer(self):
        """停止防退出登录定时器"""
        # 停止定时器（兼容旧代码）
        if hasattr(self, 'anti_logout_timer') and self.anti_logout_timer:
            try:
                self.root.after_cancel(self.anti_logout_timer)
                self.anti_logout_timer = None
            except:
                pass
        ...
        # 等待线程结束（可选，为了确保线程完全停止）
        if hasattr(self, 'anti_logout_thread') and self.anti_logout_thread:
            try:
                # 设置超时，避免线程阻塞
                import threading
                self.anti_logout_thread.join(timeout=1.0)
            except:
                pass
        
        print("防退出登录功能已停止")
    
    def _activate_window_internal(self, window_title):
        """内部窗口激活实现
    ...
    def activate_window(self, window_title):
        ...
        Returns:
            bool: 是否成功激活
        """
        return self._activate_window_internal(window_title)
    
    def anti_logout_action(self):
        """防退出登录动作：激活设置中选择的窗口，点击屏幕中央，然后切换回主程序窗口"""
        try:
            import win32gui
            import win32api
            import win32con
            ...
            try:
                ...
                def find_window_callback(hwnd, extra):
                    if win32gui.IsWindowVisible(hwnd):
                        ...
                        # 更宽松的窗口标题匹配
                        if "装炉" in window_title or "明细" in window_title:
                ...
                if hwnds:
                    print(f"防退出登录：找到 {len(hwnds)} 个可能的装炉明细窗口")
                    for hwnd, window_title in hwnds:
                        print(f"防退出登录：尝试关闭窗口 '{window_title}' (hwnd: {hwnd})")
                        
                        # 方法1: 发送WM_CLOSE消息
                        try:
                            win32gui.SendMessage(hwnd, win32con.WM_CLOSE, 0, 0)
                            print(f"防退出登录：已发送WM_CLOSE消息到窗口")
                            time.sleep(1.0)  # 增加等待时间
                        except Exception as e:
                            print(f"防退出登录：发送WM_CLOSE失败: {str(e)}")
                        
                        # 方法2: 发送WM_QUIT消息
                        try:
                            win32gui.SendMessage(hwnd, win32con.WM_QUIT, 0, 0)
                            print(f"防退出登录：已发送WM_QUIT消息到窗口")
                            time.sleep(1.0)  # 增加等待时间
                        except Exception as e:
                            print(f"防退出登录：发送WM_QUIT失败: {str(e)}")
                        
                        # 方法3: 强制终止窗口进程
                        try:
                            _, process_id = win32process.GetWindowThreadProcessId(hwnd)
                            if process_id:
                                print(f"防退出登录：窗口进程ID: {process_id}")
                                # 打开进程，使用PROCESS_TERMINATE权限
                                PROCESS_TERMINATE = 1
                                ...
                                if handle:
                                    ...
                                    print(f"防退出登录：已强制终止窗口进程 (PID: {process_id}, 结果: {result})")
                                    time.sleep(1.5)  # 增加等待时间
                                else:
                                    print(f"防退出登录：无法打开进程 (PID: {process_id})")
                        except Exception as e:
                            print(f"防退出登录：强制终止进程失败: {str(e)}")
                        
                        # 方法4: 发送WM_DESTROY消息
                        try:
                            win32gui.SendMessage(hwnd, win32con.WM_DESTROY, 0, 0)
                            print(f"防退出登录：已发送WM_DESTROY消息到窗口")
                            time.sleep(0.5)
                        except Exception as e:
                            print(f"防退出登录：发送WM_DESTROY失败: {str(e)}")
                    time.sleep(2.0)  # 增加等待时间，确保窗口完全关闭
                else:
                    print("防退出登录：未找到装炉明细窗口")
            except Exception as e:
                print(f"防退出登录：关闭装炉明细窗口失败: {str(e)}")
                import traceback
                traceback.print_exc()
            
            # 清理引用（在后台线程中不直接访问Tkinter对象）
            if hasattr(self, 'detail_window'):
                try:
                    # 尝试通过PyQt5的close方法关闭窗口
                    if self.detail_window:
                        self.detail_window.close()
                        print("防退出登录：已通过PyQt5 close()方法关闭装炉明细窗口")
                except Exception as e:
                    print(f"防退出登录：PyQt5 close()方法失败: {str(e)}")
                finally:
                    # 注意：在后台线程中删除属性可能会有问题，这里我们只打印信息
                    print("防退出登录：已清理装炉明细窗口引用")
            
            # 保存主程序窗口句柄（使用Windows API获取当前活动窗口，避免访问Tkinter方法）
            try:
                main_window_hwnd = win32gui.GetForegroundWindow()
                print(f"防退出登录：获取到主程序窗口句柄: {main_window_hwnd}")
            except Exception as e:
                print(f"防退出登录：获取主程序窗口句柄失败: {str(e)}")
                main_window_hwnd = None
            
            # 从配置中获取窗口标题（优先使用自定义窗口名称）
            ...
            print(f"防退出登录：尝试激活窗口 '{window_title}'")
            
            # 尝试激活窗口
            activation_success = self._activate_window_internal(window_title)
            ...
            if activation_success:
                ...
                # 再次尝试激活，确保窗口在最前面
                self._activate_window_internal(window_title)
                time.sleep(0.3)
                ...
                print(f"防退出登录：屏幕中央位置 ({center_x}, {center_y})")
                ...
                if main_window_hwnd:
                    try:
                        win32gui.ShowWindow(main_window_hwnd, win32con.SW_SHOW)
                        time.sleep(0.2)
                        win32gui.SetForegroundWindow(main_window_hwnd)
                        time.sleep(0.2)
                        print(f"防退出登录：已激活{window_title}窗口，点击屏幕中央，然后切换回主程序窗口")
                    except Exception as e:
                        print(f"防退出登录：已激活{window_title}窗口，点击屏幕中央，但无法切换回主程序窗口: {str(e)}")
                else:
                    print(f"防退出登录：已激活{window_title}窗口，点击屏幕中央")
            else:
                print(f"防退出登录：无法激活{window_title}窗口")
                # 尝试使用pyautogui作为备选方案
                try:
                    import pyautogui
                    print("防退出登录：尝试使用pyautogui激活窗口")
                    pyautogui.press('win')
                    time.sleep(0.5)
                    pyautogui.typewrite(window_title)
                    time.sleep(1)
                    ...
                    if main_window_hwnd:
                        try:
                            ...
                            win32gui.SetForegroundWindow(main_window_hwnd)
                            time.sleep(0.2)
                            print(f"防退出登录：使用pyautogui激活{window_title}窗口并点击屏幕中央，然后切换回主程序窗口")
                        except Exception as e:
                            print(f"防退出登录：使用pyautogui激活{window_title}窗口并点击屏幕中央，但无法切换回主程序窗口: {str(e)}")
                            # 尝试使用Alt+Tab切换窗口
                            try:
                                pyautogui.hotkey('alt', 'tab')
                                time.sleep(0.5)
                                print(f"防退出登录：使用Alt+Tab切换窗口")
                            except Exception as e:
                                print(f"防退出登录：Alt+Tab切换窗口失败: {str(e)}")
                    else:
                        print(f"防退出登录：使用pyautogui激活{window_title}窗口并点击屏幕中央")
                except Exception as e:
                    print(f"防退出登录：pyautogui激活失败: {str(e)}")
        except Exception as e:
            print(f"防退出登录动作执行失败：{str(e)}")
            import traceback
            traceback.print_exc()
    ...
    def mark_selected_as_processed(self):
        ...
        print(f"已将 {len(selection)} 个计划号标注为已处理")
    ...
    def mark_selected_as_printed(self):
        ...
        print(f"已将 {len(selection)} 个计划号标注为已打印")
    ...
    def furnace_details(self):
        ...
        # 创建一个事件，用于通知线程退出
        self.detail_window_exit_event = threading.Event()
        
        def run_furnace_details():
            try:
                # 确保QApplication实例存在
                ...
                # 导入需要的PyQt5组件
                from PyQt5.QtWidgets import QLineEdit, QMainWindow, QTableWidget, QTableWidgetItem, QVBoxLayout, QWidget, QPushButton, QLabel, QHBoxLayout, QHeaderView, QScrollArea, QMenu, QDialog, QListWidget, QListWidgetItem, QColorDialog, QGridLayout, QMessageBox
        ...
        # 窗口关闭后清理资源
        print("装炉明细窗口已关闭，清理资源...")
        try:
            if hasattr(self, 'detail_window'):
                del self.detail_window
                print("已清理装炉明细窗口引用")
            if hasattr(self, 'detail_window_exit_event'):
                self.detail_window_exit_event.set()
                print("已设置退出事件")
        except Exception as e:
        ...
        print(f"装炉明细窗口运行出错: {str(e)}")
        import traceback
        traceback.print_exc()

    # 启动线程
    detail_thread = threading.Thread(target=run_furnace_details)
    ...
    def run_excel_macro_with_pandas(self, file_path):
        ...
        # 设置左对齐
        alignment_data_left = xlwt.Alignment()
        alignment_data_left.horz = xlwt.Alignment.HORZ_LEFT
        alignment_data_left.vert = xlwt.Alignment.VERT_CENTER
        style_data_16pt_left_border.alignment = alignment_data_left
        ...
        # 8. 设置列宽（根据新的字段顺序）
        # 新顺序：序号、钢卷号、牌号（钢级）、坯宽、减宽、调宽、轧宽、公差带、粗轧报信、除鳞、坯厚、坯长、轧厚、中厚、RT2、强度、订宽、去向、切边、装炉顺
        col_widths = [
            int(4.5 * 256 * 1.2),       # 1. 序号
            int(19 * 256 * 1.04),       # 2. 钢卷号
            int(18 * 256 * 1.04),       # 3. 牌号（钢级）
            int(7 * 256 * 1.04),         # 4. 坯宽
            int(7.5 * 256 * 1.05),       # 5. 减宽（侧压量）
            int(5.6 * 256 * 1.04),       # 6. 调宽（板坯宽度调宽标记）
            int(6.5 * 256 * 1.13),       # 7. 轧宽（轧宽+（余量））
            int(10 * 256 * 1.03),        # 8. 公差带（碳）
            int(33 * 256 * 1.04),        # 9. 粗轧报信
            int(6 * 256 * 1.08),         # 10. 除鳞（层号）
            int(6.5 * 256 * 1.04),       # 11. 坯厚
            int(7 * 256 * 1.13),         # 12. 坯长
            int(6.5 * 256 * 1.04),       # 13. 轧厚
            int(5.5 * 256 * 1.04),       # 14. 中厚（中间坯厚度设定值）
            int(6.57 * 256 * 1.04),      # 15. RT2（RT2目标值）
            int(4.5 * 256 * 1.2),        # 16. 强度（硬度组）
            int(5 * 256 * 1.17),         # 17. 订宽（订货宽度）
            int(3.6 * 256 * 1.2),        # 18. 去向
            int(3.6 * 256 * 1.2),        # 19. 切边（切边方式）
            int(7 * 256 * 1.1),        # 20. 装炉顺（装炉顺序号）
            # 保留的其他字段
        ...
        if 轧厚列索引 >= 0 and 强度列索引 >= 0:
            # 计算打印时间应该写入的行号，确保不会覆盖提示信息
            # 提示信息从第1行开始写入，所以打印时间应该写入到提示信息行之后
            打印时间行号 = 1 + len(提示信息列表)
            # 确保打印时间行号不超过字段列名行号 - 1
            打印时间行号 = min(打印时间行号, 字段列名行号 - 1)
            new_sheet.write_merge(打印时间行号, 打印时间行号, 轧厚列索引, 强度列索引, f"{当前时间}", style_time)
            ...
            for col_idx, col_name in enumerate(target_columns):
                ...
                if 映射后的列名 == "粗轧报信":
                    ...
                    if 需标注:
                        value = str(value) + "Δ"
                    # 为钢卷号创建专用的左对齐样式
                    style_钢卷号 = xlwt.XFStyle()
                    font_钢卷号 = xlwt.Font()
                    font_钢卷号.name = '仿宋'
                    font_钢卷号.height = 320  # 16pt
                    font_钢卷号.bold = True
                    style_钢卷号.font = font_钢卷号
                    # 设置左对齐
                    alignment_钢卷号 = xlwt.Alignment()
                    alignment_钢卷号.horz = xlwt.Alignment.HORZ_LEFT
                    alignment_钢卷号.vert = xlwt.Alignment.VERT_CENTER
                    style_钢卷号.alignment = alignment_钢卷号
                    # 设置边框
                    borders_钢卷号 = xlwt.Borders()
                    if is_first_column:
                        borders_钢卷号.left = xlwt.Borders.THIN
                    else:
                        borders_钢卷号.left = xlwt.Borders.NO_LINE
                    borders_钢卷号.right = xlwt.Borders.NO_LINE
                    borders_钢卷号.top = xlwt.Borders.THIN
                    borders_钢卷号.bottom = xlwt.Borders.THIN
                    style_钢卷号.borders = borders_钢卷号
                    # 使用创建的左对齐样式
                    new_sheet.write(new_row_idx, col_idx, value, style_钢卷号)
                elif 映射后的列名 == "强度":
                    # 强度字段只取个位数值
                    try:
                        if value:
    ...
    def print_excel_file(self, file_path):
        ...
        try:
            ...
            # 找到强度列的索引（查找"强度"列，因为实际存储的列名是"强度"）
            强度列索引 = -1
            for col_idx, col_name in enumerate(current_columns):
                # 直接查找"强度"列（实际存储的列名）
                if col_name == "强度":
            ...
            try:
                ...
                # 设置首页和后续页面的表头
                # 首页打印表头为$1:$n，后续页面表头为$2:$n
                # 这里n为表头结束行，假设表头只占用一行
                # 对于多行表头，需要根据实际情况调整
                worksheet.PageSetup.PrintTitleRows = "$2:$2"  # 后续页面表头从第二行开始
                
                # 注意：Excel的PrintTitleRows设置会应用到所有页面
                # 要实现首页和后续页面不同的表头，需要使用Excel的"首页不同"设置
                # 但这需要更复杂的VBA操作，这里我们使用一个简单的方法：
                # 1. 将第一行作为首页特有的表头
                # 2. 从第二行开始作为所有页面的表头
                # 这样首页会显示第一行和第二行，后续页面只显示第二行
    ...
    def load_coordinate_config(self):
        ...
        default_coords = {
            ...
            # 自动执行设置
            "auto_exec_interval": 30,              # 自动执行间隔（分钟）
            "auto_exec_times": "",                 # 自动执行指定时间
            # 防退出登录设置
            "anti_logout_interval": 100 * 60,      # 防退出登录间隔（秒）
            "anti_logout_interval_value": 100,      # 防退出登录间隔（分钟）
            "anti_logout_unit": "分钟",             # 防退出登录时间单位
            "anti_logout_enabled": False            # 是否启用防退出登录
        }
        ...
        if os.path.exists(config_file):
            try:
                with open(config_file, 'r', encoding='utf-8') as f:
                    ...
                    print(f"读取到的配置: {saved}")
                    print(f"读取到的anti_logout_enabled: {saved.get('anti_logout_enabled', 'Not found')}")
                    # 合并默认值和保存值
                    default_coords.update(saved)
                    print(f"合并后anti_logout_enabled: {default_coords.get('anti_logout_enabled')}")
...
class ExportConfigGUI:
    ...
    DEFAULT_COORDS = {
        ...
        "debug_mode": False,                   # 调试模式（显示详细日志）
        # 纸张设置
        "use_pywin32_paper_size": False,        # 是否使用pywin32设置纸张大小（关闭可提高处理速度）
        
        # 防退出登录设置
        "anti_logout_interval_value": 100,      # 防止退出登录时间间隔（分钟）
        "anti_logout_interval": 6000,          # 防止退出登录时间间隔（秒）
        "anti_logout_unit": "分钟",             # 防止退出登录时间单位
    ...
    def create_widgets(self):
        ...
        entry = ttk.Entry(spacing_frame, textvariable=self.plan_spacing_var, width=10)
        ...
        entry = ttk.Entry(offset_frame, textvariable=self.first_plan_offset_var, width=10)
        entry.bind('<FocusIn>', lambda e: e.widget.selection_range(0, tk.END))
        entry.pack(side=tk.LEFT, padx=5)
        ...
        # 恢复默认值按钮
        restore_frame = ttk.Frame(self.main_frame)
        restore_frame.pack(fill=tk.X, pady=10)
        
        ttk.Button(restore_frame, text="🔄 恢复默认值",
                  command=self.restore_default_coords).pack(side=tk.LEFT, padx=5)
        ...
        # 速度选择回调函数
        def on_speed_select(value):
            self.coordinates["export_speed"] = value
            # 保存配置到文件
            self.save_coordinates()
            # 立即更新到MainApp的coordinates中
            if self.main_app:
                self.main_app.coordinates.update(self.coordinates)
            self.log(f"自动导出速度已更新: {value}")
        ...
        for speed in speed_options:
            ttk.Radiobutton(speed_frame, text=speed, variable=self.export_speed_var, value=speed, 
                          command=lambda v=speed: on_speed_select(v)).pack(side=tk.LEFT, padx=15)
        ...
        def on_custom_window_change(*args):
            custom_title = self.custom_window_var.get().strip()
            self.coordinates["custom_window_title"] = custom_title
            # 保存配置到文件
            self.save_coordinates()
            # 立即更新到MainApp的coordinates中
            if self.main_app:
                self.main_app.coordinates.update(self.coordinates)
            if custom_title:
                self.log(f"自定义窗口名称已更新: {custom_title}")
            else:
                self.log("自定义窗口名称已清空")
        
        # 绑定自定义窗口名称变化事件
        self.custom_window_var.trace('w', on_custom_window_change)
        ...
        anti_logout_frame.pack(anchor=tk.W, pady=10)
        
        # 防止退出登录时间间隔设置
        anti_logout_row = ttk.Frame(anti_logout_frame)
        anti_logout_row.pack(anchor=tk.W, fill=tk.X)
        
        ttk.Label(anti_logout_row, text="防止退出登录时间间隔：", font=('微软雅黑', 9)).pack(side=tk.LEFT, padx=5)
        self.anti_logout_interval_var = tk.StringVar(value=str(self.coordinates.get("anti_logout_interval_value", 100)))
        
        # 创建带输入限制的Entry
        def validate_input(new_value):
            """验证输入值，只允许输入正整数"""
            if not new_value:
                return True  # 允许空值，在失去焦点时处理
        ...
        validate_cmd = anti_logout_row.register(validate_input)
        entry = ttk.Entry(anti_logout_row, textvariable=self.anti_logout_interval_var, width=5, validate="key", validatecommand=(validate_cmd, "%P"))
        # 双击全选
        entry.bind('<Double-1>', lambda e: e.widget.selection_range(0, tk.END))
        # 聚焦时全选
        entry.bind('<FocusIn>', lambda e: e.widget.selection_range(0, tk.END))
        entry.pack(side=tk.LEFT, padx=5)
        ttk.Label(anti_logout_row, text="分钟", font=('微软雅黑', 9)).pack(side=tk.LEFT, padx=5)
        ttk.Label(anti_logout_row, text="（30~999）", font=('微软雅黑', 9, 'italic'), foreground="gray").pack(side=tk.LEFT, padx=5)
        
        # 标志变量，用于防止重复弹窗
        anti_logout_popup_shown = False
        
        # 为防退出登录时间间隔文本框添加事件绑定
        def on_anti_logout_return(event):
            nonlocal anti_logout_popup_shown
            # 验证输入值
            ...
            if not value:
                ...
                # 低于最小值时弹窗提示
                if not anti_logout_popup_shown:
                    anti_logout_popup_shown = True
                    import tkinter as tk
                    from tkinter import messagebox
                    messagebox.showerror("错误", "防止退出登录时间间隔范围为30~999", parent=self.root)
                    # 全选数值等待重新输入
        ...
        def on_anti_logout_focus_out(event):
            nonlocal anti_logout_popup_shown
            # 焦点离开时验证输入值
            value = event.widget.get()
            if not value:
        ...
        entry.bind('<Return>', on_anti_logout_return)
        entry.bind('<FocusOut>', on_anti_logout_focus_out)
        
        # 自动执行设置
        auto_exec_frame = ttk.LabelFrame(self.main_frame, text="自动执行设置", padding="10")
        auto_exec_frame.pack(fill=tk.X, pady=10)
        
        # 按时间间隔自动执行
        interval_frame = ttk.Frame(auto_exec_frame)
        interval_frame.pack(anchor=tk.W, fill=tk.X, pady=5)
        
        ttk.Label(interval_frame, text="按时间间隔自动执行:", font=('微软雅黑', 10, 'bold')).pack(anchor=tk.W, pady=2)
        
        interval_sub_frame = ttk.Frame(interval_frame)
        interval_sub_frame.pack(anchor=tk.W, padx=10)
        
        ttk.Label(interval_sub_frame, text="间隔时间:", font=('微软雅黑', 9)).pack(side=tk.LEFT, padx=5)
        self.auto_exec_interval_var = tk.StringVar(value=str(self.coordinates.get("auto_exec_interval", 30)))
        ...
        validate_cmd = interval_sub_frame.register(validate_input)
        interval_entry = ttk.Entry(interval_sub_frame, textvariable=self.auto_exec_interval_var, width=5, validate="key", validatecommand=(validate_cmd, "%P"))
        # 双击全选
        interval_entry.bind('<Double-1>', lambda e: e.widget.selection_range(0, tk.END))
        # 聚焦时全选
        interval_entry.bind('<FocusIn>', lambda e: e.widget.selection_range(0, tk.END))
        interval_entry.pack(side=tk.LEFT, padx=5)
        ttk.Label(interval_sub_frame, text="分钟", font=('微软雅黑', 9)).pack(side=tk.LEFT, padx=5)
        ttk.Label(interval_sub_frame, text="（5~999）", font=('微软雅黑', 9, 'italic'), foreground="gray").pack(side=tk.LEFT, padx=5)
        
        # 按指定时间自动执行
        time_frame = ttk.Frame(auto_exec_frame)
        time_frame.pack(anchor=tk.W, fill=tk.X, pady=5)
        
        ttk.Label(time_frame, text="按指定时间自动执行:", font=('微软雅黑', 10, 'bold')).pack(anchor=tk.W, pady=2)
        
        time_sub_frame = ttk.Frame(time_frame)
        time_sub_frame.pack(anchor=tk.W, padx=10)
        
        ttk.Label(time_sub_frame, text="指定时间 (格式: HH:MM, 多个时间用逗号分隔):", font=('微软雅黑', 9)).pack(anchor=tk.W, pady=2)
        self.auto_exec_times_var = tk.StringVar(value=self.coordinates.get("auto_exec_times", ""))
        entry = ttk.Entry(time_sub_frame, textvariable=self.auto_exec_times_var, width=50)
        entry.bind('<FocusIn>', lambda e: e.widget.selection_range(0, tk.END))
        entry.pack(anchor=tk.W, pady=2)
        ttk.Label(time_sub_frame, text="例如: 08:00, 12:00, 16:00", font=('微软雅黑', 9, 'italic'), foreground="gray").pack(anchor=tk.W, padx=5)
        
        # 标志变量，用于防止重复弹窗
        anti_logout_popup_shown = False
        auto_exec_popup_shown = False
        
        # 自动执行设置变化事件
        def on_auto_exec_change(*args):
            # 保存间隔时间设置
            try:
                interval = int(self.auto_exec_interval_var.get())
                self.coordinates["auto_exec_interval"] = interval
            except ValueError:
                self.coordinates["auto_exec_interval"] = 60
                self.auto_exec_interval_var.set("60")
            
            # 保存指定时间设置
            times = self.auto_exec_times_var.get().strip()
            self.coordinates["auto_exec_times"] = times
        ...
        # 绑定事件
        def on_return(event):
            nonlocal auto_exec_popup_shown
            # 验证自动执行间隔输入值
            if event.widget == interval_entry:
                ...
                if not value:
                    ...
                    # 低于最小值时弹窗提示
                    if not auto_exec_popup_shown:
                        auto_exec_popup_shown = True
                        import tkinter as tk
                        from tkinter import messagebox
                        messagebox.showerror("错误", "自动执行时间间隔范围为5~999", parent=self.root)
                        # 全选数值等待重新输入
                        event.widget.selection_range(0, tk.END)
                        event.widget.focus()
                        # 重置标志
        ...
        # 为间隔时间文本框添加事件绑定
        interval_entry.bind('<Return>', on_return)
        interval_entry.bind('<FocusOut>', on_focus_out)
        
        # 为指定时间文本框添加事件绑定
        time_entry = None
        for child in time_sub_frame.winfo_children():
            if isinstance(child, ttk.Entry):
                time_entry = child
                break
        if time_entry:
            time_entry.bind('<Return>', on_return)
    ...
    def create_coord_row(self, parent, label_text, coord_key, string_var):
        ...
        def on_coord_change(*args):
            ...
            if coord:
                ...
                self.log(f"{label_text} 坐标已更新: {coord}")
        
        # 绑定变量变化事件
        string_var.trace('w', on_coord_change)
    ...
    def load_values_to_ui(self):
        ...
        self.show_warning_var.set(self.coordinates.get("show_operation_warning", False))
        ...
        # 防退出登录间隔时间（秒）
        self.anti_logout_interval_var.set(str(self.coordinates.get("anti_logout_interval_value", 15)))
        
        # 自动执行设置
        self.auto_exec_interval_var.set(str(self.coordinates.get("auto_exec_interval", 30)))
        self.auto_exec_times_var.set(self.coordinates.get("auto_exec_times", ""))
    ...
    def on_anti_logout_changed(self):
        """防退出登录设置改变时的回调"""
        if self.main_app and hasattr(self.main_app, 'anti_logout_var'):
            # 更新间隔时间设置
            try:
                interval_value = int(self.anti_logout_interval_var.get())
                
                # 使用分钟作为单位，转换为秒存储
                interval_seconds = interval_value * 60
                
                # 保存设置
                self.coordinates["anti_logout_interval"] = interval_seconds
                self.coordinates["anti_logout_interval_value"] = interval_value
                self.coordinates["anti_logout_unit"] = "分钟"  # 固定为分钟
            except ValueError:
                # 如果输入无效，使用默认值100分钟
                self.coordinates["anti_logout_interval"] = 100 * 60
                self.coordinates["anti_logout_interval_value"] = 100
                self.coordinates["anti_logout_unit"] = "分钟"
                self.anti_logout_interval_var.set("100")
            ...
            interval_value = self.coordinates.get("anti_logout_interval_value", 100)
            self.log(f"防退出登录间隔时间已更新：{interval_value}分钟")
    ...
    def capture_coordinate(self, key, name):
        ...
        # 激活选定的窗口
        window_title = self.coordinates.get("custom_window_title", "").strip() or self.coordinates.get("test_window", "BGCMS1-宝钢股份多基地制造管理系统运行环境")
        self.log(f"正在激活窗口: {window_title}")
        # 导入必要的模块
        import win32gui
        import win32con
        import time
    ...
    def save_all_coordinates(self):
        ...
        try:
            ...
            # 如果输入无效，使用默认值90分钟
            self.coordinates["anti_logout_interval"] = 90 * 60
            self.coordinates["anti_logout_interval_value"] = 90
            self.coordinates["anti_logout_unit"] = "分钟"
    ...
    def restore_defaults(self):
        """恢复默认值"""
        import tkinter.messagebox as messagebox
        if self.custom_messagebox("确认", "确定要恢复默认值吗？"):
    ...
    def activate_window(self, window_name):
        ...
        # 使用统一的窗口激活方法
        success = self.main_app._activate_window_internal(window_name)
        ...
        if success:
            ...
            self.custom_messagebox("警告", f"未找到窗口: {window_name}")
            return False
