...
class MainWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        self.anti_logout_timer = QTimer()
        self.anti_logout_timer.timeout.connect(self.anti_logout)
        ...
        self.anti_logout_checkbox.setChecked(settings.get("antiLogout", False))
    ...
    def start_auto_execution(self):
        ...
        try:
            ...
            # 启动防注销服务
            anti_logout = settings.get("antiLogout", False)
            if anti_logout:
                anti_logout_interval = settings.get("antiLogoutInterval", 50)
                anti_logout_ms = anti_logout_interval * 60 * 1000  # 转换为毫秒
                self.anti_logout_timer.start(anti_logout_ms)
                print(f"启动防注销服务，间隔：{anti_logout_interval}分钟")
    ...
    def connect_signals(self):
        ...
        # 连接防退出登录复选框
        self.anti_logout_checkbox.stateChanged.connect(self.on_anti_logout_changed)
    ...
    def on_anti_logout_changed(self, state):
        """防退出登录复选框状态变化处理"""
        try:
            enabled = state == Qt.Checked
            print(f"防退出登录复选框状态变化: {enabled}")
            
            # 更新设置文件
            settings = self.get_settings()
            settings["antiLogout"] = enabled
            ...
            print(f"处理防退出登录复选框状态变化失败: {str(e)}")
    ...
    def print_excel_file(self, file_path):
        ...
        try:
            ...
            try:
                ...
                # 设置纸张大小为美国Fanfold (标准值为137)
                try:
                    worksheet.PageSetup.PaperSize = 137
                    print("成功设置纸张大小为美国Fanfold (137)")
                except Exception as e:
                    print(f"设置纸张大小失败: {str(e)}")
                    print("继续使用默认纸张大小")
    ...
    def auto_export(self, from_main_window=True):
        ...
        if main_window_title not in current_title:
            print(f"不在主程序画面，尝试激活: {main_window_title}")
            ...
            main_windows = []
            win32gui.EnumWindows(find_main_window, main_windows)
            
            if main_windows:
                main_hwnd = main_windows[0]
                print(f"找到主程序窗口: {main_window_title} (HWND: {main_hwnd})")
                
                # 尝试多种方法激活主程序窗口
                try:
                    import win32con
                    # 方法1：ShowWindow + SetForegroundWindow
                    win32gui.ShowWindow(main_hwnd, win32con.SW_RESTORE)
                    win32gui.SetForegroundWindow(main_hwnd)
                    print("已激活主程序窗口 (方法1)")
                ...
                print(f"未找到主程序窗口: {main_window_title}")
        else:
            print("已经在主程序画面，无需激活")
        
        # 等待主程序窗口完全激活
        import time
        time.sleep(1)  # 等待1秒，确保主程序窗口完全激活
        ...
        def export_in_background():
            ...
            try:
                ...
                try:
                    ...
                    class ActivateMainWindowEvent(QEvent):
                        def __init__(self):
                            super().__init__(QEvent.User)
                    
                    # 发送事件到主线程
                    event = ActivateMainWindowEvent()
                    QCoreApplication.postEvent(self, event)
                    print("已发送激活主窗口事件到主线程")
                except Exception as e:
                    print(f"切换到主程序窗口失败: {e}")
                    import traceback
                    traceback.print_exc()
    ...
    def activate_window(self, window_title):
        ...
        try:
            ...
            # 最终检查：即使所有方法都返回False，也要检查窗口是否真的被激活了
            try:
                # 检查当前前台窗口是否是我们尝试激活的窗口
                current_foreground = win32gui.GetForegroundWindow()
                if current_foreground == hwnd:
                    print('最终检查：窗口实际上已被激活')
                    return True
                
                # 检查窗口是否可见且在顶层
                rect = win32gui.GetWindowRect(hwnd)
                if rect[2] > rect[0] and rect[3] > rect[1]:  # 窗口有实际大小
                    # 尝试获取窗口的位置和大小，确认窗口存在
                    print('最终检查：窗口存在且可见')
                    # 即使没有成为前台窗口，只要窗口可见，就认为激活成功
                    return True
            except Exception as e:
                print(f'最终检查失败: {e}')
...
class FurnaceDetailsWindow(QMainWindow):
    ...
    def show_context_menu(self, event):
        ...
        if is_roll_changed or is_custom_info:
            ...
            def on_change_roll():
                ...
                merge_item.setBackground(QBrush(QColor('#FFFF00')))
                font = QFont()
                font.setPointSize(20)
                font.setBold(True)
                ...
                self.table_widget.setItem(new_row, 0, merge_item)
                self.table_widget.setSpan(new_row, 0, 1, 16)
    ...
    def save_roll_change_info(self, row):
        ...
        try:
            ...
            info_file = os.path.join(plan_dir, "roll_change_info.json")
            ...
            info.append({'type': 'roll_change', 'row': row})
    ...
    def save_custom_info(self, row, text):
        ...
        try:
            ...
            plan_dir = os.path.join(os.getcwd(), "计划号")
            os.makedirs(plan_dir, exist_ok=True)
            info_file = os.path.join(plan_dir, "custom_info.json")
            ...
            info.append({'type': 'custom_info', 'row': row, 'text': text})
            with open(info_file, 'w', encoding='utf-8') as f:
                json.dump(info, f, ensure_ascii=False)
        except Exception as e:
    ...
    def activate_window(self, window_title):
        ...
        try:
            ...
            if hwnd:
                ...
                win32gui.SetForegroundWindow(hwnd)
                # 确保窗口被激活
                ctypes.windll.user32.SetWindowPos(hwnd, win32con.HWND_TOPMOST, 0, 0, 0, 0, win32con.SWP_NOMOVE | win32con.SWP_NOSIZE | win32con.SWP_SHOWWINDOW)
                ctypes.windll.user32.SetWindowPos(hwnd, win32con.HWND_NOTOPMOST, 0, 0, 0, 0, win32con.SWP_NOMOVE | win32con.SWP_NOSIZE | win32con.SWP_SHOWWINDOW)
                return True
            return False
        except Exception as e:
            print(f"激活窗口失败: {e}")
        ...
        try:
            ...
            for row_idx, row_data in enumerate(rows):
                ...
                if not is_special_row:
                    ...
                    # 特殊行（换辊行或自定义信息行），如果是全部打印则包含
                    if is_all_print:
                        print(f"写入特殊行第 {print_row_idx - 2} 行: {row_data}")
                        # 写入序号
                        sheet.write(print_row_idx, 0, "", style_data_seq)
                        ...
                        # 自动调整行高
                        sheet.row(print_row_idx).height = 500  # 基础行高
                        sheet.row(print_row_idx).height_mismatch = True
    ...
    def print_excel_file(self, file_path):
        ...
        try:
            ...
            try:
                ...
                # 设置纸张大小为美国Fanfold (标准值为137)
                try:
                    worksheet.PageSetup.PaperSize = 137  # 美国Fanfold
                    print("成功设置纸张大小为美国Fanfold (137)")
                except Exception as e:
                    print(f"设置纸张大小失败: {str(e)}")
                    print("继续使用默认纸张大小")
    ...
    def restore_defaults(self):
        ...
        if default_settings is None:
            default_settings = {
                ...
                "selectedWindow": "BCGMS1-宝钢股份基地制造管理系统运行环境",
                "autoProcess": True,
                "autoPrint": True,
                "autoExec": True,
                "execMode": "interval",
