...
class FurnaceOrderManager:
    ...
    def start_auto_execution(self):
        ...
        def auto_execution_function():
            import time
            print("自动执行服务线程已启动")
            last_interval_exec_time = None  # 记录上次间隔执行时间
                ...
                try:
                    ...
                    if exec_mode == "interval":
                        ...
                        if self.is_auto_exec_running:
                            ...
                            # 检查是否已经在最近执行过（避免重复执行）
                            if last_interval_exec_time is None or current_time - last_interval_exec_time > interval * 0.9:
                                print(f"执行时间到达，执行自动导出...")
                                # 记录执行时间
                                last_interval_exec_time = current_time
                                # 使用主线程执行GUI操作
                        ...
                        if exec_times:
                            ...
                            if valid_times:
                                print(f"自动执行服务已启动，将在指定时间执行: {exec_times}")
                                ...
                                if next_exec_time:
                                    print(f"下次执行时间: {next_exec_time[0]:02d}:{next_exec_time[1]:02d}")
                                    print(f"等待时间: {min_time_diff}分钟")
