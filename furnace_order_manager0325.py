...
class FurnaceOrderManager:
    def __init__(self, root):
        ...
        self.auto_exec_event = threading.Event()
        self.auto_exec_event.set()  # 初始状态为运行
    ...
    def custom_messagebox(self, title, message, msg_type='info'):
        ...
        if msg_type == 'yesno':
            ...
            # 强制将焦点设置到确定按钮
            msg_window.after(100, lambda: ok_btn.focus_force())
            msg_window.wait_window()
    ...
    def start_anti_logout(self):
        ...
        # 获取时间间隔（分钟）
        interval_minutes = self.coordinates.get("anti_logout_interval_value", 100)
        # 转换为秒
        ...
        print(f"防退出登录已启动，每隔{interval_minutes}分钟激活窗口并按下F2键")
    ...
    def start_auto_execution(self):
        ...
        # 重置事件
        self.auto_exec_event = threading.Event()
        self.auto_exec_event.set()
        
        # 获取自动执行模式
        auto_exec_mode = self.coordinates.get("auto_exec_mode", "interval")
        
        if auto_exec_mode == "interval":
            # 按时间间隔执行
            interval_minutes = self.coordinates.get("auto_exec_interval", 30)
            interval_seconds = interval_minutes * 60
            
            def timer_function():
                import time
                while self.auto_exec_event.is_set():
                    # 等待指定的时间间隔
                    for _ in range(interval_seconds):
                        if not self.auto_exec_event.is_set():
                            return
                        time.sleep(1)
            ...
            self.auto_exec_timer = threading.Thread(target=timer_function, daemon=True)
            self.auto_exec_timer.start()
            print(f"自动执行已启动，每隔{interval_minutes}分钟执行一次")
        else:
            # 按指定时间执行
            exec_times = self.coordinates.get("auto_exec_times", "").strip()
            if exec_times:
                def time_check_function():
                    # 记录已执行的时间，避免重复执行
                    executed_times = set()
                    ...
                    while self.auto_exec_event.is_set():
                        ...
                        # 检查是否到了执行时间
                        for exec_time in exec_times.split(","):
                            exec_time = exec_time.strip()
                            execution_key = f"{current_day}_{exec_time}"
                            
                            if current_time == exec_time and execution_key not in executed_times:
                                # 确保在主程序窗口执行
                                self.root.lift()
                                self.root.focus_force()
                                
                                self.auto_exec_action()
                                executed_times.add(execution_key)
                                # 避免重复执行，等待1分钟
                                if not self.auto_exec_event.wait(60):
                                    break
                ...
                self.auto_exec_timer = threading.Thread(target=time_check_function, daemon=True)
                self.auto_exec_timer.start()
                print(f"自动执行已启动，将在指定时间执行: {exec_times}")
    
    def stop_auto_execution(self):
        """停止自动执行"""
        # 清除事件，通知线程停止
        if hasattr(self, 'auto_exec_event'):
            self.auto_exec_event.clear()
        
        # 等待线程结束
        if hasattr(self, 'auto_exec_timer') and self.auto_exec_timer is not None and self.auto_exec_timer.is_alive():
            self.auto_exec_timer.join(timeout=2)  # 最多等待2秒
        
        print("自动执行已停止")
    
    def auto_exec_action(self):
        """自动执行动作：执行自动导出按钮的所有代码"""
        try:
            # 执行自动导出
            self.auto_export()
            print("自动执行完成")
...
class ExportConfigGUI:
    ...
    def create_widgets(self):
        ...
        def on_spacing_enter(event):
            try:
                value = int(self.plan_spacing_var.get())
                if value > 0:
        ...
        def on_offset_enter(event):
            try:
                value = int(self.first_plan_offset_var.get())
        ...
        # 执行方式选择
        self.auto_exec_mode_var = tk.StringVar(value="interval")  # 默认按时间间隔执行
        ...
        ttk.Radiobutton(interval_mode_frame, text="按时间间隔自动执行:", variable=self.auto_exec_mode_var, value="interval", command=on_mode_change).pack(side=tk.LEFT, padx=5)
        ...
        ttk.Radiobutton(time_mode_frame, text="按指定时间自动执行:", variable=self.auto_exec_mode_var, value="time", command=on_mode_change).pack(side=tk.LEFT, padx=5)
        ...
        def on_time_enter(event):
            # 更新配置
            self.coordinates["auto_exec_times"] = self.auto_exec_times_var.get().strip()
            self.save_coordinates()
            # 移动焦点到框架
            auto_exec_frame.focus_set()
            # 重启自动执行服务以应用新的时间
    ...
    def load_values_to_ui(self):
        ...
        # 防止退出登录时间间隔
        self.anti_logout_interval_var.set(str(self.coordinates.get("anti_logout_interval_value", self.DEFAULT_COORDS["anti_logout_interval_value"])))
        # 自动执行设置
        self.auto_exec_mode_var.set(self.coordinates.get("auto_exec_mode", self.DEFAULT_COORDS["auto_exec_mode"]))
        self.auto_exec_interval_var.set(str(self.coordinates.get("auto_exec_interval", self.DEFAULT_COORDS["auto_exec_interval"])))
        self.auto_exec_times_var.set(self.coordinates.get("auto_exec_times", self.DEFAULT_COORDS["auto_exec_times"]))
    ...
    def save_all_coordinates(self):
        ...
        # 保存自动执行设置
        self.coordinates["auto_exec_mode"] = self.auto_exec_mode_var.get()
        self.coordinates["auto_exec_interval"] = int(self.auto_exec_interval_var.get()) if self.auto_exec_interval_var.get().isdigit() else self.DEFAULT_COORDS["auto_exec_interval"]
        self.coordinates["auto_exec_times"] = self.auto_exec_times_var.get().strip()
