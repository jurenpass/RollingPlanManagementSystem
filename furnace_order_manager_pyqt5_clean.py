...
class MainWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        self.table_widget.setHorizontalHeaderLabels(["计划�b7, "块数", "状�?, "起止钢卷�?])
        
        # 隐藏垂直表头（行号列�?
        self.table_widget.verticalHeader().setVisible(False)
        
        # 设置列宽（按照要求固定）
        self.table_widget.setColumnWidth(0, 170)  # 计划号列�?
        self.table_widget.setColumnWidth(1, 100)  # 块数列宽
        self.table_widget.setColumnWidth(2, 800)  # 状态列�?
        self.table_widget.setColumnWidth(3, 395)  # 起止钢卷号列�?
        ...
        # 统计信息标签
        # 总块数显�?
        self.stats_label = QLabel("总块�? 0 | 无文�? 0")
        ...
        # 下一次执行时间显�?
        self.next_execution_label = QLabel("下一次执�? �?)
        ...
        # 初始化标签显�?
        print("初始化下一次执行时间标�?)
        self.next_execution_label.setText("下一次执�? �?)
        ...
        self.select_all_btn = QPushButton("全�?)
        ...
        self.deselect_all_btn = QPushButton("取消全�?)
    ...
    def create_required_folders(self):
        """自动创建所需的文件夹和文�?""
        # 创建计划号文件夹
        plan_dir = os.path.join(self.plan_dir, "计划�b7)
        ...
        if not os.path.exists(backup_dir):
            ...
            try:
                print(f"�?创建备份文件�? {backup_dir}")
            except UnicodeEncodeError:
                print(f"创建备份文件�? {backup_dir}")
        
        # 清理 backup 目录中超�?2小时的文�?
    ...
    def cleanup_backup_directory(self, backup_dir):
        """清理backup目录中超�?2小时的文件，将其移动�?原始文件"子目�?
    ...
    def load_processed_plans(self):
        ...
        try:
            ...
            print(f"已加�?{len(self.processed_plans)} 个计划号，共 {total_coil_count} 个已处理钢卷�?)
        except Exception as e:
            print(f"加载已处理计划失�? {str(e)}")
    ...
    def load_printed_plans(self):
        ...
        try:
            ...
            print(f"已加�?{len(self.printed_plans)} 个计划号，共 {total_coil_count} 个已打印钢卷�?)
        except Exception as e:
            print(f"加载已打印计划失�? {str(e)}")
            self.printed_plans = {}
    ...
    def load_no_aps_plans(self):
        ...
        try:
            ...
            print(f"已加�?{len(self.no_aps_plans)} 个无APS计划")
        except Exception as e:
    ...
    def load_low_roll_width_plans(self):
        ...
        try:
            ...
            print(f"已加�?{len(self.low_roll_width_plans)} 个低轧宽计划")
        except Exception as e:
            print(f"加载低轧宽计划失�? {str(e)}")
            self.low_roll_width_plans = set()
    ...
    def has_unprinted_coils(self, plan_no, file_path):
        ...
        try:
            ...
            if not plan_no.startswith('D') and not plan_no.startswith('d'):
                ...
                # 新格式dict：如果没有打印记录，或者没有钢卷号记录，认为需要打�?
    ...
    def mark_all_coils_processed(self, plan_no, file_path):
        ...
        try:
            ...
            print(f"标记 {plan_no} �?{len(all_coil_nos)} 个钢卷号为已处理（D开头计划）")
        except Exception as e:
    ...
    def mark_all_coils_printed(self, plan_no, file_path):
        ...
        try:
            ...
            print(f"标记 {plan_no} �?{len(all_coil_nos)} 个钢卷号为已打印（D开头计划）")
        except Exception as e:
            ...
            with open(processed_file, 'w', encoding='utf-8') as f:
                ...
                if isinstance(self.processed_plans, set):
                    for plan_no in self.processed_plans:
                        f.write(f"{plan_no}\n")
                    print(f"已保�?{len(self.processed_plans)} 个已处理计划号（旧格式）")
                else:
                    # 新格式dict
                    ...
                    total_coil_count = sum(len(coils) for plan_no, coils in self.processed_plans.items() 
                                           if plan_no.startswith('D') or plan_no.startswith('d'))
                    print(f"已保�?{len(self.processed_plans)} 个已处理计划号，�?{total_coil_count} 个已处理钢卷号（仅D开头计划）")
        except Exception as e:
            ...
            with open(printed_file, 'w', encoding='utf-8') as f:
                ...
                if isinstance(self.printed_plans, set):
                    for plan_no in self.printed_plans:
                        f.write(f"{plan_no}\n")
                    print(f"已保�?{len(self.printed_plans)} 个已打印计划号（旧格式）")
                else:
                    # 新格式dict
                    ...
                    total_coil_count = sum(len(coils) for plan_no, coils in self.printed_plans.items() 
                                           if plan_no.startswith('D') or plan_no.startswith('d'))
                    print(f"已保�?{len(self.printed_plans)} 个已打印计划号，�?{total_coil_count} 个已打印钢卷号（仅D开头计划）")
        except Exception as e:
        ...
        try:
            ...
            with open(no_aps_file, 'w', encoding='utf-8') as f:
                for plan_no in self.no_aps_plans:
                    f.write(f"{plan_no}\n")
            print(f"已保�?{len(self.no_aps_plans)} 个无APS计划�b7)
        except Exception as e:
            print(f"保存无APS计划号失�? {str(e)}")
        ...
        try:
            ...
            with open(low_roll_width_file, 'w', encoding='utf-8') as f:
                for plan_no in self.low_roll_width_plans:
                    f.write(f"{plan_no}\n")
            print(f"已保�?{len(self.low_roll_width_plans)} 个低轧宽计划�b7)
        except Exception as e:
    ...
    def 匹配除鳞钢种(self, 牌号, removePhosphorusList):
        ...
        for 钢种 in removePhosphorusList:
            ...
            # 通配符匹配：�?1E"开�?
            if 钢种字符�?== "1E" and 牌号字符�?startswith("1E"):
                return True
            
            # 通配符匹配：�?-p"结尾
            if 钢种字符�?== "-p" and 牌号字符�?lower().endswith("-p"):
                return True
            
            # 通配符匹配：�?-P"结尾
            if 钢种字符�?== "-P" and 牌号字符�?endswith("-P"):
                return True
    ...
    def event(self, event):
        ...
        if hasattr(event, 'valid_no_file_plans'):
            try:
                ...
                print(f"开始导出无文件计划号明�? {valid_no_file_plans}")
                
                # 导出无文件计划号明细
                for plan_no in valid_no_file_plans:
                    print(f"导出计划�b7 {plan_no}")
                    # 调用导出计划号明细函�?
            ...
            if (not hasattr(event, 'refresh_type') and 
                ...
                try:
                    ...
                    try:
                        ...
                        print(f"win32gui激活失�? {e}")
                    
                    # 方法3：使用QApplication的方�?
                    try:
                        from PyQt5.QtWidgets import QApplication
                        QApplication.setActiveWindow(self)
                        print("已使用QApplication方法激活主窗口")
                    except Exception as e:
                        print(f"QApplication激活失�? {e}")
                    
                    # 方法4：使用keybd_event模拟按键
                    try:
                        import win32api
                        import win32con
                        # 模拟Alt键按下和释放，有时能帮助激活窗�?
                        win32api.keybd_event(win32con.VK_MENU, 0, 0, 0)
                        win32api.keybd_event(win32con.VK_MENU, 0, win32con.KEYEVENTF_KEYUP, 0)
                        print("已使用keybd_event激活主窗口")
                    except Exception as e:
                        print(f"keybd_event激活失�? {e}")
                ...
                try:
                    ...
                    if event.success:
                        ...
                        if event.exported_plans:
                            ...
                            print(f"已更新最近导出的计划�b7 {event.exported_plans}")
                            # 刷新计划号列�?使绿色标注生�?
                            ...
                            print(f"已选中 {len(event.exported_plans)} 个导出的计划�b7)
    ...
    def get_default_coordinates(self):
        ...
        return {
                    ...
                    "first_plan": [235, 207],
                    "plan_detail_export": [79, 851],
                    "plan_spacing": 21,
                    "first_plan_offset": 0,
                    "test_window": "BGCMS1-宝钢股份多基地制造管理系统运行环�?,
    ...
    def scan_and_rename_plan_files(self):
        ...
        try:
            ...
            print(f"扫描完成: 处理 {len(files)} 个文�?跳过 {skipped_count} 个文�?)
        except Exception as e:
            print(f"扫描并重命名计划号文件失�? {str(e)}")
    ...
    def process_plans(self, auto_print=False, show_result=True):
        ...
        try:
            ...
            if not selected_items:
                ...
                msg_box.setText("请先在计划号列表中选择要处理的计划�b7)
            ...
            if not os.path.exists(plan_dir):
                ...
                msg_box.setText(f"计划号文件夹不存�? {plan_dir}")
                msg_box.setStandardButtons(QMessageBox.Ok)
                msg_box.setWindowFlags(Qt.WindowStaysOnTopHint | Qt.Dialog | Qt.MSWindowsFixedSizeDialogHint)
                msg_box.setModal(True)
                QApplication.setActiveWindow(msg_box)
            ...
            if already_processed and show_result:
                ...
                msg_box.setText(f"以下计划号已处理�?无需重复处理：\n\n{', '.join(already_processed)}")
                msg_box.setStandardButtons(QMessageBox.Ok)
                msg_box.setWindowFlags(Qt.WindowStaysOnTopHint | Qt.Dialog | Qt.MSWindowsFixedSizeDialogHint)
                msg_box.setModal(True)
                QApplication.setActiveWindow(msg_box)
            ...
            if auto_print and success_files:
                ...
                if show_result:
                    ...
                    # 统计信息
                    stats_label = QLabel(f"成功: {processed_count} �?| 失败: {failed_count} �?| 跳过: {skipped_count} �?)
    ...
    def print_furnace_details(self):
        ...
        try:
            ...
            if not rows:
                ...
                QMessageBox.information(self, "提示", "没有数据可打�?)
                return
            ...
            # 设置列名
            提示�?= ["装炉明细打印"]
            headers = ["序号", "计划�b7, "钢卷�?, "牌号（钢级）", "坯宽", "减宽", "调宽", "轧宽", "公差�?,
            ...
            col_widths = [
                ...
                int(7.0 * 256 * 1.04),       # 2. 计划�b7
                int(19 * 256 * 1.04),        # 3. 钢卷�?
            ...
            # 写入提示行（第一行）并合并单元格
            sheet.write_merge(0, 0, 0, len(headers) - 1, 提示�?[0], style_title)
            ...
            # 写入数据�?
            seq_num = 1
            print(f"准备写入 {len(rows)} 行数�?)
            for row_idx, row_data in enumerate(rows):
                print(f"写入�?{row_idx + 1} �? {row_data}")
                # 写入序号
                ...
                # 写入其他字段
                for data_idx, value in enumerate(row_data[1:], 2):  # 从钢卷号开始，Excel列从2开�?
                    if data_idx == 2:  # 钢卷号列�?6pt，左对齐�?
            ...
            if os.path.exists(temp_file_path):
                ...
                if file_size > 0:
                    ...
                    if success:
                        ...
                        # 显示打印成功消息
                        from PyQt5.QtWidgets import QMessageBox
                        QMessageBox.information(self, "完成", f"打印成功\n\n文件路径：{temp_file_path}")
                    else:
                        print("打印失败，保留文件供手动打印")
                        # 显示打印失败消息
                        from PyQt5.QtWidgets import QMessageBox
                        QMessageBox.information(self, "提示", f"打印失败，文件已生成：\n{temp_file_path}\n\n请手动打开此文件查看内容并打印")
                else:
                    print("警告：文件生成但为空")
                    # 显示文件为空消息
                    from PyQt5.QtWidgets import QMessageBox
    ...
    def run_excel_macro_with_pandas(self, file_path):
        ...
        提取指定字段,删除其他字段,添加"轧宽+（余量）"�?装炉顺序�?字段
        import os
        import xlwt
        ...
        装炉顺序映射 = {}
        轧宽余量映射 = {}
        装炉顺序文件 = os.path.join(self.plan_dir, "计划�b7, "装炉顺序.xls")
        
        # 加载�?工作表的钢种列表
        钢种列表 = []
        �?工作表文�?= os.path.join(self.plan_dir, "计划�b7, "装炉顺序.xls")  # 假设�?工作表在同一个文件中
        if os.path.exists(�?工作表文�?:
            success, result = self.safe_read_excel(�?工作表文�? max_retries=3, retry_delay=0.5)
            if success:
                workbook3 = result
                try:
                    if workbook3.nsheets >= 5:
                        sheet3 = workbook3.sheet_by_index(4)  # �?工作表（索引4�?
                        for row_idx in range(sheet3.nrows):
                            钢种�?= sheet3.cell_value(row_idx, 0)
                            if 钢种�?
                                钢种列表.append(str(钢种�?.strip())
                except Exception:
                    pass
                finally:
        ...
        if os.path.exists(装炉顺序文件):
            ...
            if success:
                ...
                装炉钢卷号列 = headers.index("钢卷�?) if "钢卷�? in headers else -1
                装炉顺序号列 = headers.index("装炉顺序�?) if "装炉顺序�? in headers else -1
                轧宽余量�?= -1
                if "轧宽�?余量�? in headers:
                    轧宽余量�?= headers.index("轧宽�?余量�?)
                ...
                if 装炉钢卷号列 != -1:
                    ...
                    for 钢卷号字符串, 原装炉顺序号 in 钢卷号顺序号列表:
                        ...
                        for row_idx in range(1, sheet2.nrows):
                            钢卷�?= sheet2.cell_value(row_idx, 装炉钢卷号列)
                            if 钢卷�?and str(钢卷�? == 钢卷号字符串:
                                if 轧宽余量�?!= -1:
                                    轧宽余量�?= sheet2.cell_value(row_idx, 轧宽余量�?
                                break
        ...
        # 3. 定义目标列（按照用户要求的最终顺序）
        target_columns = [
            "顺序�?, "钢卷�?, "牌号（钢级）", "坯宽", "侧压�?, "板坯宽度调宽标记", "轧宽+（余量）",
            "�?, "粗轧报信", "层号", "坯厚", "坯长", "轧厚", "中间坯厚度设定�?,
            "RT2目标�?, "硬度�?, "订货宽度", "去向", "切边方式", "板坯头部宽度", "板坯尾部宽度",
            "计划�b7, "热轧产品分类", "炼钢钢种", "宽度负公�?, "宽度正公�?, "板坯炉后拒收次数", "轧宽",
            "装炉顺序�?
        ]
        ...
        # 注意：xlwt的paper_size值对应Excel的纸张类型枚�?
        # 39 = xlPaperFanfoldUS (14.875 x 11 inches)
        new_sheet.paper_size = 137  # 137 = xlPaperFanfoldUS (14.875 x 11 inches) - 连续折叠�?
        # 设置页边距为0（使用inches为单位）
        ...
        borders_header_strength.right = xlwt.Borders.THIN  # 右边框实�?
        ...
        borders_data_14pt_dashed.left = xlwt.Borders.NO_LINE  # 去掉左边�?
        ...
        borders_data_14pt_double.left = xlwt.Borders.MEDIUM_DASHED  # 长间隔虚�?
        borders_data_14pt_double.right = xlwt.Borders.MEDIUM_DASHED  # 长间隔虚�?
        ...
        alignment_data_16pt_left.horz = xlwt.Alignment.HORZ_LEFT  # 左对�?
        ...
        borders_data_16pt_left.bottom = xlwt.Borders.THIN  # 下边�?
        ...
        for row_idx in range(1, sheet.nrows):
            if "计划�b7 in current_columns:
                计划号�?= sheet.cell_value(row_idx, current_columns.index("计划�b7))
                if 计划号�?
                    计划�b7= str(计划号�?
                    break
        ...
        for col_idx, col_name in enumerate(target_columns):
            映射后的列名 = 字段名映�?get(col_name, col_name)
            if 映射后的列名 == "强度":
                强度列索�?= col_idx
            elif 映射后的列名 == "装炉�?:
                装炉顺列索引 = col_idx
        
        # 如果找到强度�?标题跨列到该列；否则跨所有列
        if 强度列索�?>= 0:
            标题结束�?= 强度列索�?
        else:
            标题结束�?= len(target_columns) - 1
        ...
        col_widths = [
            ...
            int(7 * 256 * 1.13),        # 7. 轧宽（轧�?+（余量））（原程序值：7�?
            ...
            int(4.71 * 256 * 1.2),      # 16. 强度（硬度组）（修改�?4.71�?
        ...
        for row_idx in range(1, sheet.nrows):
            ...
            if "钢卷�? in row_data:
                ...
                if 钢卷号�?
                    ...
                    if 钢卷号字符串 in 装炉顺序映射:
                        ...
                        row_data["装炉顺序�?] = 原装炉顺序号
                        # 存储装炉顺用于后续写�?
                        row_data["装炉�?] = 装炉顺序映射[钢卷号字符串]["装炉�?]
            ...
            if "牌号（钢级）" in row_data and "炼钢钢种" in row_data and "热轧产品分类" in row_data:
                牌号�?= str(row_data.get("牌号（钢级）", "")).strip()
                炼钢钢种�?= str(row_data.get("炼钢钢种", "")).strip()
                热轧产品分类�?= str(row_data.get("热轧产品分类", "")).strip()
                
                # 规则1：如果牌号（钢级）列�?SPHC"且炼钢钢种列�?P3A1",则牌号（钢级）列值添�?-P"
                if 牌号�?== "SPHC" and 炼钢钢种�?== "P3A1":
                    row_data["牌号（钢级）"] = 牌号�?+ "-P"
                else:
                    # 规则2：在�?工作表A列中查找当前行的牌号（钢级）字段和炼钢钢种字段�?
                    牌号在列表中 = 牌号�?in 钢种集合
                    炼钢钢种在列表中 = 炼钢钢种�?in 钢种集合
                    
                    # 规则3：如果都找不�?
                    if not 牌号在列表中 and not 炼钢钢种在列表中:
                        # 若热轧产品分类字段是"L"�?M"�?O",则牌号（钢级）字段值设为炼钢钢种字段�?
                        if 热轧产品分类�?in ["L", "M", "O"]:
                            row_data["牌号（钢级）"] = 炼钢钢种�?
                        # 若热轧产品分类字段列�?X"且C列值末尾不�?-P",则添�?-P"
                        elif 热轧产品分类�?== "X" and not 牌号�?endswith("-P"):
                            row_data["牌号（钢级）"] = 牌号�?+ "-P"
            ...
            if "层号" in target_columns and "牌号（钢级）" in row_data:
                ...
                # 检查回炉坯�?
                是回炉坯 = (回炉坯�?!= 0 and 回炉坯�?!= "" and 回炉坯�?is not None)
                ...
                if brand and apsGrades:
                    在APS列表�?= brand in apsGrades
                    if not 在APS列表�?
                        # 如果不在APS列表�?添加"无APS"标识
                        if 除鳞�?
                            除鳞�?= 除鳞�?+ "无APS"
                        else:
                            除鳞�?= "无APS"
            ...
            # 检查除鳞字段是否包�?�?�?如果是则标记需标注
            if "层号" in row_data:
                除鳞�?= str(row_data.get("层号", "")).strip()
                if 除鳞�?and "�? in 除鳞�?
                    row_data["需标注"] = True  # 除鳞包含回字时标注钢卷号
            
            # 检查坯厚是�?230,如果是则标记需标注
            if "坯厚" in row_data:
                坯厚�?= row_data.get("坯厚", 0)
                try:
                    坯厚数�?= float(坯厚�? if 坯厚�?!= "" else 0
                    if 坯厚数�?> 230:
                        row_data["需标注"] = True
                except (ValueError, TypeError):
                    pass
            
            # 检查牌号是否在APS列表�?如果不在则标记需标注
            ...
            if "板坯宽度调宽标记" in row_data and str(row_data.get("板坯宽度调宽标记", "")).strip() != "0":
                if "板坯头部宽度" in row_data and "板坯尾部宽度" in row_data and "坯宽" in row_data:
                    板坯头部宽度�?= row_data.get("板坯头部宽度", 0)
                    板坯尾部宽度�?= row_data.get("板坯尾部宽度", 0)
                    
                    try:
                        板坯头部宽度�?= float(板坯头部宽度�? if 板坯头部宽度�?!= "" else 0
                        板坯尾部宽度�?= float(板坯尾部宽度�? if 板坯尾部宽度�?!= "" else 0
                    except (ValueError, TypeError):
                        板坯头部宽度�?= 0
                        板坯尾部宽度�?= 0
                    
                    调宽差�?= 板坯头部宽度�?- 板坯尾部宽度�?
                    row_data["板坯宽度调宽标记"] = 调宽差�?
                    
                    坯宽新�?= max(板坯头部宽度�? 板坯尾部宽度�?
                    row_data["坯宽"] = 坯宽新�?
                    
                    if "轧宽" in row_data:
                        轧宽�?= row_data.get("轧宽", 0)
                        try:
                        ...
                        if 减宽新�?>= 240:
                            ...
                            row_data["减宽超标"] = True
                            row_data["需标注"] = True
                            row_data["标注信息"].append(f"减宽量超�? {减宽新值}")
                        elif 减宽新�?< 0:
                            逆宽轧制板坯�?+= 1
                            row_data["减宽超标"] = True
                            row_data["需标注"] = True
                    ...
                    # 判断头部宽度和尾部宽度中小�?=轧宽，则标注调宽三角符号
                    if "轧宽" in row_data:
                        轧宽�?= row_data.get("轧宽", 0)
                        try:
                            轧宽�?= float(轧宽�? if 轧宽�?!= "" else 0
                            # 计算头部宽度和尾部宽度的较小�?
                            宽度较小�?= min(板坯头部宽度�? 板坯尾部宽度�?
                            if 宽度较小�?<= 轧宽�?
                                # 标注调宽三角符号
                                row_data["板坯宽度调宽标记"] = str(调宽差�? + "Δ"
                                row_data["需标注"] = True
                                # 记录调宽问题信息，用于提示行显示
                                if "钢卷�? in row_data:
                                    钢卷�?= row_data.get("钢卷�?, "")
            ...
            if "板坯宽度调宽标记" not in row_data or str(row_data.get("板坯宽度调宽标记", "")).strip() == "0":
                减宽�?= 0
                if "侧压�? in row_data:
                    减宽�?= row_data.get("侧压�?, 0)
                
                try:
                    减宽�?= float(减宽�? if 减宽�?!= "" else 0
                except (ValueError, TypeError):
                    减宽�?= 0
                ...
                if 减宽�?>= 240:
                    ...
                    row_data["减宽超标"] = True
                    row_data["需标注"] = True
                    row_data["标注信息"].append(f"减宽量超�? {减宽值}")
                elif 减宽�?< 0:
                    逆宽轧制板坯�?+= 1
                    row_data["减宽超标"] = True
                    row_data["需标注"] = True
            ...
            # 检查轧�?（余量）是否<=920,如果是则添加标记
            if "轧宽+（余量）" in row_data:
                轧宽余量�?= row_data.get("轧宽+（余量）", "")
                try:
                    轧宽余量数�?= float(轧宽余量�? if 轧宽余量�?!= "" else 0
                    if 轧宽余量数�?<= 920:
                        row_data["轧宽+（余量）"] = str(int(轧宽余量数�?) + "Δ"
                        row_data["低轧宽标�?] = True
                        row_data["需标注"] = True
                        低轧宽板坯数 += 1
            ...
            if "轧宽+（余量）" in row_data and "轧宽" in row_data:
                轧宽余量�?= row_data.get("轧宽+（余量）", 0)
                轧宽�?= row_data.get("轧宽", 0)
                try:
                    轧宽余量�?= float(轧宽余量�? if 轧宽余量�?!= "" else 0
                    轧宽�?= float(轧宽�? if 轧宽�?!= "" else 0
                    if 轧宽余量�?> 轧宽�?
                        差�?= int(轧宽余量�?- 轧宽�?
                        负公差原始�?= row_data.get("宽度负公�?, 0)
                        正公差原始�?= row_data.get("宽度正公�?, 0)
                        try:
                            负公差原始�?= float(负公差原始�? if 负公差原始�?!= "" else 0
                            正公差原始�?= float(正公差原始�? if 正公差原始�?!= "" else 0
                            row_data["宽度负公�?] = int(负公差原始�?- 差�?
                            row_data["宽度正公�?] = int(正公差原始�?- 差�?
                        except (ValueError, TypeError):
                            pass
                except (ValueError, TypeError):
                    pass
            ...
            # 5. 填充公差带字�?
            if "�? in row_data and "宽度负公�? in row_data and "宽度正公�? in row_data:
                负公差�?= row_data.get("宽度负公�?, "")
                正公差�?= row_data.get("宽度正公�?, "")
                
                # 处理负公差�?
                if isinstance(负公差�? (int, float)):
                    if 负公差�?is_integer():
                        负公差�?= str(int(负公差�?)
                    else:
                        负公差�?= f"{负公差�?.1f}"
                ...
                # 检查正公差是否<=15,如果是则标注*
                try:
                    正公差数�?= float(row_data.get("宽度正公�?, 0))
                    if 正公差数�?<= 15:
                        # 在公差带标注*
                        row_data["�?] = row_data["�?] + "*"
                        # 在钢卷号标注*
                        if "钢卷�? in row_data:
                            row_data["钢卷�?] = str(row_data["钢卷�?]) + "*"
                except (ValueError, TypeError):
                    pass
        ...
        # 对D开头的计划号，检查并删除没有钢卷号的�?
        if 计划�b7and (计划�b7startswith('D') or 计划�b7startswith('d')):
            filtered_row_data = []
            for row_data in all_row_data:
                ...
                # 尝试多种可能的钢卷号列名
                coil_no_value = None
                for coil_col_name in ["钢卷�?, "序号", "COIL_NO", "coil_no"]:
            ...
            块数 = len(all_row_data)
            print(f"D开头计划号处理后，剩余{块数}�?)
        
        # 如果处理后数据为空，保留原始数据（防止文件变空白�?
        if not all_row_data:
            print("警告：处理后数据为空，保留原始数�?)
            # 重新读取原始数据
            all_row_data = []
            for row_idx in range(1, sheet.nrows):
                row_data = {}
        ...
        # 9. 写入表头
        # 第一行：大标�?轧制计划明细�?
        new_sheet.write_merge(0, 0, 0, 标题结束�? "轧制计划明细�?, style_title)
        ...
        if 无APS钢种�?> 0:
            无APS钢种列表 = ", ".join(sorted(已添加无APS的钢�?)
            提示信息列表.append(f"{序号}.无APS钢种{无APS钢种数}个{无APS块数}�?{无APS钢种列表})")
            序号 += 1
        if 减宽超标块数 > 0:
            提示信息列表.append(f"{序号}.减宽量超标{减宽超标块数}�?)
            序号 += 1
        if 逆宽轧制板坯�?> 0:
            提示信息列表.append(f"{序号}.逆宽轧制板坯{逆宽轧制板坯数}�?)
            序号 += 1
        if 低轧宽板坯数 > 0:
            提示信息列表.append(f"{序号}.有轧宽低�?30板坯{低轧宽板坯数}�?注意定宽最小辊缝要�?)
            序号 += 1
        if 极低轧宽板坯�?> 0:
            提示信息列表.append(f"{序号}.有低�?60的板坯{极低轧宽板坯数}�?注意E2设定�?)
            序号 += 1
        
        # 计算提示信息行数（先左右再上�?每行显示2条）
        if len(提示信息列表) == 0:
            提示信息行数 = 1  # 至少有一行（空行�?
        else:
            提示信息行数 = (len(提示信息列表) + 1) // 2  # 向上取整,每行2�?
        
        # 字段列名行号 = 提示信息起始�?1) + 提示信息行数
        字段列名行号 = 1 + 提示信息行数
        ...
        # 第二行及以下：计划号、块数、提示信息、打印时�?
        # 序号和钢卷号列合�?填充计划号信�?
        # 设置提示信息行的行高�?5pt = 500 twips�?
        for i in range(1, 字段列名行号):
            new_sheet.row(i).height = 500
            new_sheet.row(i).height_mismatch = True
        
        new_sheet.write_merge(字段列名行号 - 1, 字段列名行号 - 1, 序号列索�? 钢卷号列索引, f"计划号：{计划号}", style_header_info)
        
        # 牌号（钢级）列填充块数信�?
        if 牌号列索�?>= 0:
            new_sheet.write(字段列名行号 - 1, 牌号列索�? f"块数：{块数} �?, style_header_info)
        ...
        # 显示提示信息
        if 左区域起�?>= 0 and 左区域结�?>= 0 and 中间区域起始 >= 0 and 中间区域结束 >= 0:
            # 计算需要的行数（每行显�?条提示信息）
            提示信息总行�?= (len(提示信息列表) + 1) // 2  # 向上取整
            for i in range(提示信息总行�?:
                # 左列（坯宽到公差带）：显示第 i*2 条提示信�?
                左列索引 = i * 2
                if 左列索引 < len(提示信息列表):
                    new_sheet.write_merge(1 + i, 1 + i, 左区域起�? 左区域结�? 提示信息列表[左列索引], style_header_info)
                else:
                    new_sheet.write_merge(1 + i, 1 + i, 左区域起�? 左区域结�? "", style_header_info)
                
                # 中间列（粗轧报信到坯长）：显示第 i*2+1 条提示信�?
                中间列索�?= i * 2 + 1
                if 中间列索�?< len(提示信息列表):
        ...
        elif 左区域起�?>= 0 and 左区域结�?>= 0:
            # 如果只有左区�?
            for i, 提示信息 in enumerate(提示信息列表):
                new_sheet.write_merge(1 + i, 1 + i, 左区域起�? 左区域结�? 提示信息, style_header_info)
        ...
        # 右区域（中厚到装炉顺）显示打印时�?
        if 右区域起�?>= 0 and 右区域结�?>= 0:
            new_sheet.write_merge(字段列名行号 - 1, 字段列名行号 - 1, 右区域起�? 右区域结�? f"打印时间：{当前时间}", style_time)
        ...
        for row_idx, row_data in enumerate(all_row_data, 字段列名行号 + 1):
            ...
            # 根据内容长度计算行高（至�?5pt = 500 twips�?
            if 粗轧报信列索�?>= 0:
                粗轧报信�?= row_data.get("粗轧报信", "")
                if 粗轧报信�?
                    内容 = str(粗轧报信�?
                    # 按实际显示规则计算行数：每行�?3个字符（考虑中英文混合）
                    # 英文和数字占宽度较少,中文占宽度较�?
                    def 计算显示宽度(文本):
                        宽度 = 0
                        for 字符 in 文本:
                            if 字符.isascii():
                                宽度 += 0.6  # 英文字符和数字占0.6个中文宽�?
                            else:
                                宽度 += 1  # 中文字符�?个宽�?
                        return 宽度
                    
                    总宽�?= 计算显示宽度(内容)
                    每行宽度 = 13  # 每行�?3个中文宽度（根据实际Excel显示调整�?
                    # 计算需要的行数
                    需要行�?= max(1, int((总宽�?+ 每行宽度 - 1) // 每行宽度))
                    # 增加安全系数：只有内容较长时才增�?�?短内容不增加
                    # 判断标准：如果最后一行接近满行（超过80%宽度�?则增�?�?
                    最后一行宽�?= 总宽�?% 每行宽度
                    if 最后一行宽�?== 0:
                        最后一行宽�?= 每行宽度
                    if 最后一行宽�?> 每行宽度 * 0.8:
                        需要行�?= 需要行�?+ 1
                    # 行高计算：基础行高25pt(500 twips),多行时每行增�?8pt(360 twips)
                    # 1�?25pt, 2�?25+18=43pt, 3�?25+18+18=61pt, 以此类推
                    基础行高 = 500  # 25pt
                    每增加行�?= 360  # 18pt
                    # 计算总行高（至少25pt�?
                    计算行高 = 基础行高 + (需要行�?- 1) * 每增加行�?
                    # 设置行高（根据内容动态调�?最�?5pt�?
                    new_sheet.row(row_idx).height = 计算行高
                    new_sheet.row(row_idx).height_mismatch = True
                else:
                    # 无内容时使用默认行高�?5pt = 500 twips�?
            ...
            顺序�?= row_idx - 字段列名行号  # �?开始的顺序�?
            
            # 写入顺序�?
            new_sheet.write(row_idx, 0, 顺序�? style_data_12pt_left_border)
            ...
            for col_idx, col_name in enumerate(target_columns[1:], 1):
                ...
                if value == "" or value is None:
                    ...
                    alternative_names = {
                        ...
                        "�?: ["�?, "公差�?, "宽度公差"],
                        "层号": ["层号", "除鳞", "除鳞标记"],
                        "中间坯厚度设定�?: ["中间坯厚度设定�?, "中厚", "中间坯厚�?],
                        "RT2目标�?: ["RT2目标�?, "RT2", "RT2�?],
                        "硬度�?: ["硬度�?, "强度", "硬度"],
                        "宽度负公�?: ["宽度负公�?, "负公�?, "公差�?],
                        "宽度正公�?: ["宽度正公�?, "正公�?, "公差�?],
                        "板坯炉后拒收次数": ["板坯炉后拒收次数", "回炉�?, "拒收次数"],
        ...
        # 10. 保存新文�?
        new_file_path = file_path  # 覆盖原文�?
        
        try:
            new_workbook.save(new_file_path)
            print(f"文件已保�? {new_file_path}")
        except Exception as e:
    ...
    def safe_read_excel(self, file_path, max_retries=3, retry_delay=0.5):
        ...
        Returns:
            tuple: (success, result),success为布尔�?result为Workbook对象或错误信�?
        import xlrd
        import time
    ...
    def load_aps_grades(self):
        ...
        try:
            ...
            print(f"已加�?{len(aps_grades)} 个APS钢种")
        except Exception as e:
        ...
        try:
            ...
            if not os.path.exists(excel_file):
                ...
                if os.path.exists(backup_excel_file):
                    ...
                    print(f"备份计划目录中也不存在装炉顺序文�? {backup_excel_file}")
                    # 更新空表�?
                    self.plan_data = []
                    self.update_table()
                    self.stats_label.setText("总块�? 0 | 无文�? 0")
                    return False
            ...
            for idx, header in enumerate(headers):
                if '装炉顺序�? in header or '装炉顺序' in header:
                    order_col = idx
                    print(f"找到装炉顺序�? {idx}")
                elif '计划�b7 in header:
                    plan_col = idx
                    print(f"找到计划号列: {idx}")
                elif '钢卷�? in header:
                    coil_col = idx
            ...
            if None in [order_col, plan_col, coil_col]:
                print(f"缺少必要�? order_col={order_col}, plan_col={plan_col}, coil_col={coil_col}")
            ...
            print(f"开始读取数�?总行�? {sheet.nrows}, 限制行数: {max_rows}")
            for row in range(1, max_rows):
                try:
                    if row % 100 == 0:  # �?00行打印一�?减少输出
                        print(f"正在读取�?{row} �?..")
                    ...
                    if plan_no and coil_no:
                        ...
                        if row % 50 == 0:  # �?0行打印一�?减少输出
                            print(f"加载数据: 装炉顺序={order}, 计划�b7{plan_no}, 钢卷�?{coil_no} (�?{row})")
                    else:
                        if row % 100 == 0:  # �?00行打印一�?减少输出
                            print(f"跳过空数据行: 计划�b7'{plan_no}', 钢卷�?'{coil_no}' (�?{row})")
                except Exception as e:
                    print(f"读取�?{row} 失败: {e}")
                    # 继续处理下一�?不中断整个加载过�?
                    continue
            print(f"数据读取完成,共加�?{len(plan_data)} 条数�?)
        except Exception as e:
            ...
            print(f"加载�?{len(plan_data)} 条装炉数�?)
            if len(plan_data) == 0:
                print("装炉顺序.xls文件中没有数�?)
                # 更新空表�?
                self.plan_data = []
                self.update_table()
                self.stats_label.setText("总块�? 0 | 无文�? 0")
            ...
            # 获取backup文件夹中的所有xls文件（排�?原始文件"子目录）
            backup_dir = os.path.join(plan_dir, "backup")
            if os.path.exists(backup_dir):
            ...
            print(f"计划号文件夹和backup文件夹中找到 {len(plan_files)} 个文�?)
                            ...
                            try:
                                ...
                                for row_idx in range(min(10, sheet.nrows)):
                                    ...
                                    for col_idx in range(sheet.ncols):
                                        ...
                                        if any(field in cell_value for field in ["序号", "钢卷�?, "牌号", "坯厚", "轧厚", "轧宽", "装炉�?]):
                                ...
                                if field_name_row != -1:
                                    ...
                                    if roll_width_col != -1:
                                        # 检查数据行中的轧宽�?
                                        for row_idx in range(field_name_row + 1, min(field_name_row + 100, sheet.nrows)):  # 限制检查行�?
            ...
            # 更新表格
            self.update_table()
            self.stats_label.setText(f"总块�? {total_count} | 无文�? {no_file_count}")
            
            print(f"成功加载 {len(self.plan_data)} 个计划号,总计 {total_count} �?无文�?{no_file_count} �?)
            return True
    ...
    def update_table(self):
        ...
        for item in self.plan_data:
            ...
            if hasattr(self, 'processed_plans') and isinstance(self.processed_plans, dict) and plan_no in self.processed_plans:
                # 对于D开头的计划号，需要检查是否有已处理的钢卷�?
                if plan_no.startswith('D') or plan_no.startswith('d'):
                    if self.processed_plans.get(plan_no, set()):
                        status_text += f"[已处理] "
                else:
                    # 对于非D开头的计划号，只要有记录（即使是空set）就认为已处�?
                    status_text += f"[已处理] "
            ...
            if hasattr(self, 'printed_plans') and isinstance(self.printed_plans, dict) and plan_no in self.printed_plans:
                # 对于D开头的计划号，需要检查是否有已打印的钢卷�?
                if plan_no.startswith('D') or plan_no.startswith('d'):
                    if self.printed_plans.get(plan_no, set()):
                        status_text += f"[已打印] "
                else:
                    # 对于非D开头的计划号，只要有记录（即使是空set）就认为已打�?
                    status_text += f"[已打印] "
    ...
    def start_auto_execution(self):
        ...
        try:
            ...
            if exec_mode == "interval":
                ...
                interval_ms = interval_minutes * 60 * 1000  # 转换为毫�?
                ...
                if self.next_execution_time:
                    ...
                    print(f"下一次执行时�? {time.strftime('%H:%M:%S', time.localtime(self.next_execution_time))}")
                    print(f"时间�? {time_diff} �?({time_diff/60:.2f} 分钟)")
            ...
            if exec_mode == "interval":
                ...
                self.next_execution_label.setText(f"下一次执�? {next_time_str}")
                print(f"设置标签�? 下一次执�? {next_time_str}")
                # 强制更新UI
            ...
            if exec_mode == "interval":
                ...
                if self.next_execution_time:
                    ...
                    if time_diff >= 0:
                        ...
                        print(f"警告：计算出的时间差为负�?{time_diff}，不设置定时�?)
                else:
                    print("警告：未能计算出下一次执行时�?)
        ...
        try:
            ...
            for time_str in sorted_time_strings:
                try:
                    ...
                    if exec_datetime > current_datetime:
                        ...
                        print(f"找到今天的下一次执行时�? {time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(next_time.timestamp()))}")
                        break
                except Exception as e:
            ...
            if not next_time:
                try:
                    ...
                    print(f"今天没有找到合适的时间，使用明天的第一个时�? {time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(next_time.timestamp()))}")
                except Exception as e:
            ...
            print(f"计算出的下一次执行时�? {time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(self.next_execution_time))}")
        except Exception as e:
            print(f"计算下一次执行时间失�? {str(e)}")
            self.next_execution_time = None
    ...
    def load_settings(self):
        """加载设置并重启自动执行服�?""
        try:
            print("MainWindow.load_settings被调�?)
            settings = self.get_settings()
            # 安全打印设置，避免编码错�?
            try:
    ...
    def show_auto_close_message(self, title, message, duration=3000):
        """显示自动关闭的消息框 - 可手动关闭，大窗口，大字�?
        
        Args:
            title: 消息框标�?
            message: 消息内容
            duration: 持续时间（毫秒），默�?000毫秒
        from PyQt5.QtWidgets import QMessageBox, QApplication
        ...
        # 使用exec_()显示模态对话框，允许用户手动关�?
        msg_box.exec_()
        
        print(f"已显示自动关闭消息框，{duration}毫秒后关�?)
        return msg_box
    ...
    def update_next_execution_label(self):
        ...
        try:
            ...
            if not self.auto_exec_running:
                print("auto_exec_running为False，设置为'下一次执�? �?")
                self.next_execution_label.setText("下一次执�? �?)
                # 强制更新
                self.next_execution_label.repaint()
                self.next_execution_label.update()
                ...
                if hasattr(self, 'furnace_details_window') and self.furnace_details_window:
                    try:
                        if hasattr(self.furnace_details_window, 'next_execution_label'):
                            self.furnace_details_window.next_execution_label.setText("下一次执�? �?)
                            self.furnace_details_window.next_execution_label.repaint()
                            self.furnace_details_window.next_execution_label.update()
                            print("已更新装炉明细窗口的下一次执行时间为: �?)
                    except Exception as e:
                        print(f"更新装炉明细窗口的下一次执行时间失�? {str(e)}")
                return
            ...
            if exec_mode == "interval":
                ...
                next_exec_time = time.time() + (interval_minutes * 60)
                next_time_str = time.strftime("%H:%M:%S", time.localtime(next_exec_time))
                label_text = f"下一次执�? {next_time_str}"
                self.next_execution_label.setText(label_text)
                print(f"设置标签�? {label_text}")
            else:
                if self.next_execution_time:
                    next_time_str = time.strftime("%H:%M:%S", time.localtime(self.next_execution_time))
    ...
    def on_auto_exec_changed(self, state):
        ...
        try:
            ...
            print(f"处理自动执行复选框状态变化失�? {str(e)}")
    
    def on_auto_print_changed(self, state):
        """自动打印复选框状态变化处�?""
        try:
            enabled = state == Qt.Checked
            print(f"自动打印复选框状态变�? {enabled}")
    ...
    def show_context_menu(self, pos):
        ...
        # 添加打印菜单�?
        print_action = context_menu.addAction("打印")
        print_action.triggered.connect(self.print_selected)
    ...
    def show_selected(self):
        ...
        try:
            ...
            if not selected_items:
                QMessageBox.information(self, "提示", "请先在表格中选择一行数�?)
                return
            ...
            # 检查表格是否有足够的列
            if self.table_widget.columnCount() < 1:  # 需要至�?列（计划号）
                QMessageBox.warning(self, "错误", "表格列数不足，无法获取计划号")
                return
            
            # 获取计划号（计划号在�?列）
            plan_item = self.table_widget.item(row, 0)  # 计划号在第一�?
            if not plan_item:
                QMessageBox.warning(self, "错误", "无法获取计划�b7)
                return
            
            plan_no = plan_item.text().strip()
            if not plan_no:
                QMessageBox.warning(self, "错误", "计划号为�?)
                return
            
            print(f"尝试显示计划�b7 {plan_no}")
            ...
            # 如果有未处理的钢卷号，先处理该计�?
            if need_process:
                print(f"计划 {plan_no} 有未处理的钢卷号，正在处�?..")
                # 先选择当前�?
                self.table_widget.clearSelection()  # 清除之前的选择
                self.table_widget.selectRow(row)  # 选择当前�?
                # 确保选择状态已更新
                QApplication.processEvents()  # 处理事件，确保选择状态更�?
                # 调用处理计划方法
                self.process_plans(auto_print=False, show_result=False)
                # 再次检查是否有未处理的（验证处理成功）
                # 注意：对于非D开头的计划号，处理后应该已经标记为已处�?
                # 但由于处理后会保存状态，需要重新加�?
                self.load_processed_plans()  # 重新加载处理状�?
                plan_file2 = self.get_plan_file(plan_no)
                if plan_file2 and self.has_unprocessed_coils(plan_no, plan_file2):
                    # 处理失败，退�?
                    QMessageBox.warning(self, "错误", "计划处理失败，无法显�?)
                    return
                print(f"计划�b7{plan_no} 处理成功")
            ...
            if plan_file:
                ...
                error_msg = f"找不到计划号文件: {plan_no}.xls �?{plan_no}.xlsx"
                print(error_msg)
                QMessageBox.warning(self, "错误", error_msg)
        except Exception as e:
            error_msg = f"显示计划时发生错�? {str(e)}"
            print(error_msg)
            import traceback
            traceback.print_exc()
            QMessageBox.warning(self, "错误", error_msg)
    ...
    def get_plan_file(self, plan_no):
        ...
        if os.path.exists(backup_dir):
            ...
            if os.path.exists(file_path):
                print(f"找到计划号文�?backup): {file_path}")
                return file_path
        ...
        # 所有路径都找不�?
        print(f"找不到计划号文件: {plan_no}.xls �?{plan_no}.xlsx")
        return None
    ...
    def print_selected(self):
        ...
        try:
            ...
            selected_items = self.table_widget.selectionModel().selectedRows()
            if not selected_items:
                QMessageBox.warning(self, "警告", "请先在计划号列表中选择要打印的计划�b7)
                return
            ...
            # 构建选中计划号的文件路径列表
            selected_files = []
            update_progress("正在检查文�?..", f"�?{len(selected_plans)} 个计划号")
            ...
            for plan_no in selected_plans:
                ...
                if os.path.exists(file_path):
                    selected_files.append((plan_no, file_path))
                    update_progress("正在检查文�?..", f"找到文件: {plan_no}.xls")
                else:
                    ...
                    if os.path.exists(backup_file_path):
                        ...
                        update_progress("正在检查文�?..", f"从backup目录找到文件: {plan_no}.xls")
                        print(f"[OK] 从backup目录找到文件: {plan_no}.xls")
                    else:
                        update_progress("正在检查文�?..", f"文件不存�? {plan_no}.xls")
                        print(f"[ERROR] 文件不存�? {plan_no}.xls")
            ...
            update_progress("正在处理计划...", f"�?{len(selected_files)} 个计划需要处�?)
            ...
            for i, (plan_no, file_path) in enumerate(selected_files):
                ...
                if self.has_unprocessed_coils(plan_no, file_path):
                    ...
                    try:
                        # 处理单个计划 - 使用�?process_plans 相同的方�?
                        has_low_roll_width, has_no_aps = self.run_excel_macro_with_pandas(file_path)
                        update_progress("正在处理计划...", f"已处�? {plan_no}")
            ...
            if not selected_files:
                progress_window.close()
                self.show_auto_close_message("提示", "没有符合条件的计划可以打�?)
                return
            ...
            update_progress("正在打印文件...", f"�?{len(selected_files)} 个计划需要打�?)
            ...
            for i, (plan_no, file_path) in enumerate(selected_files):
                ...
                try:
                    ...
                    if self.has_unprinted_coils(plan_no, file_path):
                        ...
                        print(f"开始打�? {plan_no}.xls")
                        success = self.print_excel_file(file_path)
                        if success:
                            success_count += 1
                            success_files.append(plan_no)
                            update_progress("正在打印文件...", f"已打�? {plan_no}")
                            print(f"[OK] 已打�? {plan_no}.xls")
                            # 标记所有钢卷号为已打印
                        ...
                        update_progress("正在打印文件...", f"跳过已打�? {plan_no}")
                except Exception as e:
                    failed_count += 1
                    failed_files.append(plan_no)
            ...
            if success_count > 0 or failed_count > 0:
                # 构建结果消息
                result_message = f"成功: {success_count} �?| 失败: {failed_count} 个\n\n"
                if success_files:
                    result_message += f"成功的计划号: {', '.join(success_files)}\n"
                if failed_files:
                    result_message += f"失败的计划号: {', '.join(failed_files)}"
                
                # 使用自动关闭的消息框显示结果
                self.show_auto_close_message("打印完成", result_message, duration=3000)
        except Exception as e:
    ...
    def print_excel_file(self, file_path):
        ...
        try:
            ...
            # 先查找字段列名行（包�?序号"�?钢卷�?等字段名的行�?
            字段列名行号 = -1
            for row_idx in range(min(10, sheet.nrows)):  # 检查前10�?
                row_has_field_names = False
                for col_idx in range(sheet.ncols):
                    cell_value = str(sheet.cell_value(row_idx, col_idx))
                    # 检查是否包含常见字段名
            ...
            # 找到装炉顺列的索引（查找"装炉顺序�?列，因为实际存储的列名是"装炉顺序�?�?
            装炉顺列索引 = -1
            for col_idx, col_name in enumerate(current_columns):
                # 直接查找"装炉顺序�?列（实际存储的列名）
                if col_name == "装炉顺序�?:
            ...
            if 强度列索�?== -1:
                print("未找到强度列，使用默认打印范围：�?6�?)
                强度列索�?= 15  # 默认�?6列（索引15）为强度�?
            else:
            ...
            # 表头总行�?= 字段列名行号 + 1（包括字段列名行�?
            header_row_count = 字段列名行号 + 1
            
            print(f"字段列名行号: {字段列名行号}, 表头总行�? {header_row_count}")
            ...
            try:
                ...
                try:
                    ...
                    print(f"设置Visible属性失�? {str(vis_error)}")
                    # 即使设置失败也继续执�?
                ...
                col_start_letter = get_column_letter(print_col_start)  # 第一�?
                col_end_letter = get_column_letter(print_col_end)  # 装炉顺列对应的字�?
                # 打印所有行：从�?行到最后一行（包括表头和数据）
                ...
                print(f"打印区域: {print_area} (共{sheet.nrows}�?")
                
                # 设置纸张大小为美国Fanfold
                try:
                    # 强制使用 xlPaperFanfoldUS (137) - 美国连续折叠�?
                    ...
                    for paper_code in fallback_papers:
                        ...
                        try:
                            ...
                            try:
                                print(f"[纸张设置] �?成功设置备选纸�? {paper_code}")
                            except UnicodeEncodeError:
                                print(f"[纸张设置] 成功设置备选纸�? {paper_code}")
                            break
                        except Exception as e:
                            try:
                                print(f"[纸张设置] �?备选纸�?{paper_code} 设置失败: {str(e)}")
                            except UnicodeEncodeError:
                                print(f"[纸张设置] 备选纸�?{paper_code} 设置失败: {str(e)}")
                            continue
                ...
                try:
                    ...
                    print(f"退出Excel失败或没有Excel进程需要结�? {str(e)}")
    ...
    def print_plan(self, plan_no):
        ...
        try:
            # 找到计划号文�?
            plan_dir = os.path.join(self.plan_dir, "计划�b7, plan_no)
            if not os.path.exists(plan_dir):
    ...
    def auto_export(self, from_main_window=True):
        ...
        if hasattr(self, 'is_auto_export_running') and self.is_auto_export_running:
            print("自动导出已经在运�?跳过重复调用")
            self.show_auto_close_message("提示", "自动导出已在运行�?)
            return
        ...
        try:
            ...
            PYAUTOGUI_AVAILABLE = False
            from PyQt5.QtWidgets import QMessageBox
            QMessageBox.critical(self, "错误", "pyautogui未安�?请先安装: pip install pyautogui")
            self.is_auto_export_running = False
        ...
        print(f"检查是否在主程序画�? {main_window_title}")

        # 检查是否启用调试模�?
        debug_mode = self.coordinates.get("debug_mode", False)
        print(f"调试模式: {'开�? if debug_mode else '关闭'}")
        ...
        if unset_coords:
            ...
            # 检查应用程序是否在前台运行
            if is_in_foreground:
                QMessageBox.warning(self, "警告", f"以下坐标未设�?\n" + "\n".join(f"  - {n}" for n in unset_coords) + "\n\n请先设置这些坐标后再测试")
            else:
                print(f"以下坐标未设�? {unset_coords}\n请先设置这些坐标后再测试")
            self.is_auto_export_running = False
            if hasattr(self, 'auto_export_lock'):
                try:
                    self.auto_export_lock.release()
        ...
        def custom_event(event):
            ...
            if event.type() == QEvent.User and hasattr(event, 'valid_no_file_plans'):
                try:
                    ...
                    print(f"\n计划号明细导出完�? {len(exported_plans)} 个成�? {len(failed_plans)} 个失�?)
                    ...
                    if event.success:
                        ...
                        if event.exported_plans:
                            ...
                            try:
                                ...
                                # 调用处理计划方法，传入auto_print参数
                                # 注意：这里需要修改process_plans方法，使其不检查是否已处理�?
        ...
        def export_in_background():
            ...
            try:
                ...
                print(f"\n=== 自动导出开�?===")
                print(f"开始时�? {start_time.strftime('%Y-%m-%d %H:%M:%S')}")
                print(f"调试模式: {'开�? if debug_mode else '关闭'}")
                print("="*60)
                
                # 步骤0：系统健康检查和自动修复
                update_progress("【步�?/4】系统健康检查和自动修复...")
                print("\n===========================================================")
                print(" 【步�?/4】系统健康检查和自动修复")
                print("===========================================================")
                ...
                if not success1:
                    ...
                    # 显示激活窗口失败的错误信息
                    settings = self.get_settings()
                    test_window = settings.get("selectedWindow", "BGCMS1-宝钢股份多基地制造管理系统运行环�?)
                    export_finished(False, f"开始导出装炉顺�?..\n[1/9] 激活目标窗�? {test_window}...\n× 激活窗口失�?, [], [], {})
                    return
                
                update_progress("[√] 装炉顺序导出完成", "等待文件保存...")
                print("【导出完成】装炉顺序导出操作完�?)
                
                # 步骤2：导出总计划号列表
                update_progress("【步�?/4】正在导出总计划号列表...")
                print("\n=== 步骤2：导出总计划号列表 ===")
                ...
                # 步骤3：重新计算坐�?
                update_progress("【步�?/4】正在重新计算坐�?..")
                print("\n===========================================================")
                print(" 【步�?/4】重新计算坐�?)
                print("===========================================================")
                
                # �?try 块外部初始化 coord_map，确保步�?可以访问
                coord_map = None
                ...
                print(f"  存在: {'�? if os.path.exists(plan_list_file) else '�?}")
                ...
                if not os.path.exists(plan_list_file):
                    ...
                    try:
                        ...
                        if not coord_map:
                            ...
                            print(f"  计算�?{len(coord_map)} 个计划号坐标")
                            for plan_no, coord in list(coord_map.items())[:5]:  # 只显示前5�?
                                print(f"    {plan_no}: {coord}")
                            if len(coord_map) > 5:
                                print(f"    ... 还有 {len(coord_map) - 5} �?)
                            
                            # 特别输出无文件计划号的坐�?
                            no_file_plans = [item['plan_no'] for item in self.plan_data if item['status'] == '无文�?]
                            if no_file_plans:
                                print("\n无文件计划号的坐标：")
                            ...
                            update_progress("[√] 坐标计算完成", f"�?{len(coord_map)} 个计划号坐标")
                            print(f"[√] 坐标计算完成")
                            time.sleep(0.5)
                    except Exception as e:
                ...
                # 步骤4：导出无文件计划号的明细
                update_progress("【步�?/4】正在导出无文件计划号明�?..")
                print("\n===========================================================")
                print(" 【步�?/4】导出无文件计划号明�?)
                print("===========================================================")
                ...
                print(f"执行时间: {execution_time.total_seconds():.2f} �?)
                print("="*60)
                
                # 无论执行是否成功,都释放线程锁和重置运行标�?
                if hasattr(self, 'auto_export_lock'):
                    try:
                        self.auto_export_lock.release()
                    except:
                ...
                if not from_main_window:
                    # 自动执行完打开装炉明细画面，恢复之前的滚动状态及光标条位�?
                    print("30秒后打开装炉明细窗口...")
                    # 直接在单独的线程中打开装炉明细窗口，避免事件循环冲�?
                    import threading
    ...
    def export_zhuanglu_shunxu(self, add_debug_log=None):
        ...
        try:
            ...
            PYAUTOGUI_AVAILABLE = False
            add_debug_log('pyautogui 不可�?无法执行装炉顺序导出')
            return False
        ...
        for temp_file in glob.glob(os.path.join(plan_dir, "装炉顺序_*.tmp.xls")):
            try:
                os.remove(temp_file)
                add_debug_log(f"[√] 已清理临时文�? {os.path.basename(temp_file)}")
            except Exception as e:
        ...
        if custom_title:
            ...
            window_title = settings.get("selectedWindow", "BGCMS1-宝钢股份多基地制造管理系统运行环�?)
        ...
        add_debug_log(f"装炉顺作成管理标�? {zhuanglu_tab}")
        add_debug_log(f"装炉顺序导出按钮: {zhuanglu_export}")
        add_debug_log("")
        
        # 步骤1: 激活目标窗�?
        log_lines.append(f"[1/9] 激活目标窗�? {window_title}...")
        add_debug_log(f"【步�?】激活目标窗�? {window_title}")
        try:
            if not self.activate_window(window_title):
                log_lines.append("[×] 激活窗口失�?)
                add_debug_log("[×] 激活窗口失�?)
                print(f"[×] 导出失败: 激活窗�?{window_title} 失败")
                return False
            log_lines.append(f"[√] 已激活窗�? {window_title}")
            add_debug_log(f"[√] 已激活窗�? {window_title}")
            add_debug_log(f"  延迟: 500ms")
            time.sleep(0.5)  # 激活窗口后延迟
        except Exception as e:
            log_lines.append(f"[×] 激活窗口失�? {str(e)}")
            add_debug_log(f"[×] 激活窗口失�? {str(e)}")
            print(f"[×] 导出失败: 激活窗�?{window_title} 失败")
            return False
        ...
        try:
            # 步骤2: 鼠标点击 - 装炉顺作成管理画�?
            log_lines.append(f"[2/9] 鼠标点击: 装炉顺作成管理画�?@ {zhuanglu_tab}...")
            add_debug_log(f"【步�?】鼠标点�? 装炉顺作成管理画�?)
            ...
            add_debug_log(f"【步�?】鼠标点�? 导出按钮")
            add_debug_log(f"  坐标: ({zhuanglu_export[0]}, {zhuanglu_export[1]})")
            pyautogui.moveTo(zhuanglu_export[0], zhuanglu_export[1])
            pyautogui.click()
            add_debug_log(f"  延迟: 1000ms (等待保存对话�?")
            time.sleep(1.0)  # 点击后延�?等待保存对话框弹�?
            log_lines.append("导出按钮点击完成,等待保存对话�?..")
            add_debug_log("[√] 导出按钮点击完成")

            # 步骤6: 直接输入完整路径+文件�?
            log_lines.append("[6/9] 直接输入完整路径+文件�?..")
            add_debug_log(f"【步�?】直接输入完整路�?文件�?)
            # 等待对话框完全弹出并获取焦点
            add_debug_log(f"  延迟: 1000ms (等待对话�?")
            time.sleep(1.0)
            # 构造临时文件路径（不带扩展名）
            ...
            add_debug_log(f"  延迟: 500ms")
            time.sleep(0.5)  # 输入后延�?
            log_lines.append(f"已输入完整路�? {temp_full_path}")
            add_debug_log("[√] 已输入完整路�?)

            # 步骤8: 键盘按键 - Return
            log_lines.append("[8/9] 键盘按键: Return...")
            add_debug_log(f"【步�?】键盘按�? Return")
        ...
        try:
            if os.path.exists(temp_zhuanglu_file):
                ...
                if success:
                    ...
                    add_debug_log("[√] 原子性文件替换成�?)
                    add_debug_log("[√] 文件缓存已清�?)
                    log_lines.append("原子性文件替换成�?)
                else:
                    add_debug_log("[×] 原子性文件替换失�?)
                    log_lines.append("原子性文件替换失�?)
            else:
                add_debug_log(f"[×] 临时文件不存�? {temp_zhuanglu_file}")
                log_lines.append("临时文件不存�?)
        except Exception as e:
            add_debug_log(f"[×] 原子性文件替换异�? {str(e)}")
            log_lines.append(f"原子性文件替换异�? {str(e)}")
        ...
        try:
            ...
            if import_result:
                ...
                try:
                    ...
                    conn.close()
                    print(f"�?成功导入 {count} 条装炉顺序数据到数据�?)
                    add_debug_log(f"[√] 成功导入 {count} 条装炉顺序数据到数据�?)
                except:
                    print(f"�?成功导入装炉顺序数据到数据库")
                    add_debug_log("[√] 成功导入装炉顺序数据到数据库")
            else:
                add_debug_log("[×] 装炉顺序数据导入数据库失�?)
        except Exception as e:
        ...
        try:
            self.refresh_plan_list()
            add_debug_log("[√] 计划号列表刷新完�?)
        except Exception as e:
            add_debug_log(f"[×] 计划号列表刷新失�? {str(e)}")
        
        add_debug_log(f"【导出完成】装炉顺序导出操作完�?)
    ...
    def export_zhizhi_plan_list(self, test_window=None, test_mode=False, add_debug_log=None):
        ...
        try:
            ...
            add_debug_log(f"  选择计划�b7 {plan_select}")
            ...
            try:
                ...
                print(f"[×] 选择计划号失�? {e},继续执行")
                add_debug_log(f"[×] 选择计划号失�? {e},继续执行")
            ...
            try:
                ...
                pyautogui.click()
                add_debug_log(f"  延迟: 1500ms (等待保存对话�?")
                time.sleep(1.5)
            except Exception as e:
            ...
            # 步骤7: 直接输入完整路径+文件�?
            try:
                print("[7/7] 直接输入完整路径+文件�?..")
                add_debug_log(f"")
                add_debug_log(f"[步骤7] 直接输入完整路径+文件�?)
                add_debug_log(f"  延迟: 500ms (等待对话�?")
                time.sleep(0.5)
                ...
                add_debug_log(f"  延迟: 500ms")
                time.sleep(0.5)  # 输入后延�?
                print(f"已输入完整路�? {temp_full_path}")
                add_debug_log(f"[√] 已输入完整路�?)
                ...
                print(f"[×] 输入完整路径+文件名失�? {e},继续执行")
                add_debug_log(f"[×] 输入完整路径+文件名失�? {e},继续执行")
            ...
            try:
                ...
                if os.path.exists(temp_total_plan_file):
                    ...
                    print(f"[×] 临时文件不存�? {temp_total_plan_file}")
                    add_debug_log(f"[×] 临时文件不存�? {temp_total_plan_file}")
            except Exception as e:
                print(f"[×] 原子性文件替换异�? {str(e)}")
                add_debug_log(f"[×] 原子性文件替换异�? {str(e)}")
            ...
            try:
                ...
                if import_success:
                    add_debug_log(f"[√] 总计划号列表数据导入数据库成�?)
                else:
                    add_debug_log(f"[×] 总计划号列表数据导入数据库失�?)
            except Exception as e:
                print(f"导入总计划号列表数据到数据库失败: {str(e)}")
                add_debug_log(f"[×] 导入总计划号列表数据导入数据库失�? {str(e)}")
    ...
    def export_single_plan_detail(self, plan_no, plan_coord, plan_detail_export_btn=None, test_window=None, add_debug_log=None):
        """导出单个计划号的明细 - 严格按照原程序步骤执�?
        
        参数:
            plan_no: 计划�b7
            plan_coord: 计划号的坐标 (x, y)
            plan_detail_export_btn: 导出计划明细按钮坐标,如果为None则使用配置中的坐�?
            test_window: 测试窗口标题
            add_debug_log: 调试日志函数
        ...
        try:
            ...
            add_debug_log(f"    开始执行导出操�?..")
            add_debug_log(f"    开始导出明�?..")
            ...
            try:
                ...
                PYAUTOGUI_AVAILABLE = False
                add_debug_log('pyautogui 不可�?无法执行计划明细导出')
                print(f'pyautogui 不可�?无法执行计划明细导出: {e}')
                result = False
                return result
            except Exception as e:
                PYAUTOGUI_AVAILABLE = False
                add_debug_log(f'pyautogui 初始化失�? {e}')
                print(f'pyautogui 初始化失�? {e}')
                result = False
                return result
            
            # 步骤1: 激活窗�?
            add_debug_log(f"    [1] 激活窗�?..")
            if not self.activate_window(test_window):
                add_debug_log(f"    [×] 激活窗口失�?)
                print("[×] 激活窗口失�?)
                return False
            add_debug_log(f"    [√] 窗口已激�?)
            add_debug_log(f"    延迟: 300ms")
            time.sleep(0.3)
            ...
            # 步骤2: 鼠标点击计划号坐�?
            add_debug_log(f"    [2] 点击计划号坐�? {plan_coord}")
            # 检查坐标是否有�?
            if plan_coord and len(plan_coord) == 2:
                ...
                if isinstance(plan_coord[0], (int, float)) and isinstance(plan_coord[1], (int, float)):
                    try:
                        ...
                        print(f"[×] 点击计划号坐标失�? {e}")
                        add_debug_log(f"    [×] 点击计划号坐标失�? {e}")
                else:
                    print("[×] 计划号坐标值无�?)
                    add_debug_log(f"    [×] 计划号坐标值无�?)
            else:
                print("[×] 计划号坐标无�?)
                add_debug_log(f"    [×] 计划号坐标无�?)
            ...
            if plan_detail_export_btn and len(plan_detail_export_btn) == 2:
                ...
                if isinstance(plan_detail_export_btn[0], (int, float)) and isinstance(plan_detail_export_btn[1], (int, float)):
                    try:
                        ...
                        pyautogui.click()
                        add_debug_log(f"    延迟: 500ms (等待对话�?")
                        time.sleep(0.5)  # 等待保存对话框弹�?
                    except Exception as e:
                        print(f"[×] 点击导出按钮失败: {e}")
                        add_debug_log(f"    [×] 点击导出按钮失败: {e}")
                else:
                    print("[×] 导出计划明细按钮坐标值无�?)
                    add_debug_log(f"    [×] 导出计划明细按钮坐标值无�?)
            else:
                print("[×] 导出计划明细按钮坐标无效")
                add_debug_log(f"    [×] 导出计划明细按钮坐标无效")
            
            # 步骤4: 键盘输入计划号（使用剪贴板粘贴）
            add_debug_log(f"    [4] 输入计划�b7 {plan_no}")
            try:
                import pyperclip
            ...
            try:
                pyautogui.press('return')
                add_debug_log(f"    延迟: 1000ms (等待确认对话�?")
                time.sleep(1)  # 等待确认对话�?
            ...
            # 模拟操作执行完成,直接标记为成�?
            # 不检查文件是否存�?只按照模拟操作导出了哪些计划�b7
            print(f"[√] 计划�b7{plan_no} 的明细文件已导出")
            result = True
    ...
    def custom_messagebox(self, title, message, msg_type='info', auto_close=None):
        ...
        Args:
            ...
            auto_close: 自动关闭时间（秒），None表示不自动关�?
    ...
    def import_zhuanglu_shunxu_to_db(self, file_path):
        ...
        for attempt in range(max_retries):
            try:
                ...
                for idx, header in enumerate(headers):
                    if '计划�b7 in str(header):
                        plan_col = idx
                    elif '钢卷�? in str(header):
                        coil_col = idx
                ...
                print(f"�?{attempt+1} 次尝试导入装炉顺序数据失�? {str(e)}")
                if attempt < max_retries - 1:
    ...
    def import_zhizhi_plan_list_to_db(self, file_path):
        ...
        for attempt in range(max_retries):
            try:
                ...
                print(f"�?{attempt+1} 次尝试导入总计划号列表数据失败: {str(e)}")
                if attempt < max_retries - 1:
                    print(f"等待 {retry_delay} 秒后重试...")
                    time.sleep(retry_delay)
                    retry_delay *= 2  # 指数退�?
    ...
    def read_zhizhi_plan_list_with_coords(self, add_debug_log=None):
        ...
        try:
            ...
            add_debug_log(f"  数据库路�? {self.db_path}")
            ...
            if not plan_data:
                add_debug_log(f"  [×] 数据库中无数�?)
                print(f"总计划号列表数据不存�?)
                conn.close()
                return None
            
            add_debug_log(f"  [√] 数据库读取成�?)
            print(f"读取总计划号列表数据并计算坐�?)
            
            # 从配置中读取第一个计划号坐标、间距和偏移�?
            ...
            first_y = first_plan[1] + first_plan_offset  # 应用偏移量校�?
            
            add_debug_log(f"")
            add_debug_log(f"[坐标计算配置]")
            add_debug_log(f"  第一个计划号原始坐标: ({first_plan[0]}, {first_plan[1]})")
            add_debug_log(f"  偏移�? {first_plan_offset}")
            add_debug_log(f"  校准后坐�? ({first_x}, {first_y})")
            add_debug_log(f"  计划号间�? {plan_spacing} 像素")
            
            print(f"第一个计划号原始坐标: ({first_plan[0]}, {first_plan[1]})")
            print(f"第一个计划号偏移�? {first_plan_offset}")
            print(f"第一个计划号校准后坐�? ({first_x}, {first_y})")
            print(f"计划号间�? {plan_spacing} 像素")
            ...
            for row_index, (plan_no, seq_value) in enumerate(plan_data):
                if plan_no:
                    # 直接使用行索引计算坐�?不依赖序号�?
                    # row_index �?开�?正好对应第一个计划号的索�?
                    coord_y = first_y + (row_index * plan_spacing)
                    coord_x = first_x
                    
                    # 将计划号转换为字符串作为键，确保后续查找时类型匹�?
                    plan_coord_map[str(plan_no)] = (coord_x, coord_y)
                    zhizhi_plan_list.append((seq_value, plan_no))
                    # 只显示前13�?避免日志过长
                    if row_index < 13:
                        add_debug_log(f"  计划�b7{plan_no}: 行索�?{row_index}, 坐标=({coord_x}, {coord_y})")
                    elif row_index == 13:
                        add_debug_log(f"    ... 还有 {len(plan_data) - 13} 个计划号")
            
            # 只打印总数,不打印每个计划号的详细信�?提高计算速度
            print(f"  共计�?{len(plan_coord_map)} 个计划号的坐�?)
            ...
            print(f"\n从总计划号列表计算�?{len(plan_coord_map)} 个计划号的坐�?)
            
            # 更新缓存
            self.zhizhi_plan_list = zhizhi_plan_list
            self.zhizhi_plan_coord_map = plan_coord_map
            add_debug_log(f"  [√] 已更新总计划号列表缓存: {len(zhizhi_plan_list)} 条记�?)
            print(f"已更新总计划号列表缓存: {len(zhizhi_plan_list)} 条记�?)
            
            add_debug_log(f"  [√] 坐标计算完成")
            return plan_coord_map
            
        except Exception as e:
            print(f"读取总计划号列表并计算坐标失�? {str(e)}")
            import traceback
            traceback.print_exc()
            return None
...
class FurnaceDetailsWindow(QMainWindow):
    ...
    def __init__(self, parent=None):
        ...
        # 列名
        columns = [
            "计划�b7, "钢卷�?, "牌号（钢级）", "坯宽", "减宽", "调宽", "轧宽", "公差�?,
            "粗轧报信", "除鳞", "坯厚", "坯长", "轧厚", "中厚", "RT2", "强度", "切边", "去向", "订宽",
            "坯头部宽", "坯尾部宽", "热轧产品分类", "炼钢钢种", "负公�?, "正公�?, "回炉�?, "原轧�?
        ]
        ...
        self.current_row = 0
        self.scroll_interval = 110  # 默认 110 �?
        ...
        self.next_execution_label.setFixedWidth(160)  # 进一步增加宽度，确保完全显示"下一次执�? 12:41:00"
        ...
        self.scroll_status_label.setFixedWidth(120)  # 增加宽度，容�?自动滚动已停�?
        ...
        self.scroll_time_edit.setFixedWidth(60)  # 增加宽度，容�?位数
        self.scroll_time_edit.setAlignment(Qt.AlignCenter)  # 文本居中
        # 设置占位符提�?
        self.scroll_time_edit.setPlaceholderText("滚动间隔(�?")
        # 光标进入时全选文本，确保不影响光标条
        ...
        if not hasattr(self, 'custom_annotation_combo_type'):
            self.custom_annotation_combo_type = "�?
        
        # 初始化自定义信息和换辊信�?
        self.custom_and_roll_change_info = {
            'roll_change': {},  # 换辊信息，格�? {row: True}
            'custom_info': {}    # 自定义信息，格式: {row: text}
        }
        
        # 加载保存的自定义信息和换辊信�?
        plan_dir = os.path.join(os.getcwd(), "计划�b7)
    ...
    def show_context_menu(self, event):
        ...
        find_coil_action = context_menu.addAction("查找钢卷�?)

        def on_find_coil_number():
            input_dialog = QInputDialog(self)
            input_dialog.setWindowTitle("查找钢卷�?)
                ...
                for i in range(self.table_widget.rowCount()):
                    ...
                    if item:
                        if item.text().strip() == coil_number.strip():
                            ...
                            found = True
                            print(f"找到钢卷�?{coil_number}，位于第 {i+1} �?)
                            break
        ...
        # 添加自定义标注条件菜单项
        custom_annotation_action = context_menu.addAction("添加自定义标注条�?)
        ...
        def on_add_custom_annotation():
            ...
            # 创建对话�?
            dialog = QDialog(self)
            dialog.setWindowTitle("添加自定义标注条�?)
            ...
            fields = ["牌号（钢级）", "坯宽", "减宽", "调宽", "轧宽", "公差�?, "粗轧报信", "除鳞", "坯厚", "坯长", "轧厚", "中厚", "RT2", "强度"]
            field_combo.addItems(fields)
            condition_layout.addWidget(field_combo)
            
            # 运算符下拉列�?
            operator_label = QLabel("运算�?")
            ...
            value_edit.setPlaceholderText("请输入�?)
            # 添加自动大写功能
            ...
            # 添加固定的自定义信息选项
            custom_info_options = ["", "注意事项", "特殊处理", "重点关注", "需要检�?, "优先处理", "加�?, "延迟", "暂停", "其他"]
            ...
            combo_combo.addItems(["�?, "�?, "新建"])
            # 设置之前保存的组合方�?
            if self.custom_annotation_combo_type in ["�?, "�?]:
            ...
            # 初始化条件组
            # 条件组格�? [{'conditions': [{'text': '条件1', 'enabled': True}, ...], 'combo_type': '�?, 'enabled': True}, ...]
            if not hasattr(self, 'custom_annotation_condition_groups'):
                # 兼容旧格�?
                if hasattr(self, 'custom_annotation_conditions') and self.custom_annotation_conditions:
                    ...
                    self.custom_annotation_condition_groups = [{
                        ...
                        'combo_type': self.custom_annotation_combo_type if hasattr(self, 'custom_annotation_combo_type') else '�?,
                        'enabled': True
                    }]
                else:
                    self.custom_annotation_condition_groups = []
            ...
            def update_list_display():
                ...
                for i, condition_group in enumerate(added_condition_groups):
                    ...
                    combo_type = condition_group.get('combo_type', '�?)
            ...
            def add_condition():
                ...
                if not value:
                    QMessageBox.warning(dialog, "提示", "请输入�?)
                    return
            ...
            def remove_condition():
                if not added_condition_groups:
                    QMessageBox.warning(dialog, "提示", "没有可删除的条件�?)
                    return
                
                # 获取选中的行
                selected_rows = condition_table.selectionModel().selectedRows()
                if not selected_rows:
                    QMessageBox.warning(dialog, "提示", "请选择要删除的条件�?)
                    return
            ...
            def apply_all_annotation_conditions():
                ...
                for condition_group in added_condition_groups:
                    ...
                    # 解析条件
                    parsed_conditions = []
                    if " �?" in combined_condition:
                        parsed_conditions = combined_condition.split(" �?")
                    elif " �?" in combined_condition:
                        parsed_conditions = combined_condition.split(" �?")
                    else:
        ...
        def apply_annotation_condition(condition):
            ...
            try:
                ...
                if matched_rows:
                    ...
                    QMessageBox.information(self, "完成", f"已为 {len(matched_rows)} 个符合条件的钢卷号添加标�?)
                else:
        ...
        if is_roll_changed or is_custom_info:
            ...
            def on_cancel_mark():
                ...
                if selected_rows:
                    ...
                    self.block_count_label.setText(f"0/{total_rows}")
                print(f"已取消标注，删除了第 {row} 行的自定义标注条�?)
            ...
            def on_change_roll():
                ...
                if selected_rows:
                    ...
                    self.block_count_label.setText(f"0/{total_rows}")
                print(f"已执行换辊操作，在第 {row} 行下方添加了换辊标记�?)

            change_roll_action.triggered.connect(on_change_roll)
            context_menu.addSeparator()

            custom_info_action = context_menu.addAction("添加自定义信�?)

            def on_add_custom_info():
                input_dialog = QInputDialog(self)
                input_dialog.setWindowTitle("添加自定义信�?)
    ...
    def load_furnace_data(self):
        ...
        try:
            ...
            if hasattr(self, 'parent') and self.parent and hasattr(self.parent, 'plan_dir'):
                plan_dir = os.path.join(self.parent.plan_dir, "计划�b7)
            ...
            for row in range(1, sheet.nrows):
                try:
                    ...
                    if plan_no and coil_no_clean:
                        ...
                        print(f"加载数据: 装炉顺序={order}, 计划�b7{plan_no}, 钢卷�?{coil_no}, 纯净钢卷�?{coil_no_clean}")
                    else:
                        print(f"跳过空数据行: 计划�b7'{plan_no}', 钢卷�?'{coil_no}', 纯净钢卷�?'{coil_no_clean}'")
                except Exception as e:
                    print(f"读取�?{row} 失败: {e}")
                    continue
            ...
            print(f"加载�?{len(furnace_data)} 条装炉数�?)
    ...
    def showEvent(self, event):
        ...
        if self.parent and hasattr(self.parent, 'next_execution_label') and hasattr(self, 'next_execution_label'):
            try:
                ...
                next_exec_text = self.parent.next_execution_label.text()
                self.next_execution_label.setText(next_exec_text)
                print(f"同步下一次执行时�? {next_exec_text}")
            except Exception as e:
                print(f"同步下一次执行时间失�? {str(e)}")
            ...
            for i in range(min(10, sheet.nrows)):
                ...
                for col in range(sheet.ncols):
                    try:
                        ...
                        row_values.append('')
                has_coil_col = any('钢卷�? in v for v in row_values)
    ...
    def read_plan_file(self, file_path, target_coil_no):
        ...
        try:
            import xlrd
            print(f"提取钢卷数据: {file_path}, 钢卷�? {target_coil_no}")
            workbook = xlrd.open_workbook(file_path)
            sheet = workbook.sheet_by_index(0)
            ...
            if header_row == 0:
                for i in range(min(5, sheet.nrows)):
                    ...
                    if len(non_empty) >= 5:
                        header_row = i
                        print(f"自动检测到列名�? {i+1} (包含{len(non_empty)}个非空�?")
                        break
            ...
            for idx, header in enumerate(headers):
                col_map[header] = idx
                print(f"找到�? {idx} (表头: {header})")
            
            if '钢卷�? not in col_map and 'coil_no' not in col_map:
                print("未找到钢卷号列，尝试自动检�?..")
            ...
            for row in range(header_row + 1, sheet.nrows):
                try:
                    ...
                    if coil_no_col != -1:
                        ...
                        print(f"尝试匹配钢卷�? 目标={target_coil_no}, 单元格�?{cell_value}, 清理�?{clean_cell_value}")
                        if clean_cell_value == target_coil_no or cell_value == target_coil_no:
                            ...
                            print(f"钢卷号不匹配: 目标={target_coil_no}, 单元格�?{cell_value}, 清理�?{clean_cell_value}")
                    else:
                        print("未找到钢卷号�?)
                except Exception as e:
                    print(f"读取单元格失�? {e}")
            ...
            if not found:
                print(f"未找到钢卷号: {target_coil_no} 在文�? {file_path}")
                print("文件中的钢卷�?")
                for row in range(header_row + 1, min(header_row + 11, sheet.nrows)):
                    if 'coil_no' in col_map:
                        try:
                        ...
                        try:
                            cell_value = str(sheet.cell_value(row, col_map['钢卷�?])).strip()
                            print(f"  �?{row}: {cell_value}")
                        except:
                            pass
                print("返回基本数据结构")
    ...
    def add_plan_data_to_table(self, plan_data, plan_no, coil_no):
        ...
        try:
            ...
            row_data = [
                ...
                plan_data.get('RT2', ''),
                plan_data.get('强度', ''),
                plan_data.get('切边方式', plan_data.get('切边', '')),  # 优先使用"切边方式"，其次使�?切边"
                plan_data.get('去向', ''),
                plan_data.get('订货宽度', plan_data.get('订宽', '')),  # 优先使用"订货宽度"，其次使�?订宽"
                plan_data.get('坯头部宽', ''),
                plan_data.get('坯尾部宽', ''),
            ...
            减宽�?= plan_data.get('减宽', '')
            try:
                减宽数�?= float(str(减宽�?.replace('Δ', '').replace('*', '').strip())
                if 减宽数�?>= 240:
                    标注信息.append(f"减宽量超�? {减宽数值}")
                elif 减宽数�?< 0:
                    标注信息.append("逆宽轧制板坯")
            except:
                pass
            ...
            轧宽�?= plan_data.get('轧宽', '')
            try:
                轧宽数�?= float(str(轧宽�?.replace('Δ', '').replace('*', '').strip())
                if 轧宽数�?< 930:
            ...
            for col_idx, value in enumerate(row_data):
                ...
                if 'Δ' in value_str or '*' in value_str:
                    ...
                    # 添加annotation属性，用于样式表识�?
                    item.setData(Qt.UserRole, True)
                # 除鳞字段中包�?�?�?无APS"的单元格也改背景色为红色
                elif col_name == "除鳞" and ('�? in value_str or '无APS' in value_str):
                    # 设置红色背景
                    item.setBackground(QBrush(QColor('#FF0000')))
                    # 添加annotation属性，用于样式表识�?
                    item.setData(Qt.UserRole, True)
                ...
                if col_idx == 2:  # 牌号列（索引2�?
                    ...
                    item.setTextAlignment(Qt.AlignCenter)  # 水平和垂直居�?
                
                self.table_widget.setItem(row_idx, col_idx, item)
            
            # 保存标注信息到行数据�?
            # 注意：这里我们需要将标注信息存储在一个可以被print_furnace_details方法访问的地�?
            # 我们可以使用QTableWidgetItem的setData方法来存储标注信�?
            # 在第0列（计划号列）的item中存储标注信�?
            ...
            print(f"已插入表格数�? {row_data}")
                
        except Exception as e:
            print(f"添加计划数据到表格失�? {str(e)}")
        ...
        try:
            ...
            for row_idx in range(self.table_widget.rowCount()):
                ...
                if not is_special_row:
                    # 正常数据行，获取钢卷�?
                    coil_item = self.table_widget.item(row_idx, 1)  # 钢卷号列（索�?�?
                    if coil_item:
            ...
            # 保存最�?次的表格数据
            combined_data = [table_data] + previous_table_data[:1]  # 只保留最�?次的数据
            
            # 保存数据
            with open(table_data_file, 'w', encoding='utf-8') as f:
                json.dump(combined_data, f, ensure_ascii=False, indent=2)
            
            print(f"已保存表格数�? {table_data_file}")
            print(f"当前表格数据: {[item['coil_number'] for item in table_data]}")
            if previous_table_data:
                print(f"上一次表格数�? {[item['coil_number'] for item in previous_table_data[0]]}")
        except Exception as e:
    ...
    def save_window_state(self):
        ...
        try:
            ...
            # 收集当前状�?- 找到当前行或最近的正常数据行的钢卷�?
            current_coil_number = ""
            target_row = self.current_row
            
            # 如果当前行是特殊行（换辊行或自定义信息行），向上查找最近的正常数据�?
            if self.table_widget.rowCount() > 0 and target_row < self.table_widget.rowCount():
                while target_row >= 0:
                    ...
                    if not is_special_row:
                        coil_item = self.table_widget.item(target_row, 1)  # 钢卷号列（索�?�?
                        if coil_item:
            ...
            print(f"已保存窗口状�? {state_file}")
            print(f"状态数�? {state_data}")
        except Exception as e:
            print(f"保存窗口状态失�? {str(e)}")
            import traceback
            traceback.print_exc()
            ...
            if os.path.exists(state_file):
                try:
                    ...
                    if self.is_scrolling:
                        ...
                        self.scroll_status_label.setText("自动滚动�?)
                        print("已恢复自动滚动状�?)
                    else:
                        # 确保按钮状态正�?
                        self.scroll_toggle_btn.setText("�?)  # 播放图标
                        self.scroll_status_label.setText("自动滚动已停�?)
                        print("已恢复自动滚动停止状�?)
                    
                    print(f"已加载窗口状�? {state_file}")
                    print(f"状态数�? {state_data}")
                except Exception as e:
                    print(f"加载窗口状态失�? {e}")
            else:
    ...
    def on_selection_changed(self):
        ...
        if selected_rows:
            ...
            print(f"当前行已更新�? {self.current_row}")
    ...
    def auto_scroll(self):
        """自动向上滚动 - 光标在窗口中间不动，数据往上滚�?""
        if self.table_widget.rowCount() == 0:
            return
        
        # 每次向上滚动一个钢卷号（一行）
        # 为了实现数据往上滚动，需要让选中行向下移�?
    ...
    def print_furnace_details(self):
        ...
        try:
                ...
                try:
                    with open(table_data_file, 'r', encoding='utf-8') as f:
                        ...
                        if combined_data and isinstance(combined_data, list):
                            # combined_data[0] 是当前数据，combined_data[1] 是上一次数�?
                        ...
                        if 标注信息:
                            for info in 标注信息:
                                if "无APS钢种" in info:
                                    无APS块数 += 1
                                    # 获取牌号
                                    grade_item = self.table_widget.item(row_idx, 3)  # 牌号�?
                                    if grade_item:
            ...
            if len(提示信息列表) == 0:
                ...
                提示信息行数 = (len(提示信息列表) + 2) // 3  # 向上取整,每行3�?
            ...
            try:
                ...
                def set_cell_style(cell, font_name='仿宋', font_size=12, bold=False, 
                    ...
                    # 设置边框
                    if border_all:
                        cell.Borders(1).LineStyle = 1  # 左边�?
                        cell.Borders(2).LineStyle = 1  # 右边�?
                        cell.Borders(3).LineStyle = 1  # 上边�?
                        cell.Borders(4).LineStyle = 1  # 下边�?
            ...
            for col_idx, width in enumerate(col_widths):
                if col_idx < len(headers):
                    sheet.Columns(col_idx + 1).ColumnWidth = width / 256  # win32com�?开�?
            ...
            if len(提示信息列表) > 0:
                ...
                # 打印时间区域（坯厚到强度�?
                打印时间起始 = 12  # 坯厚�?
                打印时间结束 = len(headers)  # 强度�?
                
                # 显示提示信息
                提示信息总行�?= (len(提示信息列表) + 2) // 3  # 向上取整，每行显�?�?
                for i in range(提示信息总行�?:
                    row_num = 2 + i  # 从第二行开�?
                    
                    # 左列（序号到牌号）：显示�?i*3 条提示信�?
                    左列索引 = i * 3
                    if 左列索引 < len(提示信息列表):
                        left_range = sheet.Range(sheet.Cells(row_num, 左区域起�?, sheet.Cells(row_num, 左区域结�?)
                        left_range.Merge()
                        left_range.Value = 提示信息列表[左列索引]
                        set_cell_style(left_range.Cells(1, 1), font_name='仿宋', font_size=14, bold=True, 
                                      align_horz='center', align_vert='center')
                    
                    # 中间列（坯宽到公差带）：显示�?i*3+1 条提示信�?
                    中间列索�?= i * 3 + 1
                    if 中间列索�?< len(提示信息列表):
                    ...
                    # 右列（粗轧报信到除鳞）：显示�?i*3+2 条提示信�?
                    右列索引 = i * 3 + 2
                    if 右列索引 < len(提示信息列表):
                        right_range = sheet.Range(sheet.Cells(row_num, 右区域起�?, sheet.Cells(row_num, 右区域结�?)
                        right_range.Merge()
            ...
            for col, header in enumerate(headers):
                cell = sheet.Cells(field_name_row, col + 1)  # win32com列号�?开�?
                cell.Value = header
                if col == len(headers) - 1:  # 强度字段�?2pt�?
            ...
            for row_idx, row_data in enumerate(rows):
                ...
                if not is_special_row:
                    ...
                    if coil_item:
                        ...
                        if coil_number:
                            ...
                            if is_all_print or coil_number in coil_sequence_map:
                                print(f"写入�?{print_row_idx - 2} �? {row_data}")
                                ...
                                # 写入其他字段（只写入到强度列，即�?7列）
                                for col_idx, value in enumerate(row_data, 1):  # 从row_data的第一个元素开始写�?
                                    if col_idx > 16:  # 只写入到强度列（�?6列）
                                        break
                                    
                                    cell = sheet.Cells(print_row_idx, col_idx + 1)  # win32com列号�?开�?
                                    cell.Value = value
                                    
                                    if col_idx == 2:  # 钢卷号列�?6pt，左对齐�?
                    ...
                    # 特殊行（换辊行或自定义信息行），不保存到文件�?
                    pass
            
            print(f"共写�?{print_row_idx - 3} 行数�?)
            
            # 保存当前表格数据
            self.save_current_table_data()
            
            # 设置打印区域（第一列到强度列）
            last_col_letter = chr(64 + len(headers))  # 列字母从A开�?
...
class SettingsWindow(QDialog):
                ...
                if current_settings.get("autoExec", False) and current_settings.get("execMode", "interval") == "time":
                    ...
                    if hasattr(self.parent, 'calculate_next_execution_time'):
                        ...
                        if hasattr(self.parent, 'next_execution_time') and self.parent.next_execution_time:
                            ...
                            self.parent.next_execution_label.setText(f"下一次执�? {next_time_str}")
                            # 强制更新UI
                            self.parent.next_execution_label.repaint()
                            self.parent.next_execution_label.update()
                            self.parent.repaint()
                            self.parent.update()
                            print(f"实时更新下一次执行时�? {next_time_str}")
    ...
    def load_settings(self):
        ...
        # 默认设置文件路径
        # 使用self.parent.plan_dir确保打包后也能正确读取配置文�?
        ...
        if default_settings is None:
            default_settings = {
                ...
                # 自动执行设置
                "autoExec": False,
                "execMode": "interval",  # interval �?time
                "intervalMinutes": 1,
                "execTimes": "16:00,16:02,16:04"
            }
    ...
    def apply_settings(self, settings):
        ...
        if "furnaceWindow" in settings:
            self.ui.furnaceWindowXEdit.setText(str(settings["furnaceWindow"].get("x", 388)))
            self.ui.furnaceWindowYEdit.setText(str(settings["furnaceWindow"].get("y", 819)))
        else:
            self.ui.furnaceWindowXEdit.setText("388")
        ...
        if "selectedWindow" in settings:
            if settings["selectedWindow"] == "BGCMS1-宝钢股份多基地制造管理系统运行环�?:
    ...
    def save_settings(self, settings=None):
        ...
        if settings is None:
            ...
            settings = {
                ...
                # 窗口选择
                "selectedWindow": "Doc1.docx - Word" if self.ui.windowRadio1.isChecked() else "BGCMS1-宝钢股份多基地制造管理系统运行环�?,
        ...
        # 保存设置到文�?
        # 使用self.parent.plan_dir确保打包后也能正确保存配置文�?
        if hasattr(self, 'parent') and hasattr(self.parent, 'plan_dir'):
            config_file = os.path.join(self.parent.plan_dir, 'settings.json')
        else:
            # 备用方案：确保使用正确的路径
        ...
        try:
            with open(config_file, 'w', encoding='utf-8') as f:
                json.dump(settings, f, ensure_ascii=False, indent=2)
            print(f"[DEBUG] 设置保存成功�? {config_file}")
        except Exception as e:
        ...
        if self.parent and hasattr(self.parent, 'update_next_execution_label') and hasattr(self.parent, 'load_settings'):
            try:
                ...
                if hasattr(self.parent, 'auto_print_checkbox'):
                    ...
                    print(f"已更新主窗口自动打印复选框状�? {auto_print}")
            except Exception as e:
                print(f"更新下一次执行时间失�? {e}")
                import traceback
                traceback.print_exc()
    ...
    def update_coordinate_config(self, settings):
        ...
        if "test_window" not in coord_config:
            coord_config["test_window"] = "BGCMS1-宝钢股份多基地制造管理系统运行环�?
        if "export_speed" not in coord_config:
            coord_config["export_speed"] = "�?
    ...
    def restore_defaults(self):
        ...
        # 显示提示
        QMessageBox.information(self, "提示", "已恢复默认设�?)
    
    def get_window_coordinates(self, window_type):
        """获取窗口坐标 - 激活选择的窗口，移动鼠标，以鼠标左键点击作为最终获取的坐标�?""
        try:
            import win32gui
            import win32api
            import win32con
            ...
            # 获取当前选择的窗�?
            selected_window = "Doc1.docx - Word" if self.ui.windowRadio1.isChecked() else "BGCMS1-宝钢股份多基地制造管理系统运行环�?
            print(f"获取坐标：激活窗�? {selected_window}")
    ...
    def test_window_coordinates(self, window_type):
        """测试窗口坐标 - 激活选择的窗口，按照文本框中的坐标值把鼠标移动到坐标位�?""
        try:
            import win32gui
            import win32api
            import win32con
            ...
            # 获取当前选择的窗�?
            selected_window = "Doc1.docx - Word" if self.ui.windowRadio1.isChecked() else "BGCMS1-宝钢股份多基地制造管理系统运行环�?
            print(f"测试坐标：激活窗�? {selected_window}")
