    ...
    def furnace_details(self):
        ...
        def print_furnace_details():
            ...
            try:
                ...
                # 写入提示行（第一行）
                sheet.write(0, 0, 提示行[0])
                # 合并第一行单元格（从第0列到第16列）
                sheet.write_merge(0, 0, 0, len(headers) - 1, 提示行[0])
                ...
                sheet.write(2, 0, f"打印时间: {print_time}")
                sheet.write_merge(2, 2, 0, len(headers) - 1, f"打印时间: {print_time}")
                
                # 写入数据（从第4行开始，第1行是提示行，第2行是字段名行，第3行是打印时间）
                data_row_start = 3
                seq_num = 1  # 序号计数器
                for row_idx, row_data in enumerate(rows):
                    ...
                    if is_special_row:
                        # 换辊行或自定义信息行：合并单元格显示内容
                        content = row_data[0] if row_data else ""
                        sheet.write_merge(data_row_start + row_idx, data_row_start + row_idx, 0, len(headers) - 1, content)
                    else:
                        # 正常数据行：写入序号和数据
                        sheet.write(data_row_start + row_idx, 0, seq_num)
                        seq_num += 1
                        # 写入数据（从第1列开始，第0列是序号）
                        for col_idx, value in enumerate(row_data):
                            if col_idx < len(headers) - 1:  # 只写入到强度列
                                sheet.write(data_row_start + row_idx, col_idx + 1, value)
