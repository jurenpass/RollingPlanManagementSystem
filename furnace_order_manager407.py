    ...
    def furnace_details(self):
        ...
        def print_furnace_details():
            ...
            try:
                ...
                style_data_seq.borders = borders_data
                ...
                style_data_14pt.alignment = alignment_data
                ...
                style_data_12pt.alignment = alignment_data
                ...
                col_widths = [
                    ...
                    int(8.0 * 256 * 1.04),       # 2. 计划号
                    int(20 * 256 * 1.04),       # 3. 钢卷号
                ...
                def determine_insert_position_and_sequence(coils, current_coils, previous_coils, is_all_print):
                    ...
                    for coil in coils:
                        if is_all_print:
                            ...
                            # 计算之前列表的长度
                            previous_length = len(previous_coils)
                            # 找到钢卷在当前列表中的位置
                            current_index = None
                            ...
                            if current_index is not None:
                                if current_index >= previous_length:
                                    # 在末尾添加，序号是之前最后一个序号加1
                                    if previous_coils:
                                        last_sequence = previous_coils[-1]['sequence']
                                        coil['display_sequence'] = str(last_sequence + (current_index - previous_length + 1))
                                    else:
                                        # 第一次打印，直接使用当前序号
                                        coil['display_sequence'] = str(coil['sequence'])
                                    ...
                                    if current_index < len(previous_coils):
                                        ...
                                        if previous_coils:
                                            ...
                                            previous_sequence = 0
                                    coil['display_sequence'] = f"{previous_sequence}>"
                        ...
                        if coil_data['coil_number'] == coil['coil_number']:
                            ...
                            for col_idx, value in enumerate(row_data):
                                if col_idx < len(headers) - 1:  # 只写入到强度列（不包括最后一列的打印时间）
                                    ...
                                    elif current_col in [4, 5, 6, 7, 9, 11, 15]:  # 坯宽,减宽,调宽,轧宽,粗轧报信,坯厚,RT2 (14pt)
                                        # 14pt样式
                                        sheet.write(data_row_start + new_rows_written, current_col, value, style_data_14pt)
                                    elif current_col in [1, 8, 10, 12, 13, 14, 16]:  # 计划号，坯长，轧厚，中厚，强度，除鳞，公差带 (12pt)
                                        # 12pt样式
